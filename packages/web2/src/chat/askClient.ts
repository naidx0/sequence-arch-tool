...
export function createAskTransport(fetchImpl: typeof fetch): AskTransport {
  return {
    async stream(request, { onEvent, signal }) {
      ...
      if (!response.ok) {
        /*
         * ── THE SERVER'S OWN SENTENCE, KEPT ───────────────────────────────
         *
         * This threw the body away and returned `body: null`, so a refusal the
         * server had explained in words reached the UI as a status code and
         * drew an EMPTY assistant turn. Owner, 2026-09-20, on two such turns:
         * "it also doesn't let me send prompts, how come?" — and the answer was
         * sitting in the response body the whole time:
         *
         *     {"error":"no repo attached — open a repository from the home
         *      screen first"}
         *
         * Reading it is best-effort and bounded by the same `catch` everything
         * else here uses: a refusal that cannot be parsed is still a refusal,
         * and the status is what survives. What must not happen is the reader
         * being shown nothing at all, which is the one outcome they cannot act
         * on.
         */
        ...
        try {
          ...
          /* Not JSON, or the body was already consumed. The status stands. */
        }
        return { outcome: 'error', status: response.status, body: null, ...(message ? { message } : {}) };
      }
