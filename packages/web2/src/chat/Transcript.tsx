...
import { answerText, splitThinking } from './splitThinking';
...
export interface TranscriptProps {
  ...
  onOpen?: (target: 'canvas' | 'ai-canvas' | 'rail' | 'review' | 'files' | 'browser' | 'terminal', turnId: TurnId) => void;
  /**
  ...
  /**
   * Decide one file of one proposal — `proposal/file-decide`. Absent ⇒ the
   * approval card is not drawn, and the ledger strip renders exactly as it did.
   * A card that cannot decide anything is worse than no card.
   */
  onDecideFile?: (proposalId: string, path: string, decision: 'accepted' | 'rejected') => void;
}
...
function Item({
  ...
  /* The proposals themselves, so an `edits` item can offer Accept/Reject where
     the edit was proposed rather than only in the review overlay. */
  proposals?: Record<ProposalId, FileEditProposal>;
  onDecideFile?: (proposalId: string, path: string, decision: 'accepted' | 'rejected') => void;
}) {
  switch (item.kind) {
    ...
    case 'edits': {
      /*
       * THE LEDGER SAYS WHAT CHANGED; THE CARD LETS YOU DECIDE.
       *
       * `proposal/file-decide` has existed since the review overlay was built,
       * and the overlay was the ONLY place it could be pressed — so the reader
       * saw "3 files changed" in the transcript and had to leave it to accept
       * anything. The card puts the decision where the edit was proposed, which
       * is the owner's `ai-diff` reference: a diff with accept and reject on it.
       *
       * BOTH, not one instead of the other. The ledger is the one-line summary
       * a reader scans past when they do not care; the card is what they open
       * into when they do. Replacing the ledger with a full diff per turn would
       * make every edit turn a wall.
       */
      const decide = onDecideFile;
      const cards = decide
        ? item.proposalIds
            .map((id) => proposals[id])
            .filter((p): p is FileEditProposal => p !== undefined)
        : [];
      return (
        <>
          <EditsLedger summary={item.summary} />
          {decide === undefined
            ? null
            : cards.map((proposal) => (
            <EditApprovalCard
    ...
    case 'prose': {
      ...
      /*
       * THINKING IS NOT THE ANSWER. Local models put their reasoning inline in
       * `content`, wrapped in <think> tags — `provider.ts:199` records it
       * against granite42-hermes, "316 characters of unseen reasoning" — and
       * nothing on this side had ever looked for them, so the tags and their
       * contents printed into the answer as literal text.
       *
       * Separated, not hidden: recessive ink and a label say "this is the
       * model thinking", where a display:none would say "the model did not say
       * this", which is false and is not what this surface is for. See
       * `splitThinking.ts`. A turn with no tags yields one answer segment and
       * renders byte-identically to before.
       */
      const segments = splitThinking(stripped);
      ...
      return (
        ...
        >
          {topologyTheater}
          {toolCards}
          {/* THE ANSWER IN ITS OWN LANGUAGE — see `proseBlocks`. Fences stay
              literal; outside them headings, lists, emphasis and a disclosure
              render as what they are (Decision 12). Quotes and tables are
              still not parsed, and the header there says why. */}
          {segments.map((segment, segIndex) => {
            /*
             * A THINKING SEGMENT IS WRAPPED, NOT REWRITTEN. The blocks inside
             * it go through the identical renderer — the model's thought may
             * contain a fence or a list and deserves to read as one — and the
             * wrapper is what carries the recessive ink and the label. Styling
             * the blocks themselves would mean a second copy of this whole
             * branch, which is how the two drift.
             */
            const inner = proseBlocks(segment.text).map((block, index) => (
            <Fragment key={index}>
              {block.kind === 'code' ? (
              ...
              {segIndex === 0 && index === 0 ? inlineStack : null}
            </Fragment>
            ));
            if (segment.kind === 'answer') return <Fragment key={segIndex}>{inner}</Fragment>;
            return (
              <section
                key={segIndex}
                className="thought"
              ...
              >
                {/* "Thinking…" while the tag is still open, "Thought" once it
                    has closed — the present tense has to be rewritten a second
                    later, and sheet 12.2 bans a transcript that rewrites
                    itself EXCEPT on the one row that is not yet part of the
                    record. This is that case. */}
                <span className="thought-label">
                  {segment.open ? 'Thinking…' : 'Thought'}
                </span>
                <div className="thought-body">{inner}</div>
              </section>
            );
          })}
          ...
          {/* THE ANSWER, NOT THE RECORD OF PRODUCING IT. Pasting a model's
              <think> block into a commit message or a ticket is never what the
              reader meant by Copy. */}
          {item.streaming ? null : <CopyButton text={answerText(stripped) || stripped || item.text} />}
        </div>
      );
    }
    ...
    case 'todos':
      /*
       * ABOVE the answer and above the rows — see `pushTodos`. It is what a
       * reader checks while the turn is still running, and scrolling past
       * eleven work rows to find out defeats the point of having it.
       */
      return <TodoList items={item.items} streaming={item.streaming} />;
...
/* ── One prose block, drawn as the thing it is ───────────────────────────
   Recursive on purpose: a disclosure's body is a list of blocks parsed by the
   same rules as the top level, and a code block inside it is the same code
   block it would be outside. */
...
function ProseBlockView({ block }: { block: ProseBlock }) {
  switch (block.kind) {
    ...
    case 'heading':
      /* h1…h6 from the level the model wrote; the type ladder in chat.css
         decides what each paints at. */
