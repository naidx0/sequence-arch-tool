...
export interface ComposerProps {
  ...
  /**
   * WHERE that repository is — its root path, shown beside the name under
   * the box the way ML Harness shows it (owner, 2026-09-13). Null when there
   * is no repository, and then nothing is drawn: a path row with no path is
   * a row that says "no path" in a mono font.
   */
  ...
  /** Turn the session lock on or off. Absent ⇒ the control is not drawn. */
  onSessionLockToggle?: (on: boolean) => void;
...
export function Composer(props: ComposerProps) {
   ...
   * time — fetched on every render would be a request per keystroke.
   *
   * ONE READ ON MOUNT, ADDED 2026-09-13, and the sentence it replaces said "a
   * list fetched on mount would be stale by the time anyone looked". That was
   * right about the MENU and wrong about the TRIGGER. The owner: "when you add
   * a nickname to a model, in your chat box it should show the nickname, not
   * the model direct signature" — and the trigger has to name who is about to
   * answer BEFORE anyone opens anything, so the nickname cannot wait for the
   * menu that displays it.
   *
   * It cannot go stale the way that sentence feared, either: a nickname only
   * changes through the rename inside this very component, and a model switch
   * comes through here too, so both write `modelRows` as they happen. The menu
   * still re-reads on open, which is where a change made in another window
   * would land.
   *
  ...
  /*
   * ── RENAMING AT THE POINT OF CHOICE ───────────────────────────────────
   *
   * ml-harness `ConnectModel.tsx` puts the rename inside the picker, and its
   * comment says why: you notice that two rows are indistinguishable at the
   * moment you are trying to pick between them, and being sent to another
   * screen to fix it is how a thirty-second job becomes one nobody does.
   *
   * Its four rules are the ones implemented here. Enter and blur save. Escape
   * abandons and puts the old name back. An empty name is a NO-OP, not a row
   * with nothing written on it. And a save the server refused KEEPS THE BOX
   * OPEN WITH WHAT WAS TYPED — discarding somebody's words because a request
   * failed is the one thing this control must not do.
   */
  ...
  /* The one mount read. Failure is silent HERE and only here: the trigger
     falls back to the wire id, which is true, just less friendly. The same
     read failing on OPEN is reported loudly, because a menu that draws nothing
     is indistinguishable from one with nothing to draw. */
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      const answer = await modelPicker.list();
      if (cancelled || !modelLive.current) return;
      if (answer.outcome === 'ok') setModelRows((rows) => rows ?? answer.profiles);
    })();
    return () => {
      cancelled = true;
    };
  ...
  /*
   * WHO IS ANSWERING, IN THE READER'S OWN WORD FOR IT.
   *
   * Matched on the WIRE ID rather than on the row's `active` flag: after a
   * switch the server has confirmed a model but the rows still carry the old
   * flag, and the flag would name the previous one for as long as that lasted.
   * The id is the thing both sides agree about.
   */
  const activeWireId = switchedModel ?? composer.model.model;
  const activeNickname =
    activeWireId === ''
      ? null
      : (modelRows ?? []).find((row) => row.model === activeWireId)?.nickname ?? null;
  const modelTriggerLabel = activeNickname ?? switchedModel ?? modelLabel(composer.model);
  ...
  function startRename(row: ModelPickerProfile) {
    setRenameDraft(row.nickname);
    setRenamingId(row.id);
    setModelMenuFailure(null);
  }

  async function saveRename(row: ModelPickerProfile) {
    if (renameBusy) return;
    const wanted = renameDraft.trim();
    /* EMPTY IS A NO-OP and so is "the same name". Neither is a change, and
       neither is worth a round trip. */
    if (wanted === '' || wanted === row.nickname) {
      setRenamingId(null);
      return;
    }
    setRenameBusy(true);
    const answer = await modelPicker.rename(row.id, wanted);
    if (!modelLive.current) return;
    setRenameBusy(false);
    if (answer.outcome === 'ok') {
      setModelRows(answer.profiles);
      ...
      /* The box stays open, holding what was typed. */
      setModelMenuFailure(answer.message);
    }
  }
  ...
  function runSlash(command: (typeof slashHits)[number]): void {
    ...
    /* Named rather than `else`: the registry has a third kind now (`goal`),
       which this composer does not offer and cannot perform — `slashCommands`
       is called here WITHOUT the `goal` option, so no such row can reach this
       function. The check is what makes that fact checkable instead of a
       silent fall-through into a mode change nobody asked for. */
    /* AND `teach` IS THE FOURTH KIND, for the same reason and with the same
       proof: `slashCommands` is called here with the PERMISSION modes, so a
       teach row cannot be built. Teach is not a permission value on the wire
       - it is its own flag - and this composer has no `onTeachToggle`, so
       narrowing here is what stops a mode it cannot perform from arriving as
       a permission change nobody asked for. */
    else if (command.source.kind === 'mode' && command.source.mode !== 'teach')
      onPermissionChange(command.source.mode);
  }
    ...
    <div className="chat-scope composerwrap">
      <div className="inner">
          ...
          <div className="ctrls">
            ...
            <span className="menuwrap" ref={modelWrap}>
              ...
              >
                ...
                {/* The nickname is a NAME, so it sets in the UI face; a bare
                    wire id is an identifier and keeps the mono it has always
                    had. The two are different kinds of thing and reading
                    different tells you which one you are looking at. */}
                <span className={activeNickname === null ? 'mono' : 'nick'}>{modelTriggerLabel}</span>
                  ...
                  {(modelRows ?? []).map((row) =>
                    renamingId === row.id ? (
                      <span
                        ...
                        data-testid="composer-model-renaming"
                        data-profile={row.id}
                      >
                        <span className="stack">
                          <input
                            className="menurename"
                            data-testid="composer-model-rename-input"
                            value={renameDraft}
                            autoFocus
                            disabled={renameBusy}
                            spellCheck={false}
                            ...
                            onKeyDown={(e) => {
                              ...
                              if (e.key === 'Escape') {
                                /* ABANDON. The old name goes back, and the
                                   Escape does not also close the whole menu —
                                   it undoes the edit it opened. */
                                e.stopPropagation();
                                setRenameDraft(row.nickname);
                                setRenamingId(null);
                              }
                          ...
                          />
                          <span className="sub mono">{row.model}</span>
                        </span>
                      </span>
                    ) : (
                      <span key={row.id} className="menurow" role="none">
                        <button
                          type="button"
                          role="menuitem"
                          className="menuitem tall"
                        ...
                        >
                          <span className="stack">
                            {/* WHAT THE READER CALLS IT, over the wire id it
                                resolves to. Two facts, two lines: the name
                                alone cannot be checked against what is
                                configured, and the id alone is a wire string. */}
                            <span>{row.nickname}</span>
                            <span className="sub mono">{row.model}</span>
                          </span>
                          {/* A WORD, not a colour — sheet 12.5. */}
                          {row.active ? <span className="right">in use</span> : null}
                        </button>
                        <button
                          type="button"
                          role="menuitem"
                          className="menurename-btn"
                          data-testid="composer-model-rename"
                          data-profile={row.id}
                          aria-label={'Rename ' + row.nickname}
        ...
        <div className="undercomposer">
          ...
          {/*
            THE SESSION LOCK — "the chat controls the other tabs".
            On this row and not in a menu: it changes what the app does while
            you watch, so it has to be visible while you watch. Drawn only when
            a handler exists, the rule this row already follows for Teach — a
            control that cannot act must not be drawn dead.
          */}
          {onSessionLockToggle ? (
            <button
              type="button"
              className="lockchip"
              data-testid="composer-session-lock"
              data-on={composer.sessionLocked ? 'true' : undefined}
              aria-pressed={composer.sessionLocked}
              title={
                composer.sessionLocked
                  ? 'This chat is driving the tabs. Click to stop.'
                  : 'Let this chat open the tab each answer lands on.'
              }
              onClick={() => onSessionLockToggle(!composer.sessionLocked)}
            >
              {/* `link`, not a padlock: the book has no lock glyph and adding
                  one is its own Graphite decision (test/icon-book.test.ts
                  refuses a name the book does not define). `link` is the better
                  word anyway — the lock BINDS this chat to the surfaces, it
                  does not secure anything. */}
              <Icon name="link" size={12} />
              <span>{composer.sessionLocked ? 'Driving' : 'Drive'}</span>
            </button>
          ) : null}
          ...
          {!live && sessionStrip ? (
            ...
            <span className="branch" data-testid="composer-grounding">
              ...
              {grounding !== null && groundingPath !== null ? (
                <span className="branch-path" data-testid="composer-grounding-path" title={groundingPath}>
                  {'\u200E'}
                  {groundingPath}
                  {'\u200E'}
                </span>
          ...
          {/* The one click, and ONLY in Teach mode. It is not a general
              ...
              ON THIS LINE, NOT ABOVE IT. Owner, 2026-09-13: "it shouldn't stack
              multiple different elements, it should all be within one element
              directly … it shouldn't keep lifting the chat box bar." It sat
              outside `.undercomposer-inner` and so opened a SECOND row, which
              pushed the composer up the moment Teach was chosen — the control
              moving the thing it describes. It is a sibling of the mode and the
              grounding now, on the one rail under the box. */}
