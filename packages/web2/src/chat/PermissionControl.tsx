...
export const PERMISSION_MODES: readonly PermissionModeSpec[] = [
  {
    ...
    label: 'Plan',
    /*
     * PLAN IS THE OLD `propose`, under the name the owner uses for it: "make
     * sure proposed mode is plan mode". It is not read-only — it draws on the
     * AI Canvas and drafts a plan — so the description says what reaches your
     * FILES, which is the only consequence being consented to.
     */
    description: 'Draws and drafts. Changes need your accept.',
    icon: 'file',
  },
  {
    mode: 'build',
     ...
     * yes and it just goes." The shell is named out loud, because running a
     * command is the consequence that rejecting a proposal cannot undo.
     */
    description: 'Writes files and runs commands. No accept.',
    icon: 'sensitive',
  },
];

/** Shown under a refused Build row — point at Settings, which now exists. */
...
export const TEACH_ROW: Omit<PermissionModeSpec, 'mode'> = {
  label: 'Teach',
  /* One line at the 320px menu width — see `.menuitem.tall`. Three rows of
     three different heights was the ragged menu the owner met. */
  description: 'One concept a turn, drawn. Reads only.',
  icon: 'board',
};
  ...
  return (
    ...
    <span className="chat-scope menuwrap" ref={wrapRef}>
      <button
        type="button"
        className="permsel"
        data-teaching={teaching ? 'true' : 'false'}
        /*
         * THE PRODUCT'S OWN VOICE, in either of the two modes that have it.
         * Owner, 2026-09-13: "when you're in plan mode or teach mode it should
         * always glow purple — it shouldn't only be purple in teach mode."
         * Teach and Plan are the two modes where the turn READS and explains
         * rather than changes anything, which is what the accent is saying.
         */
        data-voice={teaching || control.mode === 'plan' ? 'true' : 'false'}
        /*
         * WHICH mode, for the hue. Owner, 2026-09-13: "add colours for each
         * mode — plan yellow, build red, teach purple. Simple." The tokens are
         * derived in graphite.css, where the cost of spending two verdict hues
         * on chrome is argued out loud.
         */
        data-tone={teaching ? 'teach' : control.mode}
