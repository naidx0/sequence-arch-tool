...
export type TranscriptItem =
  ...
  /**
   * THE TURN'S WORK LIST — its own object, above the answer, and not a work row.
   *
   * A work row is something that HAPPENED: past tense, one per event, appended
   * and never revised. A todo is something INTENDED, and the whole list is
   * replaced on every update. Folding them together would mean either revising
   * the record or appending the same three steps eight times — and the reader
   * would lose the one distinction that makes a long turn legible, between
   * what was done and what is still to do.
   *
   * Present only when the turn declared one, which is a minority of turns.
   */
  | { key: string; kind: 'todos'; turnId: TurnId; items: TodoItem[]; streaming: boolean }
  ...
  | {
      ...
      /**
       * The proposals this turn made, carried so the transcript can offer
       * Accept/Reject where the edit was PROPOSED rather than only in the
       * review overlay. The summary is counts; a decision needs the files and
       * their diffs, and deriving them a second time from the ledger would be a
       * second answer to what this turn actually changed.
       */
...
function pushAssistant(
  ...
  pushTodos(items, turn.id, turn.todos, false);
  ...
  if (edits) {
    items.push({
      ...
      turnId: turn.id,
      summary: edits,
      proposalIds: turn.evidence.proposals ?? [],
    });
  }
...
function pushInFlight(items: TranscriptItem[], turn: InFlightTurn, ctx: FoldContext, now: number): void {
  ...
  pushTodos(items, turn.turnId, turn.todos, true);
...
/**
 * THE WORK LIST, ABOVE THE ANSWER AND ABOVE THE ROWS.
 *
 * Above, because it is the thing a reader checks while the turn is still
 * running — "how far along is this?" — and scrolling past eleven work rows to
 * find out defeats the point of having it. Below the answer it would be a
 * summary of finished work, which is not what a list of intentions is for.
 *
 * Absent list ⇒ no item ⇒ every turn that never declared one renders exactly
