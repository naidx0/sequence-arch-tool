...
describe('item 2.7 — the send button is round', () => {
  it('is a 32px circle on the large rung, not a square with soft corners', () => {
    /* --control-h-lg, Decision 12. Was --control-h (28). */
    render(<Composer {...composerHandlers()} composer={composerSlice({ draft: 'x' })} />);

    const send = screen.getByTestId('composer-send');
    expect(resolvedStyle(send, 'width')).toBe(token('--control-h-lg'));
    expect(resolvedStyle(send, 'height')).toBe('32px');
    ...
    expect(parseFloat(resolvedStyle(send, 'border-radius'))).toBeGreaterThanOrEqual(16);
...
describe('item 2.7 — the control ladder and the placeholder', () => {
  it('keeps every control on ONE rung, and none of them is airy', () => {
    /*
     * Owner, 2026-09-13, first pass: "the buttons make bigger" — the controls
     * left the 24/26 rungs for --icon-btn-lg (30). Second pass the same day:
     * "make most buttons same size as well — top chat buttons, selected model
     * button, mode button, settings button; each thing should be bigger and
     * comparable to each other."
     *
     * So they share ONE rung rather than two neighbouring ones: --control-h-lg
     * (32), which the workspace pills and the send button already sat on. A row
     * of 30s beside a row of 32s is what "comparable" was asking about, and the
     * ragged edge it produced is what he was looking at.
     *
     * ASSERTED THROUGH THE TOKEN, so the rung can move without this case
     * needing a new number — and asserted as EQUAL to each other, which is the
     * claim he actually made.
     */
    render(<Composer {...composerHandlers()} composer={composerSlice({ draft: 'x' })} />);

    const rung = token('--control-h-lg');
    for (const id of ['composer-plus', 'composer-terminal', 'composer-model', 'composer-send']) {
      expect(resolvedStyle(screen.getByTestId(id), 'height'), `${id} is off the rung`).toBe(rung);
    }
    expect(rung).toBe('32px');
...
describe('teach mode lives in the mode menu (owner, 2026-09-02)', () => {
  ...
  it('picking a permission row while teaching leaves Teach', () => {
    ...
    fireEvent.click(screen.getByTestId('permission-control'));
    /* Plan is the old Propose under the name the owner uses (2026-09-13). The
       claim is unchanged: choosing a permission row while teaching LEAVES
       teach, because the two answer one question and a turn cannot be both. */
    const plan = screen
      .getAllByTestId('permission-option')
      .find((row) => row.textContent?.includes('Plan'));
