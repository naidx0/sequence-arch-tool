...
describe('TeachKnownCards', () => {
  ...
  it('renders one radio per option and marks the chosen one', () => {
    ...
    /* With a value the three are folded into a chip (see below); unfold to
       read the radios. */
    fireEvent.click(screen.getByTestId('teach-known-summary'));
  ...
  it('ONCE CHOSEN, THE THREE FOLD INTO ONE CHIP that says the choice and unfolds on press', () => {
    /*
     * Owner, 2026-09-13: the cards "stay in your face" after the click. A
     * control that has been answered and keeps asking is the interrogation
     * bug in a quieter costume. With a value the grid is gone and one chip
     * names the choice; pressing it brings the three back, with the chosen one
     * still marked, so the un-click rule above still applies.
     */
    const { rerender } = render(<TeachKnownCards value="used-it" onChange={() => {}} />);
    expect(screen.queryByTestId('teach-known')).toBeNull();
    const chip = screen.getByTestId('teach-known-summary');
    expect(chip.textContent).toContain("I've used it");
    expect(chip.getAttribute('aria-label')).toMatch(/change/i);

    fireEvent.click(chip);
    expect(screen.getByTestId('teach-known')).toBeTruthy();
    expect(screen.getByTestId('teach-known-used-it').getAttribute('aria-checked')).toBe('true');

    /* A new choice folds it again without a second press. */
    rerender(<TeachKnownCards value="new" onChange={() => {}} />);
    expect(screen.queryByTestId('teach-known')).toBeNull();
    expect(screen.getByTestId('teach-known-summary').textContent).toContain('New to it');

    /* Cleared has nothing to fold into: the three are back. */
    rerender(<TeachKnownCards value={null} onChange={() => {}} />);
    expect(screen.getByTestId('teach-known')).toBeTruthy();
    expect(screen.queryByTestId('teach-known-summary')).toBeNull();
  });
