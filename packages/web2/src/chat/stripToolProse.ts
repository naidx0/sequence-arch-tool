...
const TOOL_NAME_HIT =
  /"name"\s*:\s*"(propose_topology|propose_chart|propose_files|read_file|search_files|run_command|git_status|git_diff|call_mcp|call_plugin|fetch_url|board\.(?:add_note|add_card|connect|frame)|diagram\.(?:upsert|layout|delete)|canvas\.write_\w+|canvas\.set_story_route)"/;

const ALLOWED_TOOL =
  /^(propose_topology|propose_chart|propose_files|read_file|search_files|run_command|git_status|git_diff|call_mcp|call_plugin|fetch_url|board\.(?:add_note|add_card|connect|frame)|diagram\.(?:upsert|layout|delete)|canvas\.write_\w+|canvas\.set_story_route)$/;
...
function stripFences(text: string): StripToolProseResult {
  ...
  const stripped = text.replace(FENCED_ANY_RE, (full, lang: string, body: string) => {
    ...
    if (partialName || lang === 'sequence-tool' || lang === 'seqd') {
      /*
       * Empty / broken ```seqd fences are not topology — models dump them on
       * plot asks as architecture costume. Strip the fence; do not mint a
       * fake propose_topology card that reads as "seqd" trash in chat.
       */
      if (
        lang === 'seqd' &&
        !partialName &&
        (!trimmed || trimmed === '{' || !/"nodes"\s*:/.test(trimmed))
      ) {
        return '';
      }
...
export function isToolDumpCode(language: string | null, body: string): boolean {
  if (language === 'sequence-tool' || language === 'seqd') return true;
  const trimmed = body.trim();
...
function salvageOrphans(text: string): StripToolProseResult {
  ...
  const dangler =
    /(?:^|\n)[^<\n{]{0,80}?"name"\s*:\s*"(propose_topology|propose_chart|propose_files|read_file|search_files|run_command|git_status|git_diff|call_mcp|call_plugin|fetch_url|board\.(?:add_note|add_card|connect|frame)|canvas\.write_\w+|canvas\.set_story_route)"[\s\S]*?(?=\n(?:[A-Z*]|\d+\.|And |Here |The |Copy\b|Execution)|$)/g;
...
function salvageXmlTools(text: string): StripToolProseResult {
  const tools: StrippedTool[] = [];
  /* Models leak XML tool envelopes into chat — owner walk 2026-09-16. */
  const xml =
    /<(?:function|tool_use|tool_call|invoke)\b[^>]*\bname\s*=\s*["']([^"']+)["'][^>]*>[\s\S]*?<\/(?:function|tool_use|tool_call|invoke)>/gi;
  let stripped = text.replace(xml, (_full, name: string) => {
    const n = String(name).trim();
    if (n) tools.push({ id: n, name: n });
    return '\n';
  });
  /* Incomplete / self-closing forms mid-stream. */
  stripped = stripped.replace(
    /<(?:function|tool_use|tool_call|invoke)\b[^>]*\bname\s*=\s*["']([^"']+)["'][^>]*\/?>/gi,
    (_full, name: string) => {
      const n = String(name).trim();
      if (n && !tools.some((t) => t.name === n)) tools.push({ id: n, name: n });
      return '\n';
    },
  );
  /* Orphan closing tags / param shells after the open tag was stripped. */
  stripped = stripped.replace(/<\/?(?:param|parameter|arg|arguments)\b[^>]*>/gi, '');
  return { tools, stripped: stripped.replace(/\n{3,}/g, '\n\n') };
}
...
export function stripToolProse(text: string, opts?: { trimEnd?: boolean }): StripToolProseResult {
  ...
  const xml = salvageXmlTools(orphans.stripped);
  ...
  for (const t of xml.tools) {
    if (!byId.has(t.id)) byId.set(t.id, t);
  }
  const stripped =
    opts?.trimEnd === false
      ? xml.stripped
      : xml.stripped.replace(/\n{3,}/g, '\n\n').trimEnd();
  return { stripped, tools: [...byId.values()] };
}

export function toolCardTitle(name: string): string {
  if (name === 'propose_topology' || name.startsWith('diagram.')) {
    return 'Drawing architecture board';
  }
  if (name === 'propose_chart') return 'Drawing a chart';
  if (name.startsWith('board.')) return 'Drawing on Whiteboard';
