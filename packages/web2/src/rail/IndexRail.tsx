...
export interface IndexRailProps {
  ...
  /**
   * The local workspace listing, when there is no repository. `null`/absent
   * means the listing has not been asked for — which is NOT the same as an
   * empty folder, and the rail says a different sentence for each.
   *
   * A PROP, not a fetch. This file's own rule: "Fetching here would put a
   * second loader beside the store's and make 'the rail mounted' and 'the
   * index loaded' two different events that can disagree."
   */
...
export function IndexRail({
  ...
  if (graph === null) {
    /*
     * NO REPOSITORY IS NOT NOTHING. The rail used to answer this state with one
     * sentence about opening a repository — true, and not an answer to "where
     * does this chat keep my files", which is the question the owner actually
     * asked on 2026-09-13. The sentence is still there; the workspace is under
     * it. `WorkspaceFiles` owns the three states (not asked / empty / listed)
     * and the rule that a workspace file is NEVER a rail row.
     */
    body = <WorkspaceFiles workspace={workspace ?? null} />;
