...
/**
 * THE RAIL IS REACHABLE WITH NO REPOSITORY, and this exists because it was not.
 *
 * `WorkspaceFiles` shipped with seven passing tests and could not be seen: the
 * region in `App.tsx` read `repoRoot ? <rail/> : null`, and the only branch
 * that renders the workspace is `graph === null` — which, with a repo attached,
 * never happens. Shipped, green, unreachable.
 *
 * MEASURED on the packaged app before the fix: launched with no repository,
 * `[data-testid="rail-workspace"]` was absent from the DOM and so was every
 * `.shell-pane`. This tree has named that failure before — `canvas/proposal`
 * dispatched by nothing, `SeqChartView` with no caller — and a component test
 * proves the component, never the mount.
 */
describe('the rail with no repository still has something to say', () => {
  it('renders the WORKSPACE, not an empty column', () => {
    mount({
      graph: null,
      workspace: {
        root: 'C:/Users/max/.sequence/workspace',
        exists: true,
        entries: [{ path: 'notes.md', kind: 'file', bytes: 12 }],
        omitted: 0,
      },
    });
    expect(screen.getByTestId('rail-workspace')).toBeTruthy();
    expect(screen.getByTestId('rail-workspace-root').textContent).toBe(
      'C:/Users/max/.sequence/workspace',
    );
  });

  it('with no listing yet it says the old sentence and claims nothing', () => {
    mount({ graph: null });
