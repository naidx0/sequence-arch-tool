...
export function DocProvider({ children }: { children: ReactNode }) {
  ...
  const browseRepoPath = state.session.browseRepoPath;
  ...
  useEffect(() => {
    ...
    const url = browseRepoPath
      ? `/api/sessions/${encodeURIComponent(activeId)}?repoPath=${encodeURIComponent(browseRepoPath)}`
    ...
    void fetch(url, {
      ...
      .then((body) => {
        ...
        dispatch({ type: 'doc/base', doc, forSessionId: activeId });
      })
      .catch(() => {
        /* Unreachable server — scratch / scan paths below still apply. */
      });
    return () => controller.abort();
  }, [activeId, browseRepoPath]);

  useEffect(() => {
    function onBoardSeqd(ev: Event) {
      const detail = (ev as CustomEvent<{ sessionId?: string; boardSeqd?: string }>).detail;
      if (!detail?.boardSeqd || detail.sessionId !== activeId) return;
      const doc = fromBoardSeqd(detail.boardSeqd);
      if (!doc) return;
      restoredFromServer.current = true;
      dispatch({ type: 'doc/base', doc, forSessionId: activeId });
    }
    window.addEventListener('sequence:board-seqd', onBoardSeqd);
    return () => window.removeEventListener('sequence:board-seqd', onBoardSeqd);
  }, [activeId]);
  ...
  useEffect(() => {
    ...
    dispatch({ type: 'doc/base', doc: fromScan, forSessionId: activeId });
  }, [fromScan, session.baseGraphId, activeId]);
