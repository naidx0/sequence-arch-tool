...
export interface DocSession {
  ...
  /**
   * Chat session this document is bound to. Persist must refuse when this
   * differs from `session.activeId` — otherwise New Chat can PUT the previous
   * thread's board into the new id (child effects run before DocProvider reset).
   */
...
export type DocSessionAction =
  | { type: 'doc/base'; doc: SeqDiagramV1; forSessionId?: string | null }
  | { type: 'doc/rebase' }
  | { type: 'doc/reset' }
  | { type: 'doc/undo' }
  | DocEdit;

function adopt(doc: SeqDiagramV1, forSessionId: string | null = null): DocSession {
  return {
    doc,
...
export function docSessionReduce(session: DocSession, action: DocSessionAction): DocSession {
  switch (action.type) {
    case 'doc/base': {
      ...
      const bindId =
        action.forSessionId !== undefined ? action.forSessionId : session.forSessionId;
      const sameRepo = session.baseGraphId === incoming.grounded.graphId;
      if (session.doc === null || !sameRepo || session.edits === 0) {
        return adopt(incoming, bindId);
      }
    ...
    case 'doc/rebase': {
      ...
      return adopt(session.pendingBase, session.forSessionId);
    }
