...
describe('IT ADVERTISES NO KEY THE SHELL DOES NOT BIND', () => {
  /*
   * The register's own measurement: V3 binds exactly TWO chords (Cmd/Ctrl+K and
   * Escape) and no `SHELL_COMMANDS` entry has one of its own, so a keycap column
   * would render empty on every row — "a surface with nothing to show, which is
   * the defect this register exists to remove".
   */
  const shell = readFileSync(
    resolve(process.cwd(), 'src', 'v3', 'V3CommandPalette.tsx'),
    'utf8',
  ).replace(/\r\n/g, '\n');
...
describe('the desktop door', () => {
   ...
   *
   * This used to assert a `role: 'help'` menu, on the reasoning that a
   * downloaded app has a Help menu and a person looks at the menu bar before
   * they guess a chord. The owner overruled the premise on 2026-09-12: the
   * File / Edit / View / Window / Help functions belong on the settings page
   * inside the app, not on the Windows menu bar. So the menu is gone and help
   * now has three doors onto ONE page — F1, the Settings > Application row,
   * and the palette — all of which send on the same channel.
   *
   * What this describe block is FOR is unchanged, and is the half that keeps
   * being missing: the channel exists, the bridge relays it receive-only, and
   * the app subscribes.
   */
  ...
  it('the shell still sends on the help channel, from F1 and from the pane', () => {
    expect(main).toMatch(/sequence:open-help/);
    /* The two doors that replaced the menu item, named where they are bound:
       the F1 chord's own row in the shortcut table, and the action the in-app
       Settings row invokes. A channel with nothing left sending on it would
       pass a bare `sequence:open-help` grep and help nobody. */
    expect(main).toMatch(/accelerator: 'F1'/);
    expect(main).toMatch(/case 'open-help':/);
  });
  ...
  it('AND THE APP SUBSCRIBES — the half that is usually missing', () => {
    ...
    expect(screen.getByTestId('v3-overlay-help').contains(screen.getByTestId(HELP.root))).toBe(true);
  });
});
