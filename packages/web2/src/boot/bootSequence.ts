...
export type WireResult<T> =
  ...
  /**
   * A refusal. `message` is the server's OWN sentence when it wrote one — the
   * `error` field of a JSON body — and absent when it did not.
   *
   * Optional and not required, because most callers of this union never read a
   * body on a non-2xx and must keep compiling. It exists because a status code
   * is not a reason: a 409 drew an EMPTY assistant turn on 2026-09-20 while
   * "no repo attached — open a repository from the home screen first" sat
   * unread in the response.
   */
  | { outcome: 'error'; status: number; body: unknown; message?: string }
