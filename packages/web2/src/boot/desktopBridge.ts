...
export interface SequenceDesktopBridge {
   ...
   *
   * OPTIONAL for the same reason the two subscriptions are: a shell built
   * before 2026-09-12 exposes no `shell`, and the browser build never will.
   * Settings' Application pane checks for it and draws one honest line instead
   * of a list of controls that would do nothing — `isDesktop()` alone is not
   * enough to promise this one, because `isDesktop()` only ever promised
   * `openRepo`.
   */
  shell?(action: string): Promise<unknown>;
}
