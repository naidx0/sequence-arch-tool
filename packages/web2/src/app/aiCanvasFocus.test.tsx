...
/*
 * `.ai-canvas-export-quiet` WAS HERE AND THE CONTROL IS NOT.
 *
 * `AiCanvas.tsx` records the removal on its own line: "Export HTML removed
 * — owner walk 2026-09-16: freeform in-project only." The button went, its
 * stylesheet rule went, and this list kept asking for both — so the file
 * stayed red over a control the owner had decided against.
 *
 * A test that names its own subjects has to be edited when the product is.
 * The two below are the controls the canvas actually owns today.
 */
const CONTROLS = ['.ai-canvas-ask-block', '.ai-canvas-story-ctrl'];
...
describe('canvas controls show keyboard focus', () => {
  ...
  it('READS THE RENDERED DOM: those controls are on screen and focusable', () => {
    ...
    for (const sel of ['.ai-canvas-ask-block']) {
