...
export const CHROME_TAB_DEFS: Record<ChromeTabId, ChromeTabDef> = {
  ...
  /* `file`, not `folder`: the surface is a file you are reading and editing —
     the tree is how you reach it. Both glyphs exist in `chat/Icon.tsx`, and a
     name the book does not define is refused by `test/icon-book.test.ts`. */
...
/**
 * Fixed pill order in the main workspace bar — and, because
 * `visibleWorkspacePanes` filters THIS list, the left-to-right order of the
 * workspace columns.
 *
 * FILES IS IN THE BAR AND NOT IN THE `+` MENU ALONE, AND THAT IS FORCED, NOT
 * PREFERRED: a tab id absent from this list is filtered out of
 * `visibleWorkspacePanes`, so it can never be a column. "Stay in the + menu"
 * is therefore the same choice as "stay an overlay", which is the defect being
 * repaired — the IDE panel was mounted absolutely over `.shell-cb-body` and
 * covered the chat it is meant to be read beside. The `+` entry survives as a
 * SECOND door (it is the only one that can say "attach a repository first"),
 * but it now opens this pill.
 *
 * SECOND POSITION, directly right of chat, because the pane order is the
 * reading order: chat drives, the file you are reading sits next to it, and
 * the boards/terminal/browser are what you turn to afterwards. Appending it
 * after `browser` would have put the surface you read while chatting at the
 * far edge of the frame, and it is the first pane demoted when the window
 * narrows (`fitVisiblePanesToWidth` walks newest-first, but the reader's eye
 * walks left-to-right).
 */
