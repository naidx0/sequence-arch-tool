...
describe('the shell mounts the surfaces the lanes built', () => {
  ...
  /*
   * REVERSED — "Files" means the IDE files panel now, two ways.
   *
   * This test used to assert that the + menu's Files entry opened ARCHITECTURE
   * with the index rail expanded, because `applyHostOpen('files')` had been an
   * alias for the rail since before `src/files/` existed. The owner ruled the
   * other way: Files is the panel in `packages/web2/src/files/`, and the index
   * rail keeps its own door — the Index toggle on the Architecture board, and
   * the `rail` / `rail.focus` host commands, which still call
   * `applyHostOpen('files')` and are still locked by the board tests below.
   *
   * THE PROPERTY THIS TEST EXISTS FOR IS UNCHANGED and is why it was not
   * deleted: the entry is attach-gated, and pressing it opens a real surface
   * rather than lighting up and doing nothing. Only the name of the surface
   * moved. The alias firing BOTH — which is what the previous round shipped —
   * is the thing now asserted against: Architecture must stay shut.
   */
  it('workspace + Files opens the IDE files panel as a workspace pane when attached', () => {
    render(<App appStore={storeWithGraph(GRAPH)} />);
    ...
    /* A PANE, not an overlay: it has a split cell of its own and sits beside
       chat rather than over it. */
    expect(screen.getByTestId('workspace-tab-files').getAttribute('aria-selected')).toBe('true');
    expect(screen.getByTestId('shell-workspace-pane-files')).toBeTruthy();
    expect(screen.getByTestId('connected-files')).toBeTruthy();
    expect(screen.getByTestId('shell-workspace-chat')).toBeTruthy();

    /* And the alias is gone — the rail is no longer dragged along behind it. */
    expect(screen.getByTestId('workspace-tab-architecture').getAttribute('aria-selected')).toBe(
      'false',
    );
    expect(screen.queryByTestId('board-region')).toBeNull();
  ...
  /*
   * REVERSED alongside the test above — same ruling, second half of it.
   *
   * The opening two assertions used to read `board-region` and its expanded
   * rail, because "Files" meant the Architecture index rail. They now read the
   * IDE files pane. THE PROPERTY IS THE SAME ONE and is why the test survives:
   * a later board open must not DISPLACE what Files opened. That was the real
   * subject all along — the panes are separate columns, not one exclusive
   * board slot — and it is a stronger claim now than it was, because Files is
   * a column of its own rather than a rail inside the board it shares a slot
   * with.
   */
  it('host Files open keeps the files pane; a later board open does not displace it', async () => {
    render(<App appStore={storeWithGraph(GRAPH)} />);
    fireEvent.click(screen.getByTestId('workspace-plus'));
    fireEvent.click(
    ...
    /* Separate panes, not one exclusive board slot — Files survives the open. */
    expect(screen.getAllByTestId('board-region').length).toBeGreaterThanOrEqual(1);
    expect(screen.getByTestId('whiteboard')).toBeTruthy();
    expect(screen.getByTestId('shell-workspace-pane-files')).toBeTruthy();
    expect(screen.getByTestId('workspace-tab-files').getAttribute('data-on')).toBe('true');
  });
