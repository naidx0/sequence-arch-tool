...
describe('chrome tab workspace model', () => {
  ...
  it('workspacePaneCapacity seats exactly what the SEAT floor and gutters allow', () => {
    ...
    /* Never zero, and never more pills than the bar has. The ceiling is
       WORKSPACE_PILLS.length and is read from it rather than retyped, so
       promoting a surface to a pill (Files, 2026-09-14) moves the number
       without turning this into a test that has to be edited to stay green. */
    expect(workspacePaneCapacity(10)).toBe(1);
    expect(workspacePaneCapacity(6000)).toBe(WORKSPACE_PILLS.length);
    /* Unmeasured is not the same as no room. */
    expect(workspacePaneCapacity(0)).toBe(WORKSPACE_PILLS.length);
    expect(workspacePaneCapacity(Number.NaN)).toBe(WORKSPACE_PILLS.length);
  });
