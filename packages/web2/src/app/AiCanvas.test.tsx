...
describe('AiCanvas', () => {
  it('empty state points at ask — no SeqDraw, no giant chrome mark', () => {
    render(<AiCanvas doc={{ blocks: [] }} hadTurn={false} />);
    const empty = screen.getByTestId('ai-canvas-empty-before');
    expect(empty.textContent).toMatch(/Ask/i);
    expect(empty.textContent).not.toMatch(/SeqDraw/i);
    /* The old empty state wore a spark glyph that CSS blew up into a full-pane
       gold V — that was the owner walk complaint. Empty must stay copy, not a mark. */
    expect(document.querySelector('[data-icon="spark"]')).toBeNull();
  });
  ...
  it('shows story nav when wired — no export HTML control', () => {
    ...
    expect(screen.queryByTestId('ai-canvas-export')).toBeNull();
  });

  it('never shows export HTML on the canvas toolbar', () => {
    /* Owner walk 2026-09-16: freeform in-project only — no export chrome. */
    render(<AiCanvas doc={sampleDoc} onExport={() => {}} />);
    expect(screen.queryByTestId('ai-canvas-export')).toBeNull();
    expect(screen.queryByLabelText('Export HTML')).toBeNull();
  });
...
describe('ConnectedAiCanvas is a SeqDraw freeform plane', () => {
  it('mounts SeqDraw host with chart artifacts on the plane — no export chrome', () => {
    ...
    const parabola = {
      ...
      items: Array.from({ length: 11 }, (_, i) => {
        const x = -5 + i;
        return { id: `p${i}`, label: String(x), value: x * x, group: 'x^2' };
      }),
      axes: { x: 'x', y: 'x^2' },
    };
    /* A canvas doc belongs to a thread, so the store needs one before a
       hydration can land (`state/canvasIdentity.test.tsx`). */
    store.dispatch({
      type: 'session/index',
      sessions: [{ id: 'session-plane', title: 'Plane', createdAt: 'x', updatedAt: 'x' }],
      activeId: 'session-plane',
    });
    store.dispatch({
      type: 'session/canvas-hydrated',
      doc: { blocks: [], charts: [parabola] },
      forSession: 'session-plane',
    });
    render(
    ...
    expect(screen.getByTestId('ai-canvas-seqdraw')).toBeTruthy();
    expect(screen.getByTestId('wb-canvas')).toBeTruthy();
    /* Pane head already says AI Canvas — no second in-body title. */
    expect(document.querySelector('.ai-canvas-plane-title')).toBeNull();
    expect(screen.queryByTestId('ai-canvas-export')).toBeNull();
    expect(screen.getByTestId('ai-canvas-chart')).toBeTruthy();
    expect(screen.getByTestId('seqchart-svg').querySelector('polyline.cseries-line')).toBeTruthy();
    /* Freeform plane — artifact frame present; plane CSS clears the card fill. */
    expect(document.querySelector('.ai-canvas-plane .wb-artifact-frame')).toBeTruthy();
    expect(document.querySelector('.ai-canvas-plane')).toBeTruthy();
  });
});
