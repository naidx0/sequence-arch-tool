...
describe('a view closes from its own ×, like a browser tab', () => {
  /*
   * Owner, 2026-09-13: "have an X to close them after they open up like Chrome
   * tabs so it's easy to navigate". The pill's label already carried three
   * meanings on one press (show / focus / hide); the × carries one. And the
   * last open view refuses OUT LOUD rather than vanishing the workspace.
   */
  it('the × appears on an open view, hides it, and refuses on the last one', () => {
    reportedWidth = 1704;
    render(<App />);

    /* Chat is open at boot, so it is the one pill with a ×. */
    expect(screen.getByTestId('workspace-tab-close-chat')).toBeTruthy();
    expect(screen.queryByTestId('workspace-tab-close-architecture')).toBeNull();

    fireEvent.click(screen.getByTestId('workspace-tab-architecture'));
    expect(screen.getByTestId('workspace-tab-architecture').getAttribute('data-on')).toBe('true');
    const close = screen.getByTestId('workspace-tab-close-architecture');
    ...
    expect(screen.queryByTestId('workspace-tab-close-architecture')).toBeNull();
    expect(screen.queryByTestId('shell-workspace-pane-architecture')).toBeNull();

    /* The last view stays, and says why. */
    fireEvent.click(screen.getByTestId('workspace-tab-close-chat'));
    expect(screen.getByTestId('workspace-tab-chat').getAttribute('data-on')).toBe('true');
    const notice = screen.getByTestId('workspace-view-refused');
    expect(notice.getAttribute('role')).toBe('status');
    expect(notice.textContent).toMatch(/only view open/i);
  });
});
