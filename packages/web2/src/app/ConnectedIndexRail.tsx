...
import type { Coverage, FlowPlayback, FunctionIndex, NodeId, WorkspaceView } from '../state/types';
...
export function ConnectedIndexRail({ client = DEFAULT_CLIENT }: ConnectedIndexRailProps = {}) {
  ...
  /*
   * THE WORKSPACE LISTING — asked for only when there is NO repository.
   *
   * With one attached, the rail's whole job is the index and the workspace
   * would be a second answer to "what is in this column". Without one, it is
   * the only answer there is, and until it existed the rail said "open a
   * repository" to somebody who had deliberately not opened one.
   *
   * ASKED AGAIN WHEN A TURN SETTLES, because a turn is the thing that writes
   * files here. Keyed on the committed turn count rather than on a timer: a
   * poll would re-read the disk forever for a folder that changes a few times
   * an hour, and a listing that only refreshes on mount would show a file the
   * chat had just written as absent.
   */
  const settledTurns = state.session.turns.length;
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const body = (await res.json()) as WorkspaceView;
        if (!live || typeof body?.root !== 'string' || !Array.isArray(body.entries)) return;
        /* The APP store, not `useCanvas`'s canvas-only dispatch: the workspace
           is rail state, and the rail lives on the app store. */
        store.dispatch({ type: 'rail/workspace', workspace: body });
      } catch {
        /* Offline, or the route is not there. The rail keeps saying "open a
           repository" and claims nothing about a folder it could not read —
           a failed listing must never render as an empty one. */
      }
    })();
    return () => {
      live = false;
  ...
  return (
    <IndexRail
      ...
      functions={functions}
      coverage={coverage}
      /* Only ever read when `graph === null`; see WorkspaceFiles. */
      workspace={state.rail.workspace}
      playback={playback}
      status={status}
      repoName={
