...
describe('the shell has ONE copy of its state, and the store holds it', () => {
  ...
  /*
   * OWNER, 2026-09-13: "Opening a repo should not take over the screen."
   *
   * It did: `position: fixed; inset: 0` over rgba(4,4,6,.64) in dark, so the
   * board and chat went dark and unusable behind a 560px panel. The rule now
   * is that a scrim dims for a DECISION and not for NAVIGATION — and the half
   * that needs a lock is the half saying the MECHANICS did not go with the
   * dim. A non-modal opener that also stopped closing on click-outside would
   * be a worse control than the one being fixed.
   */
  it('the attach overlay names its kind, so the ground behind it is not dimmed', () => {
    setFrame(1280);
    const store = createStore();
    connected(store, { renderOverlay: (o) => <div data-testid="overlay-body">{o.kind}</div> });
    act(() => store.dispatch({ type: 'shell/overlay', overlay: { kind: 'attach' } }));
    expect(screen.getByTestId('shell-overlay').getAttribute('data-kind')).toBe('attach');
  });

  it('every OTHER overlay keeps its dim until somebody argues otherwise', () => {
    setFrame(1280);
    const store = createStore();
    connected(store, { renderOverlay: (o) => <div data-testid="overlay-body">{o.kind}</div> });
    act(() =>
    ...
    expect(screen.getByTestId('shell-overlay').getAttribute('data-kind')).toBe('settings');
  });

  it('attach STILL closes on click-outside — only the dim went', () => {
    setFrame(1280);
    const store = createStore();
    connected(store, { renderOverlay: (o) => <div data-testid="overlay-body">{o.kind}</div> });
    act(() => store.dispatch({ type: 'shell/overlay', overlay: { kind: 'attach' } }));
    act(() => {
      fireEvent.mouseDown(screen.getByTestId('shell-overlay'));
    });
    expect(store.getState().shell.overlay).toBeNull();
  });
});
...
describe('the panels are reachable without knowing a chord', () => {
  ...
  it('slims to repo + chord + More, with everything one visible press away', () => {
     ...
     * sidebar, so the bar carries no panel buttons of its own — Runs and
     * Rewind live behind `More`.
     *
     * SETTINGS IS NOT BEHIND MORE EITHER. Decision 12 (owner, 2026-09-13):
     * "settings is both bottom left and top right" — the gear in the sidebar
     * is the one door, and this asserts the second one stays gone in the
     * expanded group as well as on the collapsed bar.
     */
    mountBar();
    ...
    for (const id of ['activity', 'rewind']) {
  ...
  it('each opens the SAME overlay its palette command opens', () => {
    ...
    for (const id of ['activity', 'rewind'] as const) {
      fireEvent.click(screen.getByTestId('shell-more'));
    ...
    /* Settings has ONE door, the sidebar gear (Decision 12) — the palette
       still reaches overlay.settings, More does not offer it. */
    expect(onOverlay).not.toHaveBeenCalledWith({ kind: 'settings', pane: 'provider' });
