...
function Plot({ chart, note }: FamilyChartProps) {
  ...
  let rows = series.map((key) => inGroup(chart.items, key));
  let seriesKeys = series;
  /*
   * Models often emit one `group` per point (or unique labels). A polyline of
   * length 1 paints nothing — axes/grid land, curve invisible (owner walk
   * 2026-09-16). For line charts, coalesce singleton groups into one series in
   * document order so the curve still reads.
   */
  if (
    chart.kind === 'line' &&
  ...
  return (
    <ChartFrame chart={chart} width={width} height={height} note={note}>
      ...
      {/* Line plots: light verticals at each sample so the plane reads as a grid
         (owner: X/Y axes + grid for a parabola), not only horizontal ticks. */}
      {chart.kind === 'line'
      ...
      {rows.map((row, s) =>
        chart.kind === 'line' ? (
          <polyline
            ...
            data-series={seriesKeys[s] ?? `s${s}`}
            data-testid="cseries-line"
            fill="none"
            stroke="var(--viz-1, var(--accent, #f6f7fa))"
            strokeWidth={2.5}
            points={row
