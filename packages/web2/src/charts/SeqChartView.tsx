...
/** The part of a drawing the reader pointed at. */
export interface ChartPart {
  /** The item's own id, as `data-item` carries it. */
  itemId: string;
  /** What the item is called on screen — the words the reader actually saw. */
  label: string;
  /** Present only when this item stands for a real node of the scanned graph. */
  nodeId?: string;
}
...
export interface SeqChartViewProps {
  ...
  /**
   * POINT AT A PART OF THE PICTURE AND ASK ABOUT IT.
   *
   * The owner's deferred item, in his words: "the canvas draws natively and at
   * once, tldraw-style, and the person can select a drawn part and ask about
   * it." Everything before this made Sequence able to DRAW well and to READ
   * what it had drawn; this is the half where the reader can answer back.
   *
   * ONE DELEGATED HANDLER, NOT EIGHT RENDERERS. `itemAttrs` (chartLayout.ts)
   * already stamps `data-item` — and `data-node-id` where the item is grounded
   * — on every item of every family, because cross-highlighting against the
   * board needed it. So the click resolves by walking up to the nearest
   * `[data-item]`, and NodeLink, Flow, Hierarchy, Timeline, Quantitative,
   * Comparison, Cards and Loop all become selectable without being touched.
   * Eight copies of a click handler is eight places for it to drift.
   *
   * Absent ⇒ nothing is clickable and no affordance is drawn, so every chart
   * that is only being read renders exactly as it did before.
   */
  onSelectPart?: (part: ChartPart) => void;
}

export function SeqChartView({ chart, knownNodeIds, onSelectPart }: SeqChartViewProps) {
  ...
  const drawn = verdict.chart;
  if (onSelectPart === undefined) {
    return (
      <ChartBoundary chart={drawn}>
        <Renderer chart={drawn} note={FALLBACK_NOTE[family]} />
      </ChartBoundary>
    );
  }

  /*
   * THE ID MUST BE ONE THE CHART ACTUALLY HAS. `data-item` comes out of the
   * DOM, which is the one input here a bug could make arbitrary, and handing a
   * caller an item the spec does not contain would be the same fabrication the
   * validator upstream refuses. So it is resolved against `drawn.items` and a
   * miss is dropped.
   */
  const pick = (target: EventTarget | null): void => {
    const el = target instanceof Element ? target.closest('[data-item]') : null;
    const id = el?.getAttribute('data-item');
    if (!id) return;
    const item = (drawn.items ?? []).find((i) => i.id === id);
    if (!item) return;
    onSelectPart({
      itemId: item.id,
      label: item.label,
  ...
  return (
    <ChartBoundary chart={drawn}>
      {/* The listener sits on a wrapper rather than on the SVG so the same
          handler catches a click on a card renderer's HTML and on an SVG
          family's <g> — the eight families are not all SVG. */}
      <div
        className="seqchart-pick"
        data-testid="seqchart-pick"
        onClick={(e) => pick(e.target)}
        onKeyDown={(e) => {
          /* Enter and Space on a focused item, so the picture is not
             mouse-only. `itemAttrs` makes the items focusable when this is on. */
          if (e.key === 'Enter' || e.key === ' ') {
            const el = document.activeElement;
            if (el && el.closest('[data-item]')) {
              e.preventDefault();
              pick(el);
            }
          }
