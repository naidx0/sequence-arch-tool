...
export interface ItemAttrs {
  ...
  'data-focused': string;
  'data-node-id'?: string;
  /** Keyboard reachability for `onSelectPart` — see the note where it is set. */
  tabIndex?: number;
}
...
export function itemAttrs(item: ChartItem, focusItemId?: string): ItemAttrs {
  ...
  /*
   * KEYBOARD-REACHABLE, so "select a drawn part" is not a mouse-only feature.
   *
   * Unconditional, and that is the cheaper of two wrongs. Making it depend on
   * whether a selection handler is mounted would thread a flag through eight
   * renderers to save a tabstop; leaving it always on costs a tab stop per item
   * on a chart nobody can click, which `.seqchart-pick`'s CSS makes visibly
   * inert. If a chart ever grows past a few dozen items that trade flips, and
