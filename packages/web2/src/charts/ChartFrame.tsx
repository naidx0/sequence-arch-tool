...
export interface ChartFrameProps {
  ...
  note?: string;
  /**
   * `plane` — AI Canvas freeform: no scroll cage; board pan/zoom owns navigation
   * (owner walk 2026-09-16). Default keeps chat-column scroll ownership.
   */
...
export function chartLabel(chart: SeqChart): string {
  ...
  if (chart.caption && !captionEchoesTitle(chart.title, chart.caption)) {
    parts.push(chart.caption);
  }
...
/**
 * Title already names the shape ("a parabola: x^2") — repeating "a parabola, …"
 * under it reads as run-on chrome (owner walk 2026-09-16).
 */
export function captionEchoesTitle(title: string, caption: string): boolean {
  const t = title.trim().toLowerCase();
  const c = caption.trim().toLowerCase();
  if (!t || !c) return false;
  if (t === c) return true;
  const stem = t.split(/[:—–-]/)[0]!.trim();
  return stem.length >= 3 && (c === stem || c.startsWith(`${stem},`) || c.startsWith(`${stem} `));
}

export function ChartFrame({ chart, width, height, note, variant = 'default', children }: ChartFrameProps) {
  ...
  const showCaption =
    Boolean(chart.caption) && !captionEchoesTitle(chart.title, chart.caption ?? '');
  return (
    <figure
      className={`seqchart${variant === 'plane' ? ' seqchart--plane' : ''}`}
