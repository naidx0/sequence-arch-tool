...
import { MAX_CANVAS_FRAMES } from '../app/materializeCanvasArtifacts';
import type {
  CanvasArtifactFrame,
  CanvasDoc,
  CanvasDocBlock,
...
export interface CanvasMemoryShape {
  ...
  charts?: SeqChart[];
  /**
   * WHERE EACH ARTIFACT SITS, keyed by the pad's artifact id (wave B2). Until
   * this existed the arrangement lived only in `sequence.seqdraw.<id>` in this
   * origin's localStorage, so a canvas opened anywhere else re-cascaded every
   * frame down the left edge. See `CanvasDoc.frames` for why it is a map.
   */
  frames?: Record<string, CanvasArtifactFrame>;
}

/**
 * One frame, checked rather than trusted. The server stores this file
 * verbatim, and a reader of a shared `.sequence` directory is not the person
 * who wrote it: a non-finite coordinate would put an artifact where no camera
 * can reach, and a zero size would draw a frame nobody can grab.
 */
function parseFrame(raw: unknown): CanvasArtifactFrame | null {
...
function parseFrames(raw: unknown): Record<string, CanvasArtifactFrame> | undefined {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return undefined;
  const out: Record<string, CanvasArtifactFrame> = {};
  let n = 0;
...
export function toCanvasMemory(sessionId: string, doc: CanvasDoc): CanvasMemoryShape {
  return {
    ...
    ...(doc.frames && Object.keys(doc.frames).length > 0 ? { frames: doc.frames } : {}),
  };
}
...
export function fromCanvasMemory(raw: unknown): CanvasDoc {
  ...
  const charts = Array.isArray(o.charts)
    ...
    : [];
  const frames = parseFrames(o.frames);
  return {
    blocks,
