...
describe('a saved chart-only canvas is restored, not discarded', () => {
  ...
  it('hydration does not overwrite a canvas that already holds a chart', () => {
    ...
    store.dispatch({
      ...
      doc: { blocks: [] },
      /* Named for the thread that IS current, so the arm under test is the
         "already has content" refusal and not the identity guard. */
      forSession: store.getState().session.activeId,
    } as never);
    expect(
      store.getState().session.canvasDoc.charts,
