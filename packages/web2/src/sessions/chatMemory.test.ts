...
describe('what is written', () => {
  ...
  it('keeps tools-only assistants (empty prose) so generations survive reload', () => {
    const toolsOnly = assistant('', 2_000, true);
    expect(toolsOnly.text).toBe('');
    const memory = toChatMemory('s1', [user('draw a chart'), toolsOnly]);
    expect(memory.turns).toHaveLength(2);
    expect(memory.turns[1]!.role).toBe('assistant');
    expect(memory.turns[1]!.text).toBe('');
    const restored = fromChatMemory(memory);
    expect(restored).toHaveLength(2);
    expect(restored[1]!.role).toBe('assistant');
    expect(restored[1]!.role === 'assistant' && restored[1]!.work.length).toBeGreaterThan(0);
  });

  it('still drops empty user turns', () => {
    const memory = toChatMemory('s1', [user('   '), user('ok')]);
    expect(memory.turns.map((t) => t.text)).toEqual(['ok']);
  });
