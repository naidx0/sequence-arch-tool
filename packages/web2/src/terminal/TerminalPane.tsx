...
export interface TerminalPaneProps {
  /** When null/undefined, show a calm needs-project state instead of a raw disconnect. */
  repoRoot?: string | null;
}

export function TerminalPane({ repoRoot = null }: TerminalPaneProps) {
  ...
  const [detail, setDetail] = useState('Open a project to start a terminal');
  ...
  useEffect(() => {
    if (!repoRoot) {
      ...
      setBackend(null);
      setStatus('idle');
      setDetail('Open a project to start a terminal');
      return undefined;
    }
      ...
      try {
        client = await connectTerminal({
          ...
          onClose({ code, reason }) {
            if (cancelled) return;
            setStatus('closed');
            setDetail(reason || `Disconnected (${code}) — click Terminal again after reconnect`);
          },
          onError(message) {
            if (cancelled) return;
            setStatus('error');
  ...
  return (
    <div className="terminal-pane" data-testid="terminal-pane" ref={wrapRef}>
      <div className="terminal-pane-chrome" data-testid="terminal-pane-chrome">
        {/* Pane head already names Terminal — status only here. */}
