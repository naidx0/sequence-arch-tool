...
describe('which pane the panel shows', () => {
  ...
  it('names only panes the panel actually draws', () => {
    /* The old union was 'provider' | 'permissions' | 'appearance'. There is no
       ...
       'app' joined them on 2026-09-12 with the surface it names: the owner had
       the native File / Edit / View / Window / Help menu removed and everything
       it did moved into the panel, drawn by settings/AppPane.tsx. */
  ...
  it('resolves the pane the menu bar moved into', () => {
    /* The shell bar's gear dispatches `{ kind: 'settings', pane: 'app' }`. A
       pane id the resolver does not know falls back to 'provider' silently,
       so a door onto the replacement menu would quietly open the wrong page. */
    expect(resolvePane('app')).toBe('app');
  });
