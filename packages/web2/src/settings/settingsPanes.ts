...
export type SettingsPane = 'provider' | 'notifications' | 'hooks' | 'workspace' | 'app';
...
export const SETTINGS_PANES: readonly PaneTab[] = [
  ...
  /*
   * APPLICATION IS LAST, AND IT IS THE MENU BAR.
   *
   * Owner, 2026-09-12: the File / Edit / View / Window / Help functions belong
   * on the settings page inside the app rather than on the Windows menu bar. So
   * the desktop shell builds no menu any more and this pane is where reload,
   * zoom, full screen, developer tools, the help page, the version block and
   * Quit now live. Last because it is the pane you visit when something about
   * the WINDOW is wrong, not when something about the product is.
   */
  { id: 'app', label: 'Application', title: 'The window itself — reload, zoom, help, version, quit' },
];
