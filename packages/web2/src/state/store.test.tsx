...
describe('lock B — send is derived and cannot disagree', () => {
  ...
  const script: Action[] = [
    ...
    { type: 'composer/permission', mode: 'plan' },
...
describe('the store fabricates nothing', () => {
  ...
  it('WILL NOT SWITCH TO A MODE THE USER HAS NOT ENABLED — Build is opt-in', () => {
    /*
     * Build is the mode that writes without an Accept and runs commands, so it
     * is off until Settings → Workspace turns it on. Accepting the switch here
     * would make the control claim an authority the user never granted — and
     * the engine would then honour it, because the belt reads the mode this
     * store publishes.
     *
     * Was `full` before 2026-09-13; the four-rung ladder collapsed to two and
     * the rule is unchanged — only the name of the rung being refused.
     */
    const store = createStore();
