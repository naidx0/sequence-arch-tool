 ...
 */
/**
 * ONE STEP OF THE TURN'S OWN WORK LIST, as `update_todos` last set it.
 *
 * Mirrors `@sequence/analyzer`'s `TodoItem` and is deliberately its own type
 * here rather than an import: the store's types are the WIRE's shape, and this
 * arrives on the wire. `todoList.ts` owns every rule about what a valid list is
 * — the server refuses an invalid one, so the client never has to decide.
 *
 * NOT A `WorkRow`. A work row is something that HAPPENED, past tense, one per
 * event, appended and never revised. A todo is something that is INTENDED, and
 * the whole list is replaced on every update. Folding them together would mean
 * either revising history or appending the same three steps eight times.
 */
export interface TodoItem {
  id: string;
  title: string;
  status: 'pending' | 'active' | 'done' | 'blocked';
  /** Why it is blocked. Present only where it says something. */
  note?: string;
}
...
export interface WorkRow {
  ...
  opens: 'canvas' | 'ai-canvas' | 'rail' | 'review' | 'files' | 'browser' | 'terminal' | null;
  /**
...
export interface AssistantTurn {
  ...
  text: string;
  work: WorkRow[];
  /** The work list as it stood when the turn ended. Empty when none was set. */
  todos: TodoItem[];
  effect: TurnEffect;
...
export interface InFlightTurn {
  ...
  work: WorkRow[];
  /**
   * The work list, replaced wholesale on each `todo:list` frame. Empty on every
   * turn that never declares one, which is most of them.
   */
  todos: TodoItem[];
  evidence: TurnEvidence;
  usage: AskUsage | null;
...
/**
 * `chart-part` is the one kind whose `ref` is NOT a repository id.
 *
 * It names an item of a chart Sequence drew — the thing the reader clicked on
 * the picture. A teaching chart has no repository behind it at all, so a
 * `chart-part` chip may not be rendered into the prompt as grounded scope the
 * way a node or a file is; `connect.tsx` gives it its own self-describing line.
 * Kept in this union rather than in a parallel one because it IS a chip: it
 * sits in the composer, it is removable, and it is context for the next
 * question. What differs is only what it points at.
 */
export type ContextChipKind = 'node' | 'file' | 'function' | 'canvas-block' | 'chart-part';
...
export interface MentionResult {
  /**
   * NOT every `ContextChipKind`. `chart-part` is excluded because a mention is
   * something you TYPE — `@` then a name — and a chart part has no name to
   * type: it is the thing you clicked on a picture. Widening this to the full
   * union forced `rankMentions` to invent a tiebreak rank for a kind that can
   * never appear in its list, which is a table entry nothing can reach and the
   * kind of quiet untruth this contract exists to refuse.
   */
  kind: Exclude<ContextChipKind, 'chart-part'>;
  ref: string;
  label: string;
 ...
 */
/*
 * THREE MODES, AND THE THIRD ONE IS A FLAG.
 *
 * Owner, 2026-09-13: "Teach mode, plan mode, build mode. Build mode is auto
 * mode, full capabilities, full permissions — you say yes and it just goes,
 * reads files, writes, and goes from there. Plan is just planning: mostly read
 * only, writing in the AI Canvas and simple files. Make sure proposed mode is
 * plan mode. It should be pretty simple."
 *
 * The ladder was `plan | propose | autoEdit | full`, four rungs where two pairs
 * did the same job: `plan` and `propose` both stage rather than apply, and
 * `autoEdit` and `full` both apply without an accept. Four names for two
 * behaviours is how a reader ends up unable to say what the mode they are in
 * will do — which is the one question this control exists to answer.
 *
 *   plan   stages. Reads anything, writes to the AI Canvas, and proposes file
 *          changes you accept. NEVER runs a command.
 *   build  applies. Writes files directly and runs commands.
 *
 * Teach rides beside these as its own flag (`docs/teach-mode.md`) rather than
 * as a third value, because it is orthogonal: a lesson reads and explains, and
 * it refuses every mutating tool on its own regardless of which of these two is
 * set. That is why the control shows three rows and this union has two values.
 *
 * THE BOUNDARY BETWEEN THEM IS `run_command`, and it is the one that has
 * already gone wrong here: `askTools.ts` records that `propose` — the old
 * default — once fell through to the full belt while the control promised
 * "changes arrive as proposals you accept". Plan inherits propose's REFUSAL,
 * not its old hole.
 */
...
export interface ComposerSlice {
  ...
  /**
   * THE CHAT DRIVES THE TABS, OR IT DOES NOT — the session lock.
   *
   * Owner's shape, 2026-09-14: "how can we lock that chat session like cursor,
   * where the chat controls the other tabs — while the other chats can be used
   * as well by hands not just chat."
   *
   * A work row has carried `opens` since it was written — the surface its
   * result landed on — and following it has always been the READER's click.
   * Locked, the app follows it as the row arrives; unlocked, nothing changes
   * and the row stays a click target. So this adds no new destination and no
   * second affordance: it decides WHO presses the one that already exists.
   *
   * ON THE COMPOSER, WHICH IS PER-SESSION STATE, and that is the whole of "the
   * other chats can be used by hands". The lock belongs to the conversation
   * that is driving; switch away and the surfaces stop moving on their own,
   * because the thing that was moving them is not the conversation you are in.
   * A global flag would make one chat's lock reach into every other one, which
...
export interface RailSlice {
  ...
  loading: boolean;
  /**
   * THE LOCAL WORKSPACE — what the rail shows when there is no repository.
   *
   ...
   * all of that." Until this existed the rail's repo-less body was one
   * sentence — "Open a repository and the index fills" — which is true and is
   * not an answer to "where does this chat keep things".
   *
   * ON THE RAIL SLICE because it is what the rail RENDERS. It is not a second
   * repo: nothing here is scanned, there is no graph, and a workspace file
   * never becomes a `RailRow` (see `WorkspaceFiles`). The two are shown in the
   * same column and must never be shown in the same list.
   *
   * `null` means "not asked yet", which is different from `exists: false`
   * ("asked, and nothing is there"). The rail says different things for each.
   */
  workspace: WorkspaceView | null;
}

/**
 * The workspace as the client holds it — the wire body, unchanged.
 *
 * AN ALIAS, NOT A COPY. `types.test.ts` refuses a state type that is "a wire
 * type wearing a new name", and it is right to: two declarations of one shape
 * drift, and the one that drifts is always the copy. The name exists because
 * `rail.workspace: GetWorkspaceResponse` reads like a pending fetch rather
 * than like state, and nothing else.
 */
export type WorkspaceView = GetWorkspaceResponse;
...
/**
 * WHERE ONE ARTIFACT SITS ON THE AI CANVAS PLANE — board units, the pad's own
 * coordinates (carrying-harness plan, wave B2).
 */
export interface CanvasArtifactFrame {
  x: number;
  y: number;
  w: number;
  h: number;
...
export interface CanvasDoc {
  ...
  /**
   * THE THREAD THIS DOC BELONGS TO — canvas identity, not decoration.
   *
   * Owner walk 2026-09-17: "whenever I switch from chats, the chart / canvas
   * drawing disappeared", and "when I click on another chat the canvas
   * plotting kind of takes priority". Both are ONE race. `optimisticSwitch`
   * flips `activeId` to B a dispatch before `softLoadSession` replaces
   * `canvasDoc`, so for one render thread A's doc is the doc while B is
   * already the id — and every consumer that keyed on `activeId` ALONE (the
   * settled PUT and the flush snapshot in `state/connect.tsx`, the SeqDraw pad
   * in `app/AiCanvas.tsx`) spent that render writing A's blocks into B.
   *
   * A doc that carries its OWN id turns an invisible mismatch into a readable
   * one: `doc.forSession === activeId` is the entire guard. ABSENT means
   * unstamped — the boot document, and any doc a helper built before this
   * field existed — and unstamped is read as "belongs to nobody", never as
   * "belongs to whoever is asking".
   */
  forSession?: string | null;
  /**
   * WHERE EACH ARTIFACT SITS, keyed by the pad's own artifact id
   * (`artifact:chart:<i>` / `artifact:block:<id>`) — carrying-harness plan,
   * wave B2.
   *
   * ── WHY A MAP ON THE DOC AND NOT A FIELD ON EACH BLOCK ──────────────────
   *
   * The plan's row said "CanvasDocBlock gains frame". A map keyed by the
   * ARTIFACT id is the same fact in a better place, for two reasons the block
   * field cannot answer. A chart is not a block — `charts` is `SeqChart[]`
   * from `@sequence/schema`, and giving charts positions through a block field
   * would mean either widening the shared chart spec (a wire type three
   * packages read) or leaving every chart unplaceable. And the artifact id is
   * already the key the pad, the append-by-id effect and this map all agree
   * on, so nothing has to translate between two namings.
   *
   * POSITIONS WERE ONLY EVER IN THE BROWSER before this: `sequence.seqdraw.<id>`
   * in localStorage. A canvas opened on another machine — or after the origin's
   * storage was cleared — re-cascaded every artifact down the left edge, so an
   * arrangement a person had made by hand was not part of the document they
   * had made it in.
   */
  frames?: Record<string, CanvasArtifactFrame>;
  ...
  /**
   * ONE CONCRETE INPUT, TRACED THROUGH THE PICTURE — the fourth beat of the
   * teaching journey (`teachWalk.ts`). Replaced wholesale like `teachStep`
   * above: a walk belongs to the picture currently drawn, and keeping the
   * previous one would leave a story pointing at parts that are gone.
   *
   * Every `partId` was checked against the chart the server emitted, so the
   * renderer may light a part by id without re-validating it.
   */
  teachWalk?: { input: string; steps: { partId: string; says: string }[] };
}
...
export interface SessionSlice {
  ...
  canvasDoc: CanvasDoc;
  /**
   * Agent marks for SeqDraw / whiteboard — appended from `board:item` SSE.
   * Presentational canvases merge by id; never a wholesale document replace.
   */
  boardAgentItems: import('../whiteboard/whiteboardModel').WbItem[];
  /**
   * THE READER'S OWN MARKS ON THE AI CANVAS — the live SeqDraw document the
   * `Whiteboard` under the AI Canvas reports on every change. Read at send
   * time by `withDrawingState` so the model can see notes, shapes and the
   * named ink beside its own blocks (canvas moat audit, 2026-09-18). Null
   * until the surface reports, and cleared on every session switch.
   */
  canvasDrawing: import('../whiteboard/whiteboardModel').WhiteboardDoc | null;
  /**
   * Ids the human moved — layout must not yank them on the next diagram.upsert
   * (`respectPins`). Cleared on new chat.
   */
  boardPinnedIds: string[];
  /**
   * Intent ghosts: previous agent position after a human pin, drawn faintly
   * so the human move is visible against the agent's last placement.
   */
  boardIntentGhosts: { id: string; at: { x: number; y: number } }[];
  /**
   * Chat lock for the Files IDE panel — set when `edit:proposal` arrives so the
   * panel selects the first file and opens diff mode. Cleared when the user
   * picks another file or closes Files.
   *
   * UNFINISHED (Wave 3 session lock): Stage / Revert endpoints are still
   * refused by the panel when no callback is passed; this only drives selection
   * + diff view from chat proposals.
   */
  filesFocus: {
    path: string;
    view: 'code' | 'diff' | 'edit';
    /** Proposed body for diff when git diff is empty / unattached. */
  ...
  hydrating: boolean;
  /**
   * Soft-browse of a catalog thread under another repo — DocProvider / canvas
   * fetch with `?repoPath=` without attaching. Null when the active thread
   * lives under the attached root (or General).
   */
...
export type ShellOverlay =
   ...
   */
  /* 'app' is the pane the removed native menu bar moved into — owner,
     2026-09-12. Spelled out here rather than imported from settingsPanes so
     the state contract stays free of the surface that renders it. */
  | { kind: 'settings'; pane: 'provider' | 'notifications' | 'hooks' | 'workspace' | 'app' }
  | { kind: 'sessions' }
  /*
