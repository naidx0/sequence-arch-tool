...
describe('the send payload', () => {
  ...
  /*
   * STAGE 3 — THE AGENT CAN SEE THE CANVAS.
   * `docs/research/agent-drawing-and-teaching-visuals.md` measured the hole:
   * five `canvas.write_*` writers and no reader, so "change that diagram" could
   * only ever produce a second diagram. The server half is tested in
   * `canvas-read-half.test.ts`; this is THE MOUNT — the reason this file exists.
   */
  it('CARRIES THE CANVAS on the wire: every block blurry, the newest focused', async () => {
    const { sent, transport } = capturing();
    const store = attachedStore();
    /* A canvas doc belongs to a thread now, and a hydration names the thread
       it was read for — so the store needs an active id before one can land
       (`state/canvasIdentity.test.tsx`). */
    store.dispatch({
      type: 'session/index',
      sessions: [{ id: 'session-canvas', title: 'Canvas', createdAt: 'x', updatedAt: 'x' }],
      activeId: 'session-canvas',
    });
    store.dispatch({
      type: 'session/canvas-hydrated',
      doc: {
        blocks: [
          { id: 'b1', type: 'mermaid', title: 'Flow', payload: 'graph TD;', status: 'landed' },
          { id: 'b2', type: 'svg', title: 'Attention', payload: '<svg/>', status: 'landed' },
        ],
      },
      forSession: 'session-canvas',
    } as never);
    store.dispatch({
      type: 'composer/ask-surface',
      surface: { id: 'ai-canvas', title: 'AI Canvas' },
    });
    send(store, transport, 'redraw that with the mask step');
    await vi.waitFor(() => expect(sent).toHaveLength(1));
    expect(sent[0]!.surface).toEqual({
      id: 'ai-canvas',
      title: 'AI Canvas',
      canvas: {
        blocks: [
          { id: 'b1', type: 'mermaid', title: 'Flow', chars: 9 },
          { id: 'b2', type: 'svg', title: 'Attention', chars: 6, excerpt: '<svg/>' },
        ],
      },
    });
  });

  it('a PENDING block is not on the wire — it has no payload to describe', async () => {
    const { sent, transport } = capturing();
    const store = attachedStore();
    /* A canvas doc belongs to a thread now, and a hydration names the thread
       it was read for — so the store needs an active id before one can land
    ...
    store.dispatch({
      type: 'session/canvas-hydrated',
      doc: {
        blocks: [{ id: 'b1', type: 'svg', payload: '', status: 'pending' }],
      },
      forSession: 'session-canvas',
    } as never);
    store.dispatch({
    ...
    send(store, transport, 'what is on the canvas');
    await vi.waitFor(() => expect(sent).toHaveLength(1));
    expect(sent[0]!.surface).toEqual({ id: 'ai-canvas', title: 'AI Canvas' });
  });
  ...
  it('TEACH MODE IS NOT A DESIGN REQUEST — no proposeArchitecture, even with nothing attached', () => {
    /*
     * REPORTED SHAPE, 2026-09-13. Max, in Teach mode with nothing attached:
     * "what is an LLM, break it down for me please how it works, why it works".
     * What came back opened:
     *
     *     Design assumptions
     *     1. Scale: …  2. Read/write mix: …  3. Latency: …
     *     Natural modules
     *     Agent – Orchestrates tasks …  Prompt Generator …  Monitoring Service
     *
     * That is `ASK_DESIGN_PROPOSE_NOW_CLAUSE` word for word — "State 3–5
     * numbered assumptions in plain English (scale, read/write mix, latency…)".
     * A learner asking what an LLM IS was handed an architecture for an app
     * that USES one, because the client had turned his lesson into a design
     * proposal before the server ever saw it. His words: "still not drawing,
     * still not explaining".
     *
     * The flag belongs to the blank-workspace DESIGNER, who has no outline and
     * wants a diagram rather than a refusal. Two different people were sharing
     * one branch because neither of them has a repository open.
     *
     * The design payload itself must SURVIVE — it is what keeps /api/ask from
     * 409-ing with nothing attached — so this asserts both halves.
     */
    const { sent, transport } = capturing();
    const store = createStore({});
    store.dispatch({ type: 'composer/teach', on: true } as never);

    send(store, transport, 'what is an llm, break it down for me please how it works');

    expect(sent).toHaveLength(1);
    expect(sent[0]!.design).toEqual({
      title: 'Blank workspace',
      outline: 'what is an llm, break it down for me please how it works',
    });
    expect(sent[0]!.design).not.toHaveProperty('proposeArchitecture');
    /* And the turn still says it is a lesson. */
    expect(sent[0]!.teach).toBe(true);
  });
});
