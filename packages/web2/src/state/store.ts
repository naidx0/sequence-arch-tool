...
import { boardWireToWbItem } from '../draw/seqDraw';
import { artifactIdsOf, framesFromDrawing, sameFrames } from '../app/materializeCanvasArtifacts';
...
export type Action =
  ...
  | { type: 'composer/session-lock'; locked: boolean }
  ...
  | { type: 'rail/workspace'; workspace: import('./types.js').WorkspaceView }
  /**
  ...
  /**
   * Soft-open a catalog thread: replace the on-screen transcript without
   * attaching that repo. Unlike `session/hydrated`, this MAY overwrite a live
   * thread — the reader asked to leave it.
   */
  | {
      type: 'session/browse';
      activeId: SessionSlice['activeId'];
      turns: readonly Turn[];
      /** Foreign catalog root, or omit/null for the attached / General root. */
      browseRepoPath?: string | null;
    }
  /**
   * A canvas READ FROM DISK, carrying the id it was read FOR. A soft load is
   * an async round trip and the reader may leave mid-flight; without the id
   * the reducer cannot tell a late answer for the thread just left from this
   * thread's own answer, and the slower read wins by accident.
   */
  | {
      type: 'session/canvas-hydrated';
      doc: import('./types.js').CanvasDoc;
      forSession: string;
    }
  | { type: 'session/canvas-drawing'; doc: import('../whiteboard/whiteboardModel').WhiteboardDoc | null }
  | {
      type: 'session/index';
  ...
  /**
   * Human dragged a SeqDraw mark — pin it and record an intent ghost at the
   * pre-move position so the next diagram.upsert respects the human placement.
   */
  | {
      type: 'board/pin';
      id: string;
      /** Position before the drag (agent intent). */
      ghostAt: { x: number; y: number };
      /** Position after the drag (human pin). */
      at: { x: number; y: number };
    }
  | { type: 'files/focus-clear' };
...
export function reduce(state: AppState, action: Action, deps: ReduceDeps): AppState {
  switch (action.type) {
    ...
    case 'composer/session-lock':
      /*
       * PER-SESSION, so switching conversations stops the surfaces moving — see
       * `sessionLocked` on ComposerSlice. No other state changes: the lock does
       * not open anything by itself, it decides who presses the affordance that
       * already exists on every work row.
       */
      return { ...state, composer: { ...state.composer, sessionLocked: action.locked } };
    ...
    case 'composer/permission-enabled': {
      const enabled = action.enabled.includes('plan')
        ? action.enabled
        : (['plan', ...action.enabled] as PermissionMode[]);
    ...
    case 'session/browse': {
      /* Catalog soft-open / in-app session switch: the reader left the previous
         thread on purpose. Clear canvas so softLoad can hydrate this session's
         doc (or the per-session latch re-fetches when browseRepoPath is null). */
      return {
        ...state,
        session: {
          ...state.session,
          ...
          inFlight: null,
          queued: null,
          /* Empty AND STAMPED: an unowned empty doc would leave the
             persistence effects with nothing they are allowed to write, so the
             first block of this thread would never reach disk. */
          canvasDoc: { blocks: [], forSession: action.activeId },
          boardAgentItems: [],
          canvasDrawing: null,
          boardPinnedIds: [],
          boardIntentGhosts: [],
          filesFocus: null,
          browseRepoPath:
            action.browseRepoPath === undefined ? null : action.browseRepoPath,
        },
        /*
         * THE DRAFT BELONGS TO THE THREAD IT WAS TYPED IN.
         *
         * This case emptied the transcript, the in-flight turn, the queue, the
         * canvas, the board and the files focus — everything the reader can
         * SEE — and left the composer holding the previous thread's text.
         * Owner: "press new chat, still can't type", which is what a field
         * that opens already full of someone else's sentence looks like from
         * the seat. The @-mention probe goes with it: a picker open against a
         * query from a thread nobody is in is a menu with no question.
         */
        composer: { ...state.composer, draft: '', mention: null },
      };
    }

    case 'session/canvas-drawing': {
      if (state.session.canvasDrawing === action.doc) return state;
      /*
       * THE ARRANGEMENT IS PART OF THE DOCUMENT (carrying-harness plan, B2).
       *
       * The pad owns the live geometry — a drag writes `wb/move`, the resize
       * observer writes `wb/resize` — and this is the one place that geometry
       * becomes part of `canvasDoc`, which is what the settled PUT sends to
       * `.sequence/sessions/<id>/canvas.json`. Before this, positions existed
       * only in this origin's localStorage, so the same canvas opened on
       * another machine re-cascaded every artifact down the left edge.
       *
       * ONE WRITER, HERE, rather than a second action from the pad's callback:
       * the drawing and the frames it implies are the same event, and two
       * dispatches for one event is how they drift.
       *
       * Guarded three ways, because this fires on every stroke: only frames of
       * artifacts THIS document owns (`artifactIdsOf` — a content check, and
       * the one that survives a session switch mid-render; see
       * `framesFromDrawing` for why the `forSession` identity check cannot),
       * only when the drawing carries one at all, and only when the frames
       * CHANGED — otherwise drawing a line would rewrite `canvasDoc` and wake
       * the settled PUT for nothing.
       */
      const next = { ...state, session: { ...state.session, canvasDrawing: action.doc } };
      if (!action.doc) return next;
      const owned = artifactIdsOf(state.session.canvasDoc);
      if (owned.size === 0) return next;
      const frames = framesFromDrawing(action.doc.items, owned);
      if (!frames || sameFrames(frames, state.session.canvasDoc.frames)) return next;
      return {
        ...next,
        session: { ...next.session, canvasDoc: { ...state.session.canvasDoc, frames } },
      };
    }

    case 'session/canvas-hydrated': {
      /* A LATE ANSWER FOR A THREAD THE READER ALREADY LEFT IS NOT AN ANSWER.
         Two fast switches leave two reads in flight; the slower one used to
         land whatever it found on whichever thread happened to be showing. */
      if (action.forSession !== state.session.activeId) return state;
      if (state.session.inFlight !== null) return state;
      ...
      return {
        ...
        session: {
          ...state.session,
          canvasDoc: { ...action.doc, forSession: action.forSession },
        },
      };
    }
    ...
    case 'session/index': {
      ...
      const sameList =
        ...
        prev.sessions.every((entry, i) => {
          const next = action.sessions[i];
          return (
            entry.id === next?.id &&
    ...
    case 'board/pin': {
      const pins = state.session.boardPinnedIds.includes(action.id)
        ? state.session.boardPinnedIds
        : [...state.session.boardPinnedIds, action.id];
      const ghosts = [
        ...state.session.boardIntentGhosts.filter((g) => g.id !== action.id),
        { id: action.id, at: action.ghostAt },
      ];
      const dx = action.at.x - action.ghostAt.x;
      const dy = action.at.y - action.ghostAt.y;
      const boardAgentItems = state.session.boardAgentItems.map((it) => {
        if (it.id !== action.id) return it;
        if (it.kind === 'text' || it.kind === 'noderef' || it.kind === 'artifact') {
          return { ...it, at: { x: action.at.x, y: action.at.y } };
        }
        if (it.kind === 'shape') {
          return {
            ...it,
        ...
        return {
          ...it,
          points: it.points.map((p: { x: number; y: number }) => ({ x: p.x + dx, y: p.y + dy })),
        };
      });
      return {
        ...state,
        session: {
          ...state.session,
          boardPinnedIds: pins,
          boardIntentGhosts: ghosts,
          boardAgentItems,
        },
      };
    }

    case 'files/focus-clear':
      if (state.session.filesFocus === null) return state;
      return {
        ...state,
        session: { ...state.session, filesFocus: null },
      };

    case 'rail/workspace':
      /* Replaced wholesale — the endpoint returns the whole listing, and a
         merge would leave a file on screen that has been deleted on disk. */
      return { ...state, rail: { ...state.rail, workspace: action.workspace } };
...
/**
 * Blocks past this are reported as a count. Matches the server's own cap
 * (`askSurface.ts`, MAX_CANVAS_BLOCKS) so the client does not send bytes the
 * parser will drop and then count twice.
 */
const CANVAS_STATE_BLOCK_CAP = 24;
/** One block gets its opening read; the server caps it again at 1 500. */
const CANVAS_STATE_EXCERPT_CHARS = 1_500;

/**
 * WHAT IS ON THE CANVAS, AS THE AGENT IS ALLOWED TO SEE IT.
 *
 * Two resolutions, ported from tldraw's prompt parts
 * (`docs/research/agent-drawing-and-teaching-visuals.md` §3): every block
 * blurry — id, type, title, size — and the NEWEST one focused, carrying the
 * opening of its payload. Newest rather than "selected" because the canvas has
 * no selection model yet, and the block a follow-up means is overwhelmingly
 * the one just drawn.
 *
 * `pending` blocks are excluded: a block whose tool has not returned has no
 * payload to describe and no id worth handing back as an edit target.
 *
 * Returns the surface UNCHANGED when there is nothing to say — no surface or no
 * landed blocks — so an ordinary chat turn's payload stays byte-identical to
 * the one it sent before this existed.
 *
 * Called where the REQUEST BODY is built (`connect.tsx`), not where the turn is
 * recorded: the canvas is true at the moment of the send and a persisted
 * transcript should not claim a canvas that has since moved on.
 */
export function withCanvasState(
  surface: AskSurfaceContext | null,
  blocks: readonly CanvasDocBlock[],
): AskSurfaceContext | null {
  if (!surface) return surface;
  const real = blocks.filter((b) => b.status !== 'pending');
  if (real.length === 0) return surface;
  const kept = real.slice(-CANVAS_STATE_BLOCK_CAP);
  const newestId = kept[kept.length - 1]?.id;
  return {
    ...surface,
    canvas: {
      blocks: kept.map((b) => ({
        ...
        chars: b.payload.length,
        ...(b.id === newestId
          ? { excerpt: b.payload.slice(0, CANVAS_STATE_EXCERPT_CHARS) }
          : {}),
      })),
      ...(real.length > kept.length ? { omitted: real.length - kept.length } : {}),
    },
  };
}

const BOARD_STATE_ITEM_CAP = 40;

/**
 * Attach what is already on SeqDraw / whiteboard so the agent can revise by id
 * instead of only appending. Mirrors {@link withCanvasState} for the spatial board.
 *
 * Pinned items keep their human `at` in the payload so the server's
 * `respectPins` path can honour them on the next `diagram.upsert`.
 */
export function withBoardState(
  surface: AskSurfaceContext | null,
  items: readonly import('../whiteboard/whiteboardModel').WbItem[],
  _pinnedIds: ReadonlySet<string> = new Set(),
): AskSurfaceContext | null {
  if (!surface) return surface;
  if (items.length === 0) return surface;
  const kept = items.slice(-BOARD_STATE_ITEM_CAP);
  return {
    ...surface,
    board: {
      items: kept.map((it) => {
        if (it.kind === 'text') {
          return {
            id: it.id,
            kind: 'text' as const,
            label: it.text.slice(0, 80),
            at: { x: Math.round(it.at.x), y: Math.round(it.at.y) },
          };
        }
        if (it.kind === 'noderef') {
          return {
            id: it.id,
            kind: 'noderef' as const,
        ...
        if (it.kind === 'shape') {
          return {
            ...
            kind: 'shape' as const,
            label: it.shape,
            at: { x: Math.round(it.from.x), y: Math.round(it.from.y) },
          };
        }
        if (it.kind === 'artifact') {
          const label =
            it.ref.type === 'chart' ? `chart#${it.ref.index}` : `block:${it.ref.blockId}`;
          return {
            id: it.id,
            kind: 'text' as const,
        ...
        return {
          ...
          ...(p0 ? { at: { x: Math.round(p0.x), y: Math.round(p0.y) } } : {}),
        };
      }),
      ...(items.length > kept.length ? { omitted: items.length - kept.length } : {}),
    },
  };
}
...
function send(
  ...
  const turn: UserTurn = {
    id: userId,
    role: 'user',
    text: state.composer.draft.trim(),
    /*
    ...
    contextLines: [],
    /* The chrome surface alone. The canvas state rides on the REQUEST (see
       `withCanvasState`, called where the body is built) and not on the
       recorded turn — a transcript replayed next week should not claim a
  ...
  const inFlight: InFlightTurn = {
    ...
    roundCap: resolveAskRoundCap({
      question: state.composer.draft.trim(),
...
function applyCanvasBlock(
  ...
  return {
    ...
    session: {
      ...state.session,
      /* A live block belongs to the thread on screen BY DEFINITION — stamp
         it, so the doc is owned even when it was born unstamped (boot with
         nothing on disk to hydrate from) and the write still reaches disk. */
      canvasDoc: {
        ...state.session.canvasDoc,
        forSession: state.session.activeId,
        blocks: [...landed.blocks, block],
      },
      inFlight: {
        ...inFlight,
        work: [
  ...
  switch (event.type) {
          ...
          evidence: {
            ...
            tools:
              event.name === undefined
                ? inFlight.evidence.tools
                : inFlight.evidence.tools.some((t) => t.id === event.id)
                  ? inFlight.evidence.tools
                  : [
                      ...inFlight.evidence.tools,
                      {
    ...
    case 'tool:done': {
      ...
      return withInFlight(nextState, {
        ...
        evidence: {
          ...inFlight.evidence,
          tools: inFlight.evidence.tools.map((t) =>
            t.id === event.id
              ? {
                  ...t,
                  ...(event.name ? { name: event.name } : {}),
                  evidence: event.evidence ?? t.evidence ?? null,
                  text: event.evidence ?? t.text,
                }
              : t,
          ),
        },
    ...
    case 'todo:list':
      /*
       * REPLACED WHOLESALE, like `canvas:story` and `teach:step` below, and for
       * a sharper reason: the server's `update_todos` replaces. A client that
       * merged frames would drift away from the list the server is carrying
       * into the next round's prompt, and the screen would then disagree with
       * the model about what has been done.
       *
       * NO WORK ROW. The list IS the progress display; a row beside it saying
       * "Updated the work list" repeats its own name, which is the owner's
       * standing dislike and already decided for `teach:step`.
       */
      return {
        ...state,
        session: {
          ...state.session,
          inFlight: { ...inFlight, todos: event.items.map((i) => ({ ...i })) },
        },
      };
    ...
    case 'board:item': {
      const remove = new Set(event.removeIds ?? []);
      const kept = state.session.boardAgentItems.filter((i) => !remove.has(i.id));
      const have = new Set(kept.map((i) => i.id));
      const fresh = event.items.map(boardWireToWbItem).filter((i) => !have.has(i.id));
      const nextItems = [...kept, ...fresh];
      if (fresh.length === 0 && remove.size === 0) return state;
      const nextPins = state.session.boardPinnedIds.filter((id) => !remove.has(id));
      const nextGhosts = state.session.boardIntentGhosts.filter((g) => !remove.has(g.id));
      return {
        ...state,
        session: {
          ...state.session,
          boardAgentItems: nextItems,
          boardPinnedIds: nextPins,
          boardIntentGhosts: nextGhosts,
          inFlight: {
            ...inFlight,
            work: [
              ...inFlight.work,
              {
                id: `board:${[...remove, ...fresh.map((i) => i.id)].join(',') || 'noop'}`,
                group: 'change',
                verb:
                  fresh.length > 0
                    ? `Drew ${fresh.length} mark${fresh.length === 1 ? '' : 's'} on SeqDraw`
                    : `Removed ${remove.size} mark${remove.size === 1 ? '' : 's'} from SeqDraw`,
                identifier: null,
                outcome: null,
                provenance: null,
                status: 'done',
    ...
    case 'teach:walk':
      /*
       * Replaced wholesale, like `teach:step` below and for a sharper reason:
       * a walk belongs to the picture currently on the canvas, and keeping the
       * previous one would leave a story pointing at parts that are no longer
       * drawn. No work row either — the chart's own row already said a picture
       * arrived, and the walk renders as itself.
       */
      return {
        ...state,
        session: {
          ...
          canvasDoc: {
            ...state.session.canvasDoc,
            teachWalk: { input: event.input, steps: event.steps.map((s) => ({ ...s })) },
          },
        },
      };
...
function commit(
  ...
  const assistant: AssistantTurn = {
    ...
    text: inFlight.text,
    work: resolved,
    /* The list as it stood at the end. A committed turn keeps it so a reader
       scrolling back can still see what the turn set out to do and what it
       actually finished — the same reason `work` is kept rather than cleared. */
    todos: inFlight.todos,
    effect,
    /*
