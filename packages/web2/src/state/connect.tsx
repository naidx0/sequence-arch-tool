...
import { rememberCanvasForFlush, rememberChatForFlush, clearSessionFlush } from '../sessions/sessionPersist';

import { type ChatColumnProps, type ComposerProps, type ToolbeltId } from '../chat';
...
import { withCanvasState, withBoardState } from './store';
import { withDrawingState } from '../app/workspaceAskSurface';
...
/** Apply `/api/ai-config` contextWindow probe (or published fallback) to the store. */
export function applyContextWindowFromConfig(
  store: Store,
  body: unknown,
  modelHint?: string,
): void {
  const selection = modelFromConfig(body);
  const model = modelHint?.trim() || selection.model;
  ...
  store.dispatch({
    ...
    tokens: probed ?? publishedContextWindow(model),
  });
}

/** Re-fetch ai-config and refresh the composer's context window ceiling. */
export async function refreshContextWindow(store: Store, modelHint?: string): Promise<void> {
  try {
    const res = await fetch('/api/ai-config');
    if (!res.ok) return;
    const body = await res.json();
    applyContextWindowFromConfig(store, body, modelHint);
  } catch {
    /* Unreachable or aborted — ring keeps the last known ceiling. */
  }
}
...
/** The same two phases, the root path instead of the name. */
export function groundedRepoRoot(state: AppState): string | null {
  return state.repo.phase === 'attached' || state.repo.phase === 'stale'
...
export function composerPropsFrom(
  ...
  return {
    ...
    groundingPath: groundedRepoRoot(state),
    sessionStrip:
        ...
        .stream(
          {
            ...
            context: state.composer.chips.length
              ? {
                  ...
                  lines: state.composer.chips.map((chip) =>
                    /*
                     * A CHART PART IS NOT REPO-GROUNDED SCOPE, so it does not
                     * get the grounded-id shape. `chart-part:tok (Tokenizer)`
                     * under a section headed as scope reads as a claim that
                     * `tok` is something in this repository, and on a teaching
                     * chart there is no repository at all. It says what it
                     * actually is instead, in words the model cannot misread.
                     */
                    chip.kind === 'chart-part'
                      ? `the part of the drawing the user clicked: "${chip.label}"`
                      : chip.ref
                        ? `${chip.kind}:${chip.ref} (${chip.label})`
                        : chip.label,
                  ),
                }
              : undefined,
             ...
             */
            /*
             * …AND WHAT IS ALREADY ON THE CANVAS, so a follow-up can revise a
             * block instead of appending a second one. Stage 3 of
             * `docs/research/agent-drawing-and-teaching-visuals.md`: five
             * writers shipped with no reader, which is why "change that
             * diagram" could only ever draw another diagram. Attached HERE, at
             * the moment of the send, because a block streams in mid-turn and
             * the chrome effect that sets `askSurface` does not run for that.
             */
            surface: (() => {
              const withCanvas =
                withCanvasState(state.composer.askSurface, state.session.canvasDoc.blocks) ??
                state.composer.askSurface;
              /* And the READER'S marks: notes, shapes, named ink on the AI
                 Canvas plane (`session.canvasDrawing`), merged over the
                 agent's items so the model sees what was drawn back at it. */
              return (
                withDrawingState(
                  withBoardState(
                    withCanvas,
                    state.session.boardAgentItems,
                    new Set(state.session.boardPinnedIds ?? []),
                  ),
                  state.session.canvasDrawing,
                ) ?? undefined
              );
            })(),
            /*
            ...
            ...(unattached
              ? {
                  /*
                   * Outline is the question itself, so a repo-less ask has
                   * grounding text instead of inventing architecture, and so the
                   * engine answers rather than returning a raw 409.
                   *
                   * `proposeArchitecture` IS NOT SET IN TEACH MODE, and this is
                   * the whole of a reported defect. Owner, 2026-09-13, in Teach
                   * mode with nothing attached: "what is an LLM, break it down
                   * for me please how it works, why it works". What came back
                   * opened:
                   *
                   *     Design assumptions
                   *     1. Scale: … 2. Read/write mix: … 3. Latency: …
                   *     Natural modules
                   *     Agent – Orchestrates tasks … Prompt Generator …
                   *
                   * That is `ASK_DESIGN_PROPOSE_NOW_CLAUSE` word for word —
                   * "State 3–5 numbered assumptions in plain English (scale,
                   * read/write mix, latency …)". The flag turns any turn into a
                   * proposal for a system to BUILD, so a learner asking what an
                   * LLM is got an architecture for an app that USES one. His
                   * words: "still not drawing, still not explaining".
                   *
                   * The flag was written for the blank-workspace DESIGNER, who
                   * has no outline and wants a diagram rather than a refusal.
                   * A learner in Teach mode is the other person entirely: they
                   * asked to be taught a subject, and the lesson decides what to
                   * draw. Two different jobs were sharing one branch because
                   * both of them happen to have no repository.
                   *
                   * The design payload itself STAYS in teach mode: it is what
                   * keeps `/api/ask` from 409-ing with nothing attached, and it
                   * carries the question as grounding text either way.
                   */
                  design: {
                    title: 'Blank workspace',
                    outline: question,
                    ...(state.composer.teach ? {} : { proposeArchitecture: true }),
                  },
                }
              : {}),
          },
                ...
                message:
                  result.outcome === 'unreachable'
                    ...
                    : result.outcome === 'not-json'
                      ...
                      /* THE ENGINE'S OWN SENTENCE WHEN IT WROTE ONE. "The engine
                         answered 409" is a status; "no repo attached — open a
                         repository from the home screen first" is the thing the
                         reader can act on, and it was in the response body all
                         along (askClient now keeps it). The status stays as the
                         fallback for a refusal that explained nothing. */
                      : (result.message ?? `the engine answered ${result.status}`),
                route: `POST ${ASK_STREAM_ROUTE}`,
                at: Date.now(),
              },
            });
    ...
    /* The session lock — "the chat controls the other tabs". Per-session, so
       switching conversations stops the surfaces moving on their own. */
    onSessionLockToggle: (locked: boolean) =>
      dispatch({ type: 'composer/session-lock', locked }),
...
export function chatColumnPropsFrom(state: AppState, store: Store): ChatColumnProps {
  ...
  return {
    ...
    /*
     * ACCEPT AND REJECT, WHERE THE EDIT WAS PROPOSED. The action has existed
     * since the review overlay was built and the overlay was the only place it
     * could be pressed — the reader saw "3 files changed" in the transcript and
     * had to leave it to decide anything. Same action, one more door.
     */
    onDecideFile: (proposalId: string, path: string, decision: 'accepted' | 'rejected') =>
      dispatch({ type: 'proposal/file-decide', proposalId: proposalId as ProposalId, path, decision }),
    contextWindow: state.composer.contextWindow,
    /*
    ...
    onOpen: (target: 'canvas' | 'ai-canvas' | 'rail' | 'review' | 'files' | 'browser' | 'terminal', turnId: TurnId) => {
      if (target === 'canvas') {
...
export function ConnectedChatColumn() {
  ...
  /* Decision 22: product chat chrome is V3Chat; effects below still hydrate composer. */
  ...
  useEffect(() => {
    ...
    void fetch('/api/ai-config', { signal: controller.signal })
      ...
      .then((body) => {
        ...
        applyContextWindowFromConfig(store, body);
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    void fetch('/api/canvas-doc', { signal: controller.signal })
      ...
      .then((body) => {
        ...
        /*
         * WHOSE CANVAS THIS IS, read from the LIVE store rather than the
         * render that opened the fetch. `/api/canvas-doc` resolves "the active
         * session" on the server and answers without naming it, so the only id
         * available is the one the client is on when the answer arrives — and
         * if the reader switched threads in between, that is exactly the id
         * the reducer must compare against before landing anything.
         */
        const forSession = store.getState().session.activeId;
        if (!forSession) return;
        store.dispatch({ type: 'session/canvas-hydrated', doc, forSession });
      })
      .catch(() => {
        /* No stored canvas — ordinary for a new session. */
      });
    return () => controller.abort();
    /* `activeSessionId` is a dependency ON PURPOSE. The latch above reopens
       when the id changes, but an effect that does not list the id never runs
       again to act on that — on a cold load the first fetch fired before the
       session index had named the thread, found no id, returned, and nothing
       ever tried again: the AI Canvas opened empty over a canvas.json that
       held the block (measured 2026-09-18: 0 frames over 8 s). */
  }, [store, canvasEmpty, state.session.inFlight, activeSessionId]);
  ...
  const browsingForeign = Boolean(state.session.browseRepoPath);
  useEffect(() => {
    if (settledTurns === null) return;
    if (!worthPersisting(settledTurns)) return;
    /* Soft-browsing a catalog chat must not PUT into the home workspace root —
       that is how foreign threads leaked under Workspace (owner 2026-09-16). */
    if (browsingForeign) return;
  ...
  }, [settledCount, settledTurns, browsingForeign]);
  ...
  }, [state.session.turns, browsingForeign]);

  const settledCanvas =
    state.session.inFlight === null ? state.session.canvasDoc : null;
  const settledCanvasBlocks = settledCanvas?.blocks.length ?? -1;
  const canvasSessionId = state.session.activeId ?? null;
  /*
   * ONLY THE DOC THAT NAMES THIS THREAD MAY BE WRITTEN TO THIS THREAD.
   *
   * `optimisticSwitch` moves `activeId` to B one dispatch before `browse`
   * empties the canvas, so between the two `canvasDoc` is still A's while
   * `activeId` already reads B. Keying the PUT on `activeId` ALONE spent that
   * render writing A's chart into B's canvas.json, which is both halves of the
   * owner's report at once: A's drawing appeared in B ("the canvas takes
   * over"), and the flush on the next switch wrote the wrong doc over A ("it
   * disappeared"). `CanvasDoc.forSession` is the name; this is the check.
   */
  const settledCanvasOwned =
    settledCanvas !== null &&
    canvasSessionId !== null &&
    settledCanvas.forSession === canvasSessionId;
  useEffect(() => {
    if (settledCanvas === null) return;
    if (!settledCanvasOwned) return;
    if (!worthPersistingCanvas(settledCanvas)) return;
    if (browsingForeign) return;
    if (!canvasSessionId) return;
    /* The DOC's own id, never "whoever is active when the PUT is issued". */
    rememberCanvasForFlush(settledCanvas, settledCanvas.forSession ?? null);
    /* The id rides in the URL, never "whoever is active when the server gets
       to it" — see `sessionPersist.rememberCanvasForFlush` for the New Chat
       race that the anonymous `/api/canvas-doc` write lost. */
    void fetch(`/api/sessions/${encodeURIComponent(canvasSessionId)}`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ canvas: toCanvasMemory(canvasSessionId, settledCanvas) }),
    }).catch(() => {
      /* Best effort — same contract as chat-memory. */
    });
  }, [settledCanvasBlocks, settledCanvas, settledCanvasOwned, browsingForeign, canvasSessionId]);

  const liveCanvas = state.session.canvasDoc;
  const liveCanvasOwned = canvasSessionId !== null && liveCanvas.forSession === canvasSessionId;
  useEffect(() => {
    if (browsingForeign) return;
    /* A doc that does not name this thread is not REMEMBERED either, so the
       flush the next switch issues cannot carry it over anyone. The switch
       paths call `clearSessionFlush()` right after their flush, so skipping
       here leaves nothing stale behind. */
    if (!liveCanvasOwned) return;
    rememberCanvasForFlush(liveCanvas, liveCanvas.forSession ?? null);
  }, [liveCanvas, liveCanvasOwned, browsingForeign]);
