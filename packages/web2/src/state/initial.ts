...
export const EMPTY_PERMISSION: PermissionControl = {
   ...
   */
  /* Plan is always available; Build is opt-in from Settings → Workspace,
     because it is the mode that runs commands. */
  enabled: ['plan'],
};
...
export const EMPTY_COMPOSER: ComposerSlice = {
  ...
  history: [],
  askSurface: null,
  /* OFF by default: a surface that moves on its own before anyone asked it to
     is the app taking the wheel. The lock is a thing you turn on. */
  sessionLocked: false,
  teach: false,
  teachKnown: null,
};
...
export const EMPTY_RAIL: RailSlice = {
  ...
  coverageByPath: {},
  loading: false,
  /* null = not asked yet. Distinct from `exists: false` (asked, nothing there),
     and the rail says a different sentence for each. */
  workspace: null,
};
