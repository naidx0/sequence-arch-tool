...
describe('item 0.1 — boot touches no font CDN', () => {
  ...
  it('issues only the chat-column boot reads while booting', () => {
    ...
    for (const path of hosts) {
      /* `/api/workspace`: the composer footer names the home workspace's
         real path when nothing is attached (owner, 2026-09-17) — a local
         read with no side effect, and the only way the path is not invented. */
      expect(
        ['/api/ai-config', '/api/sessions', '/api/chat-memory', '/api/canvas-doc', '/api/workspace'].includes(
          path,
        ),
      ).toBe(true);
    }
