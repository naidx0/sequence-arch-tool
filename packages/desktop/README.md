...
node tools/desktop-zip-smoke.mjs          # the PORTABLE zip, run outside the build tree
```

The third is new on 2026-09-19 and it checks the one thing the second cannot.
`--packaged` runs `release/win-unpacked/Sequence.exe`, which is INSIDE the
build tree, so a path baked in at build time passes it — and that is not
hypothetical: the Tauri shell in this project's history did exactly that.
`desktop-zip-smoke` extracts the artifact a person actually downloads into a
directory that is not the build tree and whose name contains a SPACE (because
`C:\Users\First Last` and `~/Downloads/Sequence 0.1.0` both have one, and an
unquoted spawn survives every path in a repository and dies on the first real
machine), and runs it there.

Measured 2026-09-19 on `Sequence-0.1.0-win.zip` (143.0 MB): **PASS** — the
child server came up on 127.0.0.1 and the window settled on `composer`.
...
- **macOS and Linux installers.** The config targets dmg/zip/AppImage/deb;
  only `--win nsis` and `--win zip` have been run. `pnpmInvocation`'s
  non-Windows arm and `adhoc-sign.mjs`'s darwin arm are unit-tested, not
  executed.
- **Code signing.** `no signing info identified, signing is skipped` — the
  artifacts are unsigned, so Windows SmartScreen warns on first run and macOS
  Gatekeeper asks for a right-click → Open. See the next section.
- **An application icon.** `default Electron icon is used`.

---

## "The package is broken" — 2026-09-19, and what was actually wrong

A person who downloaded the published **0.1.0** Windows installer reported it
as corrupted. The hosted asset was fetched and tested the same day:

| check | result |
| --- | --- |
| `GET …/v0.1.0/Sequence.Setup.0.1.0.exe` | 200, **187,852,868 bytes** — exactly what the release page lists |
| first two bytes | `MZ` — a valid PE |
| `7za t` | **"Everything is Ok"** — 17,001 files, 962 MB uncompressed |
| `Get-AuthenticodeSignature` | **NotSigned** |

**The archive is intact.** So the fault was never in the file, and the three
fixes below are about everything around it. Each is locked by
`tools/ci/desktop-packaging.test.mjs`.

### 1. A Windows artifact with no installer to corrupt

`win.target` now builds a **zip** beside the NSIS installer. An unsigned
180 MB NSIS installer is the shape SmartScreen blocks, that some antivirus
products quarantine mid-write, and that NSIS itself then refuses with *"the
installer you are trying to use is corrupted or incomplete"* once a single
byte has been touched — all three of which reach the person as "it's
corrupted". A zip has none of that surface, Windows extracts it with no
third-party tool, and it is the only artifact that can run from a USB stick or
a laptop where the person may not install anything.

### 2. A macOS app that can open at all

`mac.identity: null` is an owner ruling (2026-09-10: no Apple developer
account) and it stays. What it *does* is the problem — read
`app-builder-lib@25.1.8/out/macPackager.js:182-189`:
...
It returns **before** any signing, ad-hoc included. Electron ships its own
binaries ad-hoc signed, but packaging rewrites `Info.plist`, renames the
executable and adds the resource tree, which invalidates that. On Apple
silicon an unsigned executable cannot launch at all, and the message macOS
gives is

> "Sequence" is damaged and can't be opened. You should move it to the Trash.

— a corrupted-download report about a perfectly good download.
`scripts/adhoc-sign.mjs` runs as `afterPack` and signs the bundle with `-`. It
is a no-op on every other platform and **stands down when `CSC_LINK` or
`CSC_NAME` is set**, because replacing a real signature with a throwaway one
is a worse failure than the one it fixes: the artifact still looks signed.

This is not notarisation. Gatekeeper still warns on first open and the reader
still right-clicks → Open once. What it buys is that the app *can* open, which
without it it cannot, at any number of clicks.

### 3. A claim somebody can check

`pnpm desktop:dist` now ends in `scripts/checksums.mjs`, which writes
`release/SHASUMS256.txt` over every installer and archive (never `.blockmap` —
it is derived from the file beside it). **Upload it with the artifacts.** A
checksum does not stop a bad download; it turns an unanswerable report into a
one-line answer:

```bash
# Windows
certutil -hashfile "Sequence Setup 0.1.0.exe" SHA256
# macOS / Linux
shasum -a 256 -c SHASUMS256.txt
```

That is the whole point. Before this there was nothing published to compare a
local copy against, so "truncated download", "antivirus ate a byte" and
"SmartScreen blocked it" were indistinguishable from each other and from a bad
build.
...
bundle via `pnpm deploy`), then `electron-builder`, then
`scripts/checksums.mjs`. Artifacts land in `packages/desktop/release/`:

- macOS: `Sequence-<version>-arm64.dmg`, `-x64.dmg`, and matching `.zip`
- Windows: `Sequence Setup <version>.exe` (NSIS) **and `Sequence-<version>-win.zip`**
- Linux: `Sequence-<version>.AppImage`, `sequence_<version>_amd64.deb`
- All: `SHASUMS256.txt` — upload it with them
