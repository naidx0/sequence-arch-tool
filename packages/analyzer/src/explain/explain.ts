...
import { isExplanatoryHomeAsk } from '../server/askIntent.js';
 ...
 */
/*
 * SPLIT IN TWO, BECAUSE ONE HALF MUST NOT REACH A LEARNER.
 *
 * The clarifying half orders the model to interrogate before it draws. The
 * teach contract in the belt forbids exactly that ("NEVER ask the learner which
 * chart kind, which format … or what level of detail"). Composed, one prompt
 * both ordered and forbade the interrogation — and a lesson needs no tickers,
 * no schedule and no accuracy bar, so the clause had nothing to ask about even
 * when it fired.
 *
 * The cost half is about honesty and is true on every turn, lesson included: a
 * learner asking what it costs to run a model deserves "unknown" over a
 * confident "1¢" just as much as an architect does.
 *
 * Concatenated below with a single space so a NON-teach prompt is byte-identical
 * to the one that existed when this was one constant.
 */
const ASK_PREPARE_BEFORE_PROPOSE_CLAUSE =
  ...
  'fence until they answer.';

const ASK_COST_HONESTY_CLAUSE =
  'Never invent a dollar cost, a per-day price, or a "1¢" figure. ' +
...
const ASK_PREP_AND_COST_CLAUSE =
  ASK_PREPARE_BEFORE_PROPOSE_CLAUSE + ' ' + ASK_COST_HONESTY_CLAUSE;

/** The prep clause on an ordinary turn; the cost half alone on a lesson. */
function prepAndCostClause(compose?: AskComposition): string {
  return compose?.teach ? ASK_COST_HONESTY_CLAUSE : ASK_PREP_AND_COST_CLAUSE;
}
...
/**
 * THE SAME "DRAW IT", ASKED BY A LEARNER.
 *
 * `ASK_SHOW_ON_BOARD_CLAUSE` above is the right answer to "put our auth service
 * on the board" and the wrong answer to "draw me how an LLM works". Both match
 * `isShowOnBoardAsk`; only the second is a lesson. This is what a lesson gets
 * instead — the identical override of the refusal ("nothing is defined yet"),
 * with the design framing removed, because there is nothing to design.
 *
 * It names the drawing tools a teach turn actually has. `propose_topology` is
 * not one of them: the teach contract refuses it, so instructing the model to
 * call it produced a turn that either disobeyed the belt or hit a refusal at
 * the door. The canvas writers and `propose_chart` are always offered on a
 * teach turn (`canvasToolsEnabled` returns true for one unconditionally), so
 * this clause can name them without checking a surface.
 */
const ASK_TEACH_SHOW_CLAUSE =
  'DRAW WHAT YOU ARE TEACHING: The learner asked to see it, so produce a picture this turn. ' +
  'Draw the SUBJECT as it actually works in the world — its real parts, in their real order, ' +
  'with the names the field uses. This is NOT a software design task: do NOT propose an ' +
  'architecture for a system that would implement the subject, do NOT state design assumptions, ' +
  'do NOT name modules by role ("Input Handler", "Processing Engine", "Output Layer"), and do NOT ' +
  'call `propose_topology`. Use `propose_chart` or a `canvas.write_*` tool. Do NOT refuse with ' +
  '"nothing is defined yet" or "no repository is attached" — a lesson needs no repository. Write ' +
  'the explanation in plain English beside the picture, never instead of it.';

/**
 * "DRAW ME HOW AN LLM WORKS" MATCHED NEITHER OVERRIDE, WHICH IS ITS OWN BUG.
 *
 * `isShowOnBoardAsk` wants the board named, or the bare object of "draw it /
 * this / that". `isFigureItOutAsk` wants "show me how". Both were written for
 * an ARCHITECT at a board, and both miss the sentence a learner actually types
 * — the verb with the subject right after it. Measured on the owner's own
 * wording: zero matches, so a lesson that asked for a picture in plain English
 * got no instruction to produce one.
 *
 * Deliberately teach-only and deliberately narrow: request verbs, never nouns.
 * "graph" and "chart" are out because a repository question about a dependency
 * graph is not a request to draw one, and this must not widen the design path
 * by a single question.
 */
function isPlainDrawAsk(question: string): boolean {
  return /\b(draw|sketch|diagram|illustrate|visuali[sz]e)\b/.test(question.toLowerCase());
}

/**
 * THE TWO OVERRIDES THAT SAY "DRAW NOW", AND THE ONE A LEARNER GETS INSTEAD.
 *
 * `isFigureItOutAsk` and `isShowOnBoardAsk` are different questions with the
 * same answer — stop asking, produce the picture — so they were two independent
 * `if`s pushing two clauses. On a lesson they are still the same answer, and it
 * is a THIRD clause, so the branch belongs in one place rather than copied into
 * two builders where it could drift.
 *
 * MEASURED, and this is why the first fix was not enough: the owner's exact
 * phrasing, "show me how attention works", does NOT match `isShowOnBoardAsk` —
 * it matches `isFigureItOutAsk`'s `\bshow me (?:how|what|the)\b`. And
 * `ASK_FIGURE_IT_OUT_CLAUSE` reads *"State 3–5 numbered assumptions in plain
 * English, then CALL the `propose_topology` tool"*. The numbered assumptions he
 * reported came from here, not from the show-on-board clause that was blamed
 * for them. A locking test built from his words, rather than from the
 * convenient words, is what found it.
 *
 * Never both, and never more than one: three clauses all claiming to override
 * the same turn is how a prompt ends up arguing with itself.
 */
function pushDrawOverrideClause(
  L: string[],
  question: string,
  compose?: AskComposition,
): void {
  const figureItOut = isFigureItOutAsk(question);
  const showOnBoard = isShowOnBoardAsk(question);
  if (compose?.teach === true) {
    if (figureItOut || showOnBoard || isPlainDrawAsk(question)) L.push(ASK_TEACH_SHOW_CLAUSE);
    return;
  }
  if (figureItOut) L.push(ASK_FIGURE_IT_OUT_CLAUSE);
  if (showOnBoard) L.push(ASK_SHOW_ON_BOARD_CLAUSE);
}
...
export function buildAskPrompt(
  ...
  L.push(prepAndCostClause(compose));
  pushDrawOverrideClause(L, question, compose);
...
export interface AskComposition {
  ...
  subjectNodeId?: string;
  /**
   * TRUE WHEN THIS TURN IS A LESSON, AND THE PROMPT BUILDER COULD NOT TELL.
   *
   * Owner report, 2026-09-13: he asked teach mode to draw how an LLM works and
   * got back a *design proposal* — numbered assumptions, modules named by role,
   * `propose_topology` fired. Measured, not reasoned about: the belt builder
   * (`renderAskInstructionBelt`) has always taken `teach`, but the PROMPT
   * builders on this side never did. So a lesson that used the word "draw"
   * matched `isShowOnBoardAsk`, and both builders pushed
   * `ASK_SHOW_ON_BOARD_CLAUSE` — whose text is *"Frame the answer as a design
   * proposal (state assumptions), then CALL `propose_topology`"*. In design
   * mode it went further: `buildDesignAskPrompt`'s `propose` flag is an OR over
   * `isShowOnBoardAsk`, so the whole greenfield-architecture directive stack
   * landed on top of a question about transformers.
   *
   * The clauses were not wrong. They were answering a question nobody asked,
   * because the only two things they could see were the words and the outline,
   * and a lesson about a system and a request to design one use the same words.
   *
   * Teaching a thing and proposing a thing are different jobs: one draws what
   * already exists in the world, the other invents something that does not.
   * With this set, "draw it" still means DRAW IT — it just means draw the
   * subject rather than a system that might implement it.
   */
  teach?: boolean;
}
...
export function buildDesignAskPrompt(
   ...
   *
   * EXCEPT ON A LESSON. A General chat with nothing attached sends
   * `design: { title: 'Blank workspace' }`, so every teach turn without a
   * repository lands in this builder — and "draw me how an LLM works" matched
   * `isShowOnBoardAsk`, flipped `propose`, and pulled in the entire greenfield
   * architecture stack: PROPOSE ARCHITECTURE, the seqd breakdown directive, and
   * PROPOSE NOW's numbered assumptions. That is why the owner's transformer
   * lesson came back as a design brief. A learner asking to see a thing has not
   * asked to design a system, and `proposeArchitecture` is left in the OR so an
   * EXPLICIT design request in teach mode is still honoured.
   */
  /*
   * AND NEVER FOR AN EXPLANATION. `prefersSeqdDiagramAsk` matches the bare
   * noun "architecture", so "explain LLMs and their internal architecture"
   * produced the assumptions-then-seqd directives the owner read back to us
   * (2026-09-17). `isExplanatoryHomeAsk` is the first clause of the predicate
   * the route and the belt use for the home workspace, so the three cannot
   * disagree about whether an ask is an explanation.
   */
  const propose =
    (design.proposeArchitecture === true ||
      (compose?.teach !== true && (isShowOnBoardAsk(question) || prefersSeqdDiagramAsk(question)))) &&
    !isExplanatoryHomeAsk(question);
  const L: string[] = [];
  ...
  /*
   * NEVER ON A LESSON, and this one is the sharper half of the 2026-09-13 bug.
   *
   * `ASK_DESIGN_BREAKDOWN_SEQD_DIRECTIVE` opens with "If required inputs are
   * missing … ask those clarifying questions FIRST and do not emit a fence
   * until they answer", and names "explain how X works" as one of its triggers.
   * The teach contract in the belt says the opposite in the same prompt:
   * "NEVER ask the learner which chart kind, which format … or what level of
   * detail". Two lines of one prompt disagreeing, with the model free to obey
   * either — the exact failure `ASK_DESIGN_PROPOSE_NOW_CLAUSE`'s comment above
   * describes, arrived at from a different direction.
   *
   * Its body is a design brief besides: "natural modules for THAT domain",
   * detail.whatItDoes per module, an Execution order list. A transformer does
   * not have modules; it has layers, and this directive cannot say so.
   */
  if (compose?.teach !== true && (prefersSeqdDiagramAsk(question) || propose)) {
   ...
   */
  if (propose) {
    /* An explicit design request — the design framing is the right answer, teach or not. */
    L.push(ASK_DESIGN_PROPOSE_NOW_CLAUSE);
    if (isShowOnBoardAsk(question)) L.push(ASK_SHOW_ON_BOARD_CLAUSE);
  } else {
    pushDrawOverrideClause(L, question, compose);
  }
