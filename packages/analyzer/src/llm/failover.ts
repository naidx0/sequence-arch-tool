 ...
 */
/**
 * A QUOTA ERROR IS AN ANSWER, NOT WEATHER (wave C1 of the carrying-harness plan).
 * A 402, or a 4xx whose body says the credits are gone, will not clear in four
 * seconds and must never spend a retry or a failover hop. One regex, here, used
 * by both this module's transient check and the provider's error classifier.
 */
const QUOTA_BODY =
  /insufficient[_ ]?(?:quota|credits?|balance|funds)|credits?[^.]{0,24}(?:exhausted|depleted|gone|used up)|quota[^.]{0,16}(?:exceeded|exhausted|reached)|billing|out of credits|payment required/i;
...
export function isQuotaExhausted(err: unknown): boolean {
  ...
  if ((status === 429 || status === 400 || status === 422) && QUOTA_BODY.test(body)) return true;
  return false;
}
