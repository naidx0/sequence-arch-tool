...
test('repoServer reaches the model ONLY through the trust gate', () => {
  ...
  /* THREE since 2026-09-17: the two ask routes, plus the goal run's own turn
     (`goalRunDeps`), which assembles a pipeline input with no request behind it
     and therefore had to be given the gated renderer deliberately. The count is
     the deliberate-widening guard; the ABSENCE assertion above is the lock. */
  assert.equal(gated.length, 3, `expected the three ask call sites, found ${gated.length}`);
});
