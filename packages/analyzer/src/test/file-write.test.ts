...
test('PUT /api/file: workspace jail when no repo is attached; 415 on wrong content-type', async () => {
  const { base, close } = await startServer(null);
  try {
    const res = await putFile(base, 'a.txt', 'x');
    assert.strictEqual(res.status, 200, 'no repo ⇒ write into ~/.sequence/workspace');
    const body = (await res.json()) as { ok?: boolean; path?: string };
    assert.strictEqual(body.ok, true);
    assert.strictEqual(body.path, 'a.txt');
  } finally {
    await close();
  }
