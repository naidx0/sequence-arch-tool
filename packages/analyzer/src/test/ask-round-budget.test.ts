...
test('re-reading the same file stops the loop early — the budget is progress, not a count', async () => {
  ...
  try {
     ...
     */
    /* +2 since wave A3 of the carrying-harness plan: when the model asks for
       the same thing again past the budget, the loop steers ONCE (a `steer`
       row of its own) and lets that request run as one more round; the stop
       comes on the usual terms the round after. */
    assert.strictEqual(
      toolStarts(events),
      3 + UNPRODUCTIVE_ROUND_LIMIT,
      'the loop steers once, buys one round, then stops once rounds stop bringing anything back',
    );
    assert.ok(
...
test('a repeated FAILURE is not mistaken for new evidence', async () => {
  ...
  try {
    ...
    assert.strictEqual(
      toolStarts(events),
      3 + UNPRODUCTIVE_ROUND_LIMIT, /* +2: the one wave A3 steer row and the round it buys */
