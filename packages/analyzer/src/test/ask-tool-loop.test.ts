...
import { runAskPipeline, computeAskInstructionHash, hashAskInstructionBelt, renderAskInstructionBelt, TEACH_MODE_INSTRUCTIONS, GENERAL_KNOWLEDGE_CAPTION, type AskPipelineInput, type AskStreamEvent } from '../server/askPipeline.js';
import { classifyAskIntent } from '../server/askIntent.js';
 ...
 */
test('ASK_TOOL_ALLOWLIST is exactly these twenty, and widening it is deliberate', () => {
  /*
   *   `write_plan` / `mark_step_done` — added 2026-09-17. `update_todos` below
   *   owns a list that dies with the TURN, and says so in `todoList.ts`'s first
   *   paragraph. These two own a plan that OUTLIVES it, which is the whole
   *   difference: owner, "once you create a goal, you can tell the agent to
   *   work on the goal, and that way we can enforce long-running tasks even
   *   with weaker models." A weak model cannot hold a twenty-step job across
   *   twenty turns; it does not have to, because the job is on disk and each
   *   turn is handed one step. `mark_step_done` writing the `[x]` is the ONLY
   *   thing that closes a step — a loop that reads the model's prose for a
   *   verdict ends the moment it says something that sounds like finishing, and
   *   models say that constantly. Both are off every belt unless the session
   *   HAS a goal, and `mark_step_done` is off every belt but Build.
   */
  /*
   ...
   *
   *   `update_todos` — added 2026-09-13. A turn may run sixteen rounds and the
   *   person waiting saw `step:start provider` sixteen times: the pipeline's own
   *   phases, identical whatever the job was. Meanwhile Plan mode produced PROSE
   *   ("End with the plan itself") that nothing executed, so a long piece of work
   *   had a plan with no execution and an execution with no plan. This is the
   *   join: the model declares the WHOLE list, it is carried into every later
   *   round's prompt above the (capped, droppable) evidence ledger, and a step
   *   reaching `done` resets the unproductive-round counter — because re-reading
   *   a file to apply an edit you already planned is "nothing new" to the ledger
   *   and is the entire job to the reader. It writes nothing, spawns nothing and
   *   runs nothing; `todoList.ts` says so in its own header, because a list that
   *   LOOKS like an executor invites a wrong model of the product.
   *
   *   `walk_example` — added 2026-09-14, the fourth beat of the teaching
   *   journey ("ask what the learner knows -> DRAW the thing -> have you seen
   *   this before? -> walk one real input through the picture"). The first
   *   three shipped; a box-and-arrow chart shows the PARTS and only a worked
   *   example shows what happens, which is the owner's whole complaint about
   *   the three-box LLM drawing. Every partId is checked against the chart the
   *   turn actually emitted and one invented id voids the WHOLE walk, because
   *   a walk with a silent hole is one the reader cannot tell is incomplete.
   *   It is teach-only: `askToolsForJobMode` filters it out of the base belt
   *   and the teach branch puts it back.
   */
...
test('askPipeline: stops re-reading one file rather than spending the whole budget on it', async () => {
  ...
  /* No steer here: the fourth scripted reply carries no tool call, and the wave
     A3 steer is for a model that asked for tools again, not one that answered. */
  assert.strictEqual(toolStarts.filter((e) => (e as { name?: string }).name === 'steer').length, 0);
...
test('computeAskInstructionHash: same fragments yield same hash; hint change changes hash', () => {
  ...
  /* THE MODE IS IN THE HASH, compared against the OTHER rung. The base is
     `plan`, so `build` is what must hash differently. After the two-rung
     collapse this line compared plan against plan — a tautology, and the kind
     that FAILS rather than silently passing, which is how it was caught. */
  const buildHash = computeAskInstructionHash({ ...base, permission: 'build' });
  assert.notStrictEqual(hashA, buildHash);

  /* Work mode differs from code mode by exactly one tool — the shell — so the
     two belts only diverge in a mode that HAS the shell. Under Plan neither
     carries it and the hints are legitimately identical. */
  const workHint = renderAskToolHintSection('work', 'build').join('\n');
  const codeHint = renderAskToolHintSection('code', 'build').join('\n');
  assert.notStrictEqual(workHint, codeHint);
  const fullBase = { ...base, permission: 'build' as const };
...
test('askPipeline: run_command emits command:log SSE with structured output', async () => {
  ...
  const input: AskPipelineInput = { ...base, permission: 'build' };
  const events: AskStreamEvent[] = [];
  await runAskPipeline(input, (e) => events.push(e));
  const { byType } = collect(events);
...
/* ==================================================== plot/math draw routing ===== */

test('plot/math draw: belt prefers propose_chart and omits diagram/topology IR', () => {
  const belt = renderAskInstructionBelt({
    designMode: false,
    repoRoot: '/repo',
    question: 'draw a parabola',
  } as never);
  assert.match(belt, /PLOT \/ MATH DRAW/);
  assert.match(belt, /propose_chart/);
  assert.doesNotMatch(belt, /SYSTEM BOARD \(SeqDraw structured\)/);
  assert.doesNotMatch(belt, /--- THE WHITEBOARD/);
  /* Topology tool fence must not be advertised; "Do NOT call propose_topology" is fine. */
  assert.doesNotMatch(belt, /"name"\s*:\s*"propose_topology"/);
});

test('plot/math draw: askToolsForJobMode omits propose_topology', () => {
  const belt = askToolsForJobMode('code', 'plan', { plot: true });
  assert.ok(!belt.includes('propose_topology'));
  assert.ok(belt.includes('propose_chart'));
  const normal = askToolsForJobMode('code', 'plan');
  assert.ok(normal.includes('propose_topology'));
});

test('plot a parabola classifies as draw (forced chart harness)', () => {
  assert.strictEqual(classifyAskIntent('plot a parabola for me'), 'draw');
  assert.strictEqual(isPlotOrMathDrawAsk('plot a parabola for me'), true);
});

test('plot/math draw: propose_topology and diagram.upsert are soft-refused', async () => {
  const repo = shopfrontRepo();
  const root = fs.realpathSync(repo);
  const ctx = {
    resolveReadable: (p: string) => resolveInRepo(root, p),
    repoRoot: root,
    designMode: false,
    question: 'draw a parabola for me',
  };
  const topo = await executeAskTool(
    'propose_topology',
    {
      title: 'Input modules',
      nodes: [{ id: 'svc:input', label: 'InputModule', kind: 'service' }],
      edges: [],
    },
    ctx,
  );
  assert.strictEqual(topo.ok, false);
  assert.match(String(topo.evidence), /math\/plot|propose_chart/i);

  const diagram = await executeAskTool(
    'diagram.upsert',
    {
      layoutProfile: 'flow',
      nodes: [{ id: 'n1', role: 'note', label: 'curve' }],
    },
    ctx,
  );
  assert.strictEqual(diagram.ok, false);
  assert.match(String(diagram.evidence), /math\/plot|propose_chart/i);
});

test('plot/math draw: canvas.write_markdown is soft-refused (must propose_chart)', async () => {
  const repo = shopfrontRepo();
  const root = fs.realpathSync(repo);
  const ctx = {
    resolveReadable: (p: string) => resolveInRepo(root, p),
    repoRoot: root,
    designMode: false,
    question: 'plot a parabola on AI Canvas',
    canvasToolsEnabled: true,
  };
  const md = await executeAskTool(
    'canvas.write_markdown',
    { title: 'Parabola Visualization', content: '  /\\\n /  \\\n/    \\' },
    ctx,
  );
  assert.strictEqual(md.ok, false);
  assert.match(String(md.evidence), /propose_chart|math\/plot|not a chart/i);
});

test('plot a parabola: product floor emits propose_chart when the model only dumps prose/seqd', async () => {
  /*
   * Owner walk 2026-09-16: AI Canvas plot ask → architecture prose + empty
   * seqd, never a line chart. Forced round still fails; the floor must draw.
   */
  const repo = shopfrontRepo();
  const { calls, callProvider } = makeProviderScript([
    {
      text:
        'I will design a parabola plotting system from scratch.\n\n' +
        '```seqd\n\n```\n\nAssumptions\nScale: a single evaluation.',
    },
    {
      text: 'Architecture only — still no propose_chart.',
    },
  ]);
  const base = await makeAttachedInput(repo, callProvider);
  const input: AskPipelineInput = {
    ...base,
    question: 'Plot a parabola for me on the AI Canvas. Show f(x)=x^2 with axes.',
    surface: { id: 'ai-canvas', title: 'AI Canvas' },
  };
  const events: AskStreamEvent[] = [];
  const result = await runAskPipeline(input, (e) => events.push(e));
  const charts = events.filter((e) => e.type === 'chart:proposal') as Array<{
    type: 'chart:proposal';
    chart: { kind: string; title: string; items: unknown[]; axes?: { x?: string; y?: string } };
  }>;
  assert.ok(charts.length >= 1, 'product floor must emit at least one chart:proposal');
  const chart = charts[0]!.chart;
  assert.strictEqual(chart.kind, 'line');
  assert.ok(chart.items.length >= 10, 'curve has enough points to read as a parabola');
  assert.ok(
    chart.axes?.x === 'x' || /x\^2|parabola/i.test(chart.title),
    'chart names the parabola / x axis',
  );
  const toolStarts = events.filter(
    (e) => e.type === 'tool:start' && (e as { name?: string }).name === 'propose_chart',
  );
  assert.ok(toolStarts.length >= 1, 'chat must get a propose_chart tool card');
...
test('askPipeline: the identity follows the permission — engineer under full+edit, answerer otherwise', async () => {
  ...
  const root = fs.realpathSync(repo);
  {
    ...
    await runAskPipeline(
      { ...base, question: 'Fix the gateway health route', permission: 'build', applyProposedFiles: testAutoApply(root) },
      () => {},
    );
...
test('askPipeline: no named path, or a chat question, seeds nothing', async () => {
  ...
  await runAskPipeline(
    { ...base2, question: 'Fix invented/path/nowhere.py please', permission: 'build' },
    () => {},
  );
...
test('run_command sandbox path: arbitrary shell runs ONLY under sandboxExec + full, and reports its exit code', async () => {
  ...
  const gated = await executeAskTool(
    ...
    { ...base, permission: 'build' } as never,
  );
  ...
  const freed = await executeAskTool(
    'run_command',
    { cmd: 'echo hi | tr a-z A-Z' },
    { ...base, permission: 'build', sandboxExec: true } as never,
  );
  ...
  const propose = await executeAskTool(
    'run_command',
    { cmd: 'echo nope | cat' },
    { ...base, permission: 'plan', sandboxExec: true } as never,
  );
...
test('a FAILING run_command feeds its full output (and the run_command header) back to the model', async () => {
  ...
  const input: AskPipelineInput = { ...base, question: 'run the test', permission: 'build' };
  await runAskPipeline(input, () => {});
...
test('sequence ask: --done-when-skip without a reason is a usage error, --done-when flows through', () => {
  ...
  const cmd = parseAskArgs(['fix the gateway', '--permission', 'build', '--done-when', 'pytest -q']);
...
test('subject not in the repo: a chart the MODEL drew still says where it came from', async () => {
  /*
   * MEASURED, 2026-09-11, through the shipped route on the shopfront fixture —
   * eight services of e-commerce, no machine learning anywhere — with Teach on
   * and the question "what is machine learning?".
   *
   * The learner was shown a chart titled "How a request reaches the ML model",
   * items `User (Composer)`, `Prediction Service`, `ML Engine (trained model)`,
   * `Feature Store`. None of those is in that repository. It was drawn on the
   * board of an attached repo with `caption: null` — nothing saying it did not
   * come from the scan.
   *
   * ── WHY THE EXISTING GUARDS ALL HELD AND IT STILL HAPPENED ───────────────
   *
   * `nodeIds` were all null, so zero-fabrication-by-construction worked: the
   * chart claimed no node of this graph. But an uncaptioned diagram on an
   * attached repository's canvas READS as that repository's own, and no node id
   * is needed to make that claim.
   *
   * The caption lives inside `drawFromGeneralKnowledge`, which is a floor for
   * when the model draws NOTHING (`chartsThisTurn === 0`). Here the model drew,
   * so the floor never ran. That gate is right when the subject is in the
   * repository — a chart the model wrote about the code beats a derived one —
   * and exactly wrong when `subjectNotInRepo` is true, because then the model is
   * drawing something the repository does not contain.
   *
   * Asserted on the EVENTS, not on the prose: what reaches the canvas is the
   * claim, and the prose is not what the learner looks at.
   */
  const repo = shopfrontRepo();
  ...
  const script = (): CannedProvider[] => [
      ...
      toolRequests: [
        {
          ...
          args: {
            version: 1,
            kind: 'data-flow',
            title: 'How a request reaches the ML model',
            /* No nodeId on any item — exactly what the real model produced, and
               what the no-fabrication rule already allows through. */
            items: [
              { id: 'ui', label: 'User (Composer)' },
              { id: 'srv', label: 'Prediction Service' },
              { id: 'ml', label: 'ML Engine (trained model)' },
            ],
            links: [
              { from: 'ui', to: 'srv' },
              { from: 'srv', to: 'ml' },
    ...
    { text: 'Machine learning is a way for computers to learn from examples.' },
  ];
  ...
  await runAskPipeline(
    {
      ...
      teach: true,
      /* The flag the real turn carries: a repository IS attached and the
         subject is not in it. */
      teachContext: { subjectNotInRepo: true },
    },
    (e) => events.push(e),
  );

  const charts = (collect(events).byType['chart:proposal'] ?? []) as Array<{
    chart: { caption?: string; items: { nodeId?: string }[] };
  }>;

  /* NON-VACUITY: the model really drew, or the assertion below is satisfied by
     a turn where nothing was charted at all. */
  assert.ok(charts.length >= 1, 'the model drew no chart — this case tests the one it drew');

  for (const c of charts) {
    assert.strictEqual(
      c.chart.caption,
      GENERAL_KNOWLEDGE_CAPTION,
      'a chart drawn while the subject is absent from the repository reached the canvas ' +
        'without saying so — it reads as a diagram of this repository',
    );
    /* The rule that already held, kept held: no chart about an idea may claim a
       node of this graph. */
    for (const item of c.chart.items) {
      assert.strictEqual(item.nodeId, undefined, 'an idea chart claimed a node of this repository');
    }
  }

  /* And the step that rides it carries the same provenance, because the step's
     caption is what the board shows beside the picture. */
  const steps = (collect(events).byType['teach:step'] ?? []) as Array<{ caption: string }>;
  assert.ok(steps.length >= 1, 'a teach turn with a chart emitted no step');
  assert.strictEqual(
    steps[0]!.caption,
    GENERAL_KNOWLEDGE_CAPTION,
    'the step beside the picture did not say where the picture came from',
  );
});
