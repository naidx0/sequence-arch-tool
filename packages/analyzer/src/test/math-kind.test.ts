...
test('a plain parabola plot uses x^2, not a shifted quadratic', () => {
  const plot = buildExamplePlot('Plot a parabola for me. Show a regular f(x)=x^2.')!;
  assert.ok(plot);
  assert.strictEqual(plot.source, 'x^2');
  assert.match(plot.chart.title, /parabola|x\^2/i);
  assert.strictEqual(plot.chart.kind, 'line');
  assert.strictEqual(plot.chart.axes?.x, 'x');
});
