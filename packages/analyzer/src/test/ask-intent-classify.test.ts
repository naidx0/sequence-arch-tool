...
import { classifyAskIntent, isDrawishAskQuestion, isPlotOrMathDrawAsk, isTeachAskQuestion } from '../server/askIntent.js';
...
/* ════════════════════════════════════════════════════════════════════════════
   PLOT / MATH DRAW — owner walk 2026-09-15.

   "draw a parabola" classified draw (correct) but armed topology/diagram and
   landed invented service modules. These lock the plot detector and that
   architecture-scoped draws still stay on the board path.
   ════════════════════════════════════════════════════════════════════════════ */

test('isPlotOrMathDrawAsk: parabola / curve / plot — yes; architecture scope — no', () => {
  assert.strictEqual(isPlotOrMathDrawAsk('draw a parabola'), true);
  assert.strictEqual(isPlotOrMathDrawAsk('plot y = x^2'), true);
  assert.strictEqual(isPlotOrMathDrawAsk('sketch a sine curve'), true);
  assert.strictEqual(isPlotOrMathDrawAsk('draw me a line chart of revenue'), true);
  assert.strictEqual(classifyAskIntent('draw a parabola'), 'draw');
  /* Owner walk wording — must be draw so the forced chart round runs. */
  assert.strictEqual(classifyAskIntent('plot a parabola for me'), 'draw');

  assert.strictEqual(isPlotOrMathDrawAsk('draw the auth architecture'), false);
  assert.strictEqual(isPlotOrMathDrawAsk('diagram the microservices on the board'), false);
  assert.strictEqual(isPlotOrMathDrawAsk('what calls billing?'), false);
});
