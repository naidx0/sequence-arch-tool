...
test('no-repo: /api/status is {attached:false}; graph endpoints 409, not a crash', async () => {
  ...
  try {
    ...
    // Graph endpoints refuse cleanly (409). /api/tree soft-binds to the
    // default workspace when unattached (Files / General home).
    {
      const r = await fetch(`${base}/archgraph.json`);
      assert.strictEqual(r.status, 409, '/archgraph.json should be 409 with no repo');
      assert.match(((await r.json()) as { error: string }).error, /no repo attached/);
    }
    {
      const r = await fetch(`${base}/api/tree`);
      assert.strictEqual(r.status, 200, '/api/tree should serve the default workspace when unattached');
    }
...
test('no-repo → attach → repo endpoints work and /api/status flips', async () => {
  ...
  try {
    ...
    assert.strictEqual(before.status, 200, 'unattached tree is the default workspace');
    const beforeGraph = await fetch(`${base}/archgraph.json`);
    assert.strictEqual(beforeGraph.status, 409);
    ...
    assert.strictEqual(treeAfter.status, 200, 'after detach, tree soft-binds back to workspace');
    const graphAfter = await fetch(`${base}/archgraph.json`);
    assert.strictEqual(graphAfter.status, 409);
  } finally {
    await close();
    t.cleanup();
  }
