...
test('permissionAutoWrites: ONLY build, and every other value stages', () => {
  /*
   * Two rungs since 2026-09-13 (owner: "teach mode, plan mode, build mode …
   * it should be pretty simple"). This gate was an exclusion list naming three
   * of four values and letting the fourth fall through — a default-ALLOW
   * written as a default-deny, and one missing name away from a defect.
   *
   * It is a membership test now, so the UNKNOWN cases below are the point: a
   * stale client, a hand-written body, or a mode added later and not wired
   * here all STAGE rather than write. The safe direction for a value this
   * build has never heard of.
   */
  assert.equal(permissionAutoWrites('build'), true);
  assert.equal(permissionAutoWrites('plan'), false);
  assert.equal(permissionAutoWrites(undefined), false);
  for (const stale of ['propose', 'autoEdit', 'full', 'FULL', 'build ', '']) {
    assert.equal(
      permissionAutoWrites(stale as never),
      false,
      `${JSON.stringify(stale)} must not be able to write without an accept`,
    );
  }
});
