...
/*
 * GIT_DIR BEATS cwd, so it is removed from this process before anything runs.
 *
 * Deleted from `process.env` rather than stripped per-spawn on purpose: the
 * server helpers under test — `gitCreateBranch`, `gitHandoffWorktree` — spawn
 * their own git and inherit this process's environment, so a fix that only
 * cleaned the local helper below would leave exactly the calls that write.
 * See the comment on `freshGitRepo` for what that cost.
 */
...
/**
 * A repository of this test's own, PROVED to be its own before it is used.
 *
 * ── THE INCIDENT THIS GUARD EXISTS FOR, 2026-09-11 ────────────────────────
 *
 * Four branches this test invents as fixtures — `dup`, `feature/handoff`,
 * `handoff/one`, `preexisting` — turned up as real refs in the PRODUCT
 * checkout, authored by `Agent <agent@example.com>`, which is the identity
 * configured six lines below. The repository also gained a registered worktree
 * pointing into a temp directory.
 *
 * `cwd` was never wrong: every call already passed this temp repo. `GIT_DIR`
 * was set in the environment, and GIT_DIR BEATS cwd. Git does not discover a
 * repository when it has been handed one. The suite had been launched from a
 * git hook, and git exports GIT_DIR into hooks, so every `git` in every test
 * of that run addressed the product repository no matter which directory it
 * ran in.
 *
 * Measured, two throwaway repos, same command:
 *
 *   cwd=temp, no GIT_DIR      -> branch created in temp   (correct)
 *   cwd=temp, GIT_DIR=prod    -> branch created in PROD   (the leak)
 *
 * The hook shim now unsets GIT_DIR, which closes the door this came through.
 * This is the second lock, because a test must own its specimen whatever the
 * environment hands it: the scrub at the top of this file applies to the
 * server helpers too, which spawn their own git and would otherwise inherit
 * the same variable, and the assertion below refuses to hand back a directory
 * that git does not agree is its own repository.
 */
function freshGitRepo(): string {
  ...
  /*
   * THE SPECIMEN IS ITS OWN, or this test does not run. `--show-toplevel` is
   * git's own answer to "which repository am I in", asked from inside the
   * directory we just initialised. If anything — an inherited GIT_DIR, a
   * failed init, a temp directory nested inside a checkout — makes git answer
   * with some other tree, every assertion after this would be measuring that
   * tree instead, and the writes would land there.
   */
  const seenAs = git(repo, ['rev-parse', '--show-toplevel']).stdout.trim();
  const resolved = seenAs === '' ? '' : fs.realpathSync(seenAs);
  assert.strictEqual(
    resolved,
    repo,
    `this test would have operated on ${resolved || '(no repository)'} instead of its own ` +
      `fixture at ${repo}. Refusing to create branches in somebody else's repository.`,
  );
