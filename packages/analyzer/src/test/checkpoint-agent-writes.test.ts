...
async function runFullTurnBuffered(
  ...
  try {
    ...
    const res = await fetch(`${base}/api/ask`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ question: 'Fix the notes route on both sides', permission: 'build', ...body }),
    });
    const text = await res.text();
...
test('a Build turn that writes two files leaves a restore point to the PRE-turn tree', async () => {
