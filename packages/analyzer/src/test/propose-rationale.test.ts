...
describe('propose_files', () => {
  ...
  it('THE MODEL IS TOLD IT CAN SEND ONE', async () => {
    ...
    /* The HINT BLOCKS, not the first mention of the name - which is a comment
       at the top of the file and nowhere near the prompt text.

       EVERY occurrence, not the first. There are two hints now — the attached
       repo's and the local workspace's (`renderWorkspaceToolHintSection`) —
       and this assertion read only whichever happened to come first in the
       file. The workspace hint landed without `rationale`, so a General chat
       would never have sent one, and the guard written to prevent exactly that
       stayed green because it was looking at the other hint. A rule that
       protects the first of N copies protects nothing. */
    const shown = [...src.matchAll(/"name":"propose_files"/g)].map((m) => m.index ?? 0);
    assert.ok(shown.length > 0, 'the prompt shows the model a propose_files call');
    for (const at of shown) {
      assert.match(
        src.slice(at, at + 400),
        /rationale/,
        `every propose_files hint must advertise rationale (one at index ${at} does not)`,
      );
    }
  });
});
