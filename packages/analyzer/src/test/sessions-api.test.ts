...
test('sessions A/B isolation + board.seqd round-trip + PUT active', async () => {
  ...
  try {
    ...
    /* New Chat seeds empty board.seqd — B must not inherit A's board. */
    assert.ok(getB.boardSeqd, 'new session seeds an empty board.seqd');
    assert.ok(!getB.boardSeqd.includes('board-a'), 'B must not carry A\'s board');
    assert.ok(getB.boardSeqd.includes('"nodes":[]') || getB.boardSeqd.includes('"nodes": []'));
...
test('REPO SCOPE still serves the catalog; writes reach catalogued repos only', async () => {
  // Owner 2026-09-14: attaching a project must not hide other projects' chats.
  // GET discloses the same recent-repos catalog in both scopes; the write fence
  // accepts exactly that list. An uncatalogued path is still a 400.
  ...
  const stranger = path.join(homeRoot, 'stranger');
  fs.mkdirSync(homeRoot, { recursive: true });
  fs.cpSync(PLAINAPP, attached, { recursive: true });
  fs.cpSync(PLAINAPP, other, { recursive: true });
  fs.cpSync(PLAINAPP, stranger, { recursive: true });
  const otherIndex = ensureSessionsMigrated(other);
  const strangerIndex = ensureSessionsMigrated(stranger);
  const storeDir = path.join(homeRoot, '.sequence');
  addRecent(storeDir, other);
  // `stranger` has sessions on disk but is NOT in recents — must stay refused.
  ...
  try {
    ...
    assert.ok(
      listed.repos.some((r) => r.path === other),
      'repo scope still lists other recent projects',
    );

    const renamed = await fetch(`${base}/api/sessions/${otherIndex.activeId}`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ title: 'From catalog while attached', repoPath: other }),
    });
    assert.strictEqual(renamed.status, 200, 'a catalogued foreign thread stays editable');
    assert.strictEqual(
      readIndexOnDisk(other).sessions.find((s) => s.id === otherIndex.activeId)?.title,
      'From catalog while attached',
    );

    const strangerBefore = readIndexOnDisk(stranger);
    const refused = await fetch(`${base}/api/sessions/${strangerIndex.activeId}`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ title: 'SHOULD NOT LAND', repoPath: stranger }),
    });
    assert.strictEqual(refused.status, 400, 'an uncatalogued repo is a refusal');
    const err = (await refused.json()) as { error: string };
    assert.match(err.error, /repoPath/, `the refusal must name the field: ${err.error}`);
    assert.deepStrictEqual(readIndexOnDisk(stranger), strangerBefore);

    // Naming the repo you ARE in behaves like omitting the field.
