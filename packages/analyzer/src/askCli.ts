...
const USAGE = `usage: sequence ask <question> [--repo <dir>] [--json] [--stdin]
  ...
  --permission <p>  what the turn may DO. Omit it and the turn is words only —
                    ...
                    this command did before the flag existed.
                      plan      full belt; changes are STAGED, never written,
                                and never a shell
                      build     staged changes ARE written, plus run_command
...
interface ParsedAskArgs {
   ...
   *
   *   plan  — the full reading belt; proposals are staged, never written, and
   *            `run_command` is refused
   *   build — proposals are WRITTEN through the jail and pre-write hooks, plus
   *            `run_command` under the same permission rules
   *
   * Two rungs since 2026-09-13 (owner: "teach mode, plan mode, build mode … it
   * should be pretty simple"). `propose` and `autoEdit` were the middle of a
   * four-rung ladder where each pair did one job; see state/types.ts.
   */
...
`);
  ...
  if (permissionArg !== undefined) {
    ...
    if (
      ...
      return {
        ...base,
        error: `--permission must be plan or build (got '${permissionArg}')`,
      };
    }
...
export async function runAskCli(argv: string[], io: AskCliIo = defaultIo): Promise<number> {
   ...
   */
  if (parsed.doneWhen && parsed.permission !== 'build') {
    io.err(
      `sequence ask: --done-when needs a writing mode (--permission build); ` +
