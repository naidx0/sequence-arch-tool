...
const EXAMPLES: { match: RegExp; example: Example }[] = [
  ...
  /*
   * A plain "plot a parabola" / "f(x)=x^2" ask wants the unit parabola, not a
   * shifted quadratic chosen for complete-the-square lessons. Owner walk
   * 2026-09-16: X/Y axes + grid + f(x)=x^2 on AI Canvas.
   */
  {
    match: /\bparabola\b|y\s*=\s*x\s*\^\s*2|f\s*\(\s*x\s*\)\s*=\s*x|x\s*\^\s*2|x\s*squared/i,
    example: { expression: 'x^2', noun: 'a parabola' },
  },
  /* Shifted quadratic for lessons that name the algebra, not the shape alone. */
  {
    match:
      /polynomial|quadratic|complete the square|derivative|slope|tangent|gradient descent|loss function|minimi[sz]/i,
    example: { expression: 'x^2 - 3x + 2', noun: 'an example quadratic' },
  },
];
