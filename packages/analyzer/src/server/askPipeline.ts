...
async function applyAskToolRoundResult(
  ...
  if (result.chart) {
    ...
    push({ type: 'chart:proposal', chart });
    /*
...
const CANVAS_BLOCK_CRAFT_SECTION = [
  ...
  'LABEL EVERY EDGE with what actually crosses it (`POST /api/ask`, `writes rows`, `on failure`). An unlabelled arrow asserts a relationship and names none.',
...
export function renderAskInstructionBelt(input: AskInstructionHashInput): string {
  ...
  const boardOn =
    !plotAsk &&
    (input.surface?.id === 'task-board' || isDrawishAskQuestion(input.question));
...
${todoText}${lastToolSectionText}`);
  ...
  if (
    ...
    const drawDemand = plotAsk
      ? '--- YOU HAVE NOT DRAWN ANYTHING ---\n' +
        'The user asked for a math/plot/chart picture, and no `propose_chart` call succeeded this turn. ' +
        'Prose is not a drawing. Reply with EXACTLY ONE ```sequence-tool fence calling `propose_chart` ' +
        '(prefer kind `"line"` for a curve/parabola). Do NOT call propose_topology or diagram.upsert. ' +
        'Keep it COMPACT. Do not call read tools.'
      : '--- YOU HAVE NOT DRAWN ANYTHING ---\n' +
    ...
    for (const tr of forcedRequests) {
      ...
      if (forcedResult.chart) {
        ...
        push({ type: 'chart:proposal', chart: forcedResult.chart });
      }
