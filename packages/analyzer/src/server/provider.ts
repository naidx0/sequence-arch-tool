...
/**
 * Retries the APP takes by default on the allow-listed transient failures
 * (wave C1). Applied by the metered route in repoServer, never by the primitive
 * below, which keeps a blank knob meaning one request.
 */
...
export class ProviderError extends Error {
   ...
   */
  fix?: 'provider';
  /** The provider's own Retry-After, in milliseconds, when it sent one (wave C1). */
  retryAfterMs?: number;
  constructor(
    message: string,
    opts?: { status?: number; body?: string; httpStatus?: number; fix?: 'provider'; retryAfterMs?: number },
  ) {
    super(message);
...
/**
 * WHAT KIND OF FAILURE THIS IS, for the retry loop and for the person reading
 * the composer (wave C1 of docs/research/carrying-harness-plan.md).
 *
 * Most harnesses retry on "looks transient" and burn their budget on a quota
 * error, which will not clear in four seconds. Sixteen kinds is what the
 * MiniMax Code harness separates; these are the ones this provider layer can
 * actually tell apart from a status and a body, and each has one plain
 * sentence. `quota` is the one that matters most: a 402, or a 429 whose body
 * says the credits are gone, is an answer, not weather.
 */
...
export function classifyProviderError(e: unknown): ProviderErrorKind {
  if (!(e instanceof Error)) return 'unknown';
  if (e.name === 'ReasoningOnlyError') return 'reasoning_only';
  if (!(e instanceof ProviderError)) {
    return /timed out|timeout/i.test(e.message) ? 'timeout' : /fetch failed|ECONN|ENOTFOUND|EAI_AGAIN|socket/i.test(e.message) ? 'network' : 'unknown';
  }
  if (e instanceof RateLimitError) return 'quota';
  const st = e.status;
  ...
  if (st === 400 || st === 422) return 'bad_request';
  if (st === 503 || st === 529 || st === 502) return 'overloaded';
  if (typeof st === 'number' && st >= 500) return 'server';
  if (/did not answer within/.test(e.message)) return 'timeout';
  if (/^provider request failed:/.test(e.message)) return 'network';
  if (/could not locate assistant text|exceeded the .*-byte cap/.test(e.message)) return 'empty';
  return 'unknown';
}

/** One plain sentence per kind — what the reader can do about it. */
export function describeProviderErrorKind(kind: ProviderErrorKind): string {
  switch (kind) {
    case 'auth':
      return 'the provider rejected the key; check the key and the provider in Settings';
    case 'bad_request':
      return 'the provider rejected the request; check the model name and the Advanced settings';
    case 'not_found':
      return 'the provider did not find that model or endpoint; check the model name and the base URL';
    case 'quota':
      return 'the account is out of credits or over its quota; retrying will not help until it is topped up';
    case 'rate_limited':
      return 'the provider is rate-limiting this key; it was retried and will clear on its own';
    case 'overloaded':
      return 'the provider is overloaded; it was retried and will clear on its own';
    case 'server':
      return 'the provider had an internal error; it was retried';
    case 'timeout':
      return 'the provider did not answer in time; a slower model or a longer Timeout in Advanced may help';
    case 'network':
      return 'the provider could not be reached; check the base URL and that the server is running';
    case 'too_large':
      return 'the request was larger than the provider accepts; a smaller scope or a bigger context window is needed';
    case 'reasoning_only':
      return 'the model spent its whole output on hidden reasoning; it was asked again with reasoning off';
    case 'empty':
      return 'the provider answered with nothing usable';
    default:
      return 'the provider failed for a reason this build does not classify';
  }
}

/** Parse an HTTP Retry-After header (seconds or an HTTP date) to milliseconds. */
...
function extractModelText(provider: ProviderKind, parsed: unknown): string | undefined {
   ...
   * it killed the whole turn. Reasoning is still not read HERE — it is not the
   * answer — but it is no longer thrown away either: {@link extractReasoningText}
   * reads it into its own channel, exactly as the streamed path does.
   */
...
/**
 * THE MODEL'S THINKING, FROM A BUFFERED REPLY. Never the answer.
 *
 * The buffered twin of the streamed accumulator's `reasoningText`, and it exists
 * for the parity reason the old "deliberately NOT read" comment gave: two paths
 * that read different fields are two products. Same two spellings (`reasoning`
 * is Ollama's, `reasoning_content` is DeepSeek's), same separate channel.
 *
 * `''` and `undefined` are NOT collapsed by the caller — an empty reasoning
 * string is "it used the channel and said nothing", which is still a different
 * fact from never using one. Here both arrive as `''` because nothing downstream
 * can act differently on them; the distinction that matters is empty vs not.
 *
 * ANTHROPIC IS UNTOUCHED. Its thinking arrives as `thinking` content blocks on
 * its own wire, which this function does not read and this repair does not
 * change.
 */
function extractReasoningText(provider: ProviderKind, parsed: unknown): string {
...
/**
 * `finish_reason` from a buffered reply, or undefined.
 *
 * Never read anywhere in this file before 2026-09-18, on either wire — so a
 * completion truncated at max_tokens was indistinguishable from a complete one.
 * That is exactly the shape a thinking model produces when it spends its whole
 * output budget reasoning, and `length` is the one word that makes the refusal
 * something a reader can act on.
 */
function extractFinishReason(provider: ProviderKind, parsed: unknown): string | undefined {
 ...
 *
 * A ProviderError subclass — so every `instanceof ProviderError` catch already
 * written still handles it — for the one failure this whole repair is about: a
 * 200 whose assistant content is empty while the reasoning channel is full.
 *
 * IT IS A SUBCLASS AND NOT A MESSAGE PATTERN because the retry above it has to
 * branch on it, and the repository's own rule for exactly this is that what
 * would fix a failure is set at the THROW SITE — a caller matching prose to
 * decide is a caller broken by the next edit to the prose.
 *
 * Before it, this shape returned `text: ''` and threw nothing at all: every
 * downstream honesty field reported success, the receipt recorded 0 output
 * tokens, and the reader was told the model did not produce an answer.
 */
export class ReasoningOnlyError extends ProviderError {
  /** How much thinking came back. The evidence that the turn was not empty. */
  readonly reasoningChars: number;
  /** `length` means it ran out of budget mid-thought; `stop` means it chose to. */
  readonly finishReason: string | undefined;
  constructor(reasoningChars: number, finishReason: string | undefined) {
    super(
      `the model spent its output on reasoning and returned no answer ` +
        `(${reasoningChars} characters of thinking` +
        `${finishReason !== undefined ? `, finish_reason: ${finishReason}` : ''}). ` +
        `Set Advanced › Reasoning effort to none, or raise Max output tokens.`,
      { fix: 'provider' },
    );
...
export interface ProviderStreamOptions {
  ...
  /**
   * The model's THINKING, as it arrives — never its answer.
   *
   * OpenAI-compatible wire only. Ollama and DeepSeek put a thinking model's
   * scratch work on `reasoning` / `reasoning_content` beside an empty
   * `content`, and this file read neither until 2026-09-18: a whole turn
   * (measured: 6,746 tokens in, 0 out, 27 s) reached the reader as "I did not
   * produce an answer".
   *
   * ABSENT IS NOT "DROP IT". The accumulator keeps the thinking whether or not
   * anyone is listening, because {@link refuseReasoningOnly} needs it to tell
   * "thought and said nothing" from "said nothing at all" — two faults with two
   * different fixes. This callback only decides whether a caller SEES it.
   *
   * Whatever reaches here must never be stored or rendered as the answer.
   */
...
export class StreamDeltaAccumulator {
  ...
  >();
  /**
   * THE MODEL'S THINKING, KEPT OUT OF THE ANSWER.
   *
   * Measured on Ollama 0.33.2, 2026-09-18: a thinking model (qwen3.5:4b,
   * deepseek-*) asked through `/v1/chat/completions` returns
   * `message.content: ""` with the whole reply in `message.reasoning`, and
   * `finish_reason: length` once max_tokens is spent thinking. One real turn —
   * 6,746 tokens in, 0 out, 27 s — reached the reader as "I did not produce an
   * answer", with the model's actual words on the floor.
   *
   * It is accumulated SEPARATELY and it is never `text`. Reasoning is not the
   * answer: folding it in would put a model's scratch work in a receipt as the
   * thing it said, which is the same class of lie as inventing content. It is
   * held so the turn can be EXPLAINED — and so {@link requestModelTextOnce} can
   * tell "thought and said nothing" apart from "said nothing at all", which are
   * two different faults with two different fixes.
   */
  private reasoningOut = '';
  /**
   * `finish_reason` off choice 0, last one wins.
   *
   * Never read before this, on either wire — so a completion truncated at
   * max_tokens was indistinguishable from a complete one, which is precisely
   * the case above. `length` is the fact that makes the refusal actionable.
   */
  private finishReasonOut: string | undefined;
  /** Live thinking, when a caller wants to show it. Optional; absent ⇒ accumulate only. */
  private readonly onReasoning: ((delta: string) => void) | undefined;

  constructor(wire: ProviderKind, onReasoning?: (delta: string) => void) {
    this.wire = wire;
  ...
  /**
   * Everything the model put on the reasoning channel. NEVER the answer — see
   * {@link reasoningOut}. Empty string means it never used one.
   */
  get reasoningText(): string {
    return this.reasoningOut;
  }
  /** `finish_reason` from choice 0, or undefined when the provider sent none. */
  get finishReason(): string | undefined {
    return this.finishReasonOut;
  }
  ...
  private pushOpenAi(f: Record<string, unknown>): string[] {
    ...
    for (let i = 0; i < choices.length; i++) {
      ...
      if (index === 0 && typeof choice.finish_reason === 'string') {
        this.finishReasonOut = choice.finish_reason;
      }
      /*
       * THE THINKING CHANNEL, READ AT LAST — into `reasoningOut`, never into
       * `text`.
       *
       * CORRECTION, 2026-09-18. This read nothing, and said so: "`reasoning_content`
       * (DeepSeek-R1 and friends) is deliberately NOT read: the non-streaming
       * extractor returns `message.content` alone, so folding reasoning in here
       * would make the two paths answer differently." The PARITY argument was
       * right and is preserved — `extractReasoningText` now reads the same two
       * fields on the buffered path, into the same separate channel, so the two
       * paths still answer identically. What was wrong was the conclusion that
       * parity required dropping it: dropping it made a thinking model's whole
       * turn vanish (6,746 tokens in, 0 out, "I did not produce an answer").
       *
       * `reasoning` is Ollama's spelling and `reasoning_content` is DeepSeek's.
       * Reading both costs nothing; reading one is a bug on the other's server.
       *
       * IT DOES NOT SET `textPartSeen`. A turn that only ever thought did not
       * carry a text part, and claiming it did would hand the caller an empty
       * answer with no explanation — the exact silence this repair exists to end.
       */
      if (index === 0) {
        const thought = delta.reasoning ?? delta.reasoning_content;
        if (typeof thought === 'string' && thought !== '') {
          this.reasoningOut += thought;
          this.onReasoning?.(thought);
        }
      }
...
async function readCappedStream(
  ...
  const acc = new StreamDeltaAccumulator(wire, onReasoning);
...
interface ModelTextResult {
  ...
  /**
   * THE PROVIDER'S OWN WORD FOR WHY IT STOPPED — `stop`, `length`, `tool_calls`.
   *
   * `extractFinishReason` has existed since the audit that found "a completion
   * truncated at max_tokens was indistinguishable from a complete one", and it
   * was read at exactly ONE site: `refuseReasoningOnly`, to put a number in an
   * error message. Every other caller got the text and no way to know whether
   * it was all of it.
   *
   * That is the whole of the defect the owner reported on 2026-09-19 — "it
   * seems to render incomplete under certain changes when I ran the breakdown
   * skill" — with an answer that stopped mid-list and a product that presented
   * it as finished. The bytes were never lost; the FACT that they were cut off
   * was, right here, and nothing downstream could have recovered it.
   *
   * Optional because a provider may send none, and inventing `stop` for a
   * silent provider would be the same lie in the other direction.
   */
  finishReason?: string;
  /**
...
async function requestModelTextOnce(
  ...
  if (!res.ok) {
    ...
    const probe = new ProviderError(`provider returned HTTP ${res.status}`, { status: res.status, body: errBody });
    const kind = classifyProviderError(probe);
    /* The status stays first, exactly as before; the kind's sentence follows
       it so the composer shows what to do rather than a number (wave C1). */
    throw new ProviderError(`provider returned HTTP ${res.status} — ${describeProviderErrorKind(kind)}`, {
      status: res.status,
      body: errBody,
  ...
  if (streaming && onDelta) {
    const { acc, raw } = await readCappedStream(
      res,
      wire,
      onDelta,
    ...
    if (acc.frames === 0) {
      ...
      refuseReasoningOnly(wire, textOut, wholeTools, extractReasoningText(wire, parsedWhole), extractFinishReason(wire, parsedWhole), opts.onReasoning);
    ...
    /*
     * BEFORE the no-text-part refusal below, and the order is the finding.
     *
     * The first cut put this after it, and the test caught it: a reasoning-only
     * stream carries no text part either, so the older and vaguer branch fired
     * first and the reader was told "could not locate assistant text" for a turn
     * whose whole cause and whole fix were known. Two exits that both accept the
     * same case prove nothing about either — the SPECIFIC diagnosis has to be
     * asked first, or it is unreachable.
     *
     * Live reasoning already reached `opts.onReasoning` frame by frame, so this
     * call is told there is nothing left to hand over.
     */
    refuseReasoningOnly(wire, acc.text, acc.toolRequests, acc.reasoningText, acc.finishReason);
  ...
  refuseReasoningOnly(
    ...
    modelText ?? '',
    toolRequests,
    extractReasoningText(wire, parsed),
    extractFinishReason(wire, parsed),
    opts.onReasoning,
  );
  const providerUsage = extractProviderUsage(wire, parsed);
  const finishReason = extractFinishReason(wire, parsed);
  return {
    text: modelText ?? '',
    providerUsage,
...
/**
 * THE ONE PLACE "IT THOUGHT AND SAID NOTHING" IS DECIDED.
 *
 * Three return sites reach this shape — streamed, buffered, and the fallback for
 * a provider that ignored `stream:true` — and all three used to return `''`. One
 * function rather than three copies, because three copies of a predicate is how
 * two of them end up disagreeing about what an empty answer is.
 *
 * EVERY CONJUNCT IS LOAD-BEARING, and each one alone is an ordinary turn:
 *
 *   text !== ''            a model that answered is not this failure, however
 *                          much it thought first.
 *   no tool requests       a tool-calls-only turn IS an answer — it carries
 *                          structured work — and B2.2 says so at the sites above.
 *   reasoning === ''       a genuinely empty reply with no thinking is the
 *                          OTHER fault, already reported by its own message; a
 *                          reader told to turn reasoning off would be sent to
 *                          fix a setting that was never the cause.
 *
 * `onReasoning` is handed the thinking on the buffered paths only. The streamed
 * path already delivered it frame by frame and passes nothing, or the caller
 * receives the same words twice.
 */
function refuseReasoningOnly(
  wire: ProviderKind,
  text: string,
  toolRequests: ProviderToolRequest[],
  reasoning: string,
  ...
  if (text !== '' || toolRequests.length > 0 || reasoning === '') return;
  /* Handed over BEFORE the throw: the thinking is the only evidence the turn
     produced anything, and a caller that wants to show it must get it even
     though this call is about to fail. */
  onReasoning?.(reasoning);
  throw new ReasoningOnlyError(reasoning.length, finishReason);
}
...
function isTransientProviderFailure(e: unknown): boolean {
  ...
  /* A quota error is an answer, not weather, whatever status it wears (wave C1). */
  if (classifyProviderError(e) === 'quota') return false;
  const st = e.status;
...
async function requestModelTextWithUsage(
  ...
  const params = cfg.params ?? {};
  /* ZERO HERE, BY DESIGN. This primitive sends exactly what it was given: a
     knob left blank is one request (`maxRetries defaults to 0` is locked in
     provider-model-config.test.ts and the anthropic-wire lock relies on it).
     The retries a person actually gets come from the CALLERS that own a
     seat — the CLI sets two, and the app's metered route sets
     DEFAULT_PROVIDER_RETRIES (wave C1) — so a bench or a probe that builds a
     config by hand is never surprised by silent re-sends. */
  const maxRetries = params.maxRetries ?? 0;
  const timeoutMs = params.timeoutMs;
  /*
   * ── ASK IT AGAIN, WITHOUT THE THINKING ────────────────────────────────
   *
   * Taken at most ONCE, and it is not a retry in the `maxRetries` sense: that
   * budget is for WEATHER — a 429, a 5xx, a socket that died — and this is not
   * weather. A thinking model that spent its whole output budget reasoning will
   * do it again every time, so re-sending the same body is guaranteed waste;
   * re-sending a DIFFERENT body is the fix. Measured on Ollama 0.33.2,
   * 2026-09-18: the same qwen3.5:4b turn that returned empty content answers in
   * 35 tokens with `finish_reason: stop` when `reasoning_effort: "none"` is on
   * the wire. (`think: false` is IGNORED on `/v1/chat/completions` — it is an
   * Ollama-native field, and sending it changes nothing.)
   *
   * WHY NOT SEND IT BY DEFAULT ON EVERY LOOPBACK CALL, which was the shape
   * first proposed: this file's own rule is that a knob left blank is OMITTED
   * from the body, because several OpenAI-compatible servers answer 400 to a
   * field they did not expect (see AiParams, and the byte-identical-body note
   * on `params`). Defaulting it on would put a new field in front of every
   * llama.cpp and vLLM user to fix a fault only thinking models have. Sent on
   * the SECOND attempt, it only ever reaches a server that has already proved
   * it understands reasoning by producing some.
   *
   * NEVER over the reader's own setting: someone who asked for `high` gets
   * high, and the honest error instead.
   */
  const mayDropReasoning =
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      const once = await requestModelTextOnce(effectiveCfg, prompt, attemptOpts);
      return { ...once, retries: attempt };
    } catch (e) {
      /*
       * ONCE, and only when not one delta has reached the caller — the same law
       * every other retry here obeys, and it holds by construction: this failure
       * IS "no text arrived". Checked before the deadline branch is irrelevant;
       * checked after it would mean a call that both thought-only and timed out
       * reported the wrong one.
       */
      if (e instanceof ReasoningOnlyError && mayDropReasoning && !droppedReasoning && !emitted) {
        droppedReasoning = true;
        effectiveCfg = {
          ...effectiveCfg,
          params: { ...(effectiveCfg.params ?? {}), reasoningEffort: 'none' },
        };
      ...
      if (attempt < maxRetries && !emitted && isTransientProviderFailure(e)) {
        ...
        /* Retry-After first, when the provider said how long; else exponential
           with half-jitter, so a burst of clients does not re-collide (wave C1). */
        const retryAfterMs = e instanceof ProviderError ? e.retryAfterMs : undefined;
        const exponential = Math.min(4_000, 250 * 2 ** attempt);
        const backoffMs =
          retryAfterMs !== undefined
            ? Math.min(15_000, retryAfterMs)
...
export async function generateTextWithUsage(
  ...
  retries?: number;
  /** See `ModelTextResult.finishReason` — why the provider stopped. */
  finishReason?: string;
}> {
