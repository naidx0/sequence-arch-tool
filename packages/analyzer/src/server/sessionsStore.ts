...
export interface SessionIndexEntry {
   ...
   *
   * NOT `title`, and the distinction is the feature. The title is a label on a
   * rail row; the server rewrites it from the transcript whenever the person
   * has not edited it (`titleFromChat` below), so it is ours as much as theirs.
   * The goal is a STANDING INSTRUCTION: it is rendered into every prompt this
   * session assembles, and the goal run aims at it. Owner, 2026-09-17: "Goal
   * right now just sets you a personal goal… in ML Harness, once you create a
   * goal, you can tell the agent to work on the goal, and that way we can
   * enforce long-running tasks even with weaker models."
   *
   * THREE STATES, AND `''` IS NOT THE SAME AS ABSENT.
   *   `undefined` — NEVER SET, and nothing in the engine may set it.
   *   `''`        — THE PERSON CLEARED IT.
   *   non-empty   — theirs, set by PUT {goal} and by nothing else.
   *
   * NO TURN EVER SETS THIS. Owner, walking the installed app 2026-09-17: "I
   * don't like that it sets the goal automatically." The store used to adopt
   * the first substantive user message as the standing goal, on the argument
   * that the person had already said what they wanted. That argument is wrong
   * about what a goal IS here: it is rendered into every later prompt and the
   * goal run aims at it, so a question typed once became a standing
   * instruction for every turn after it — which nobody asked for and which the
   * person then had to notice and clear.
   *
   * A goal now exists only when the person set one: `/goal`, the New goal
   * form, or any other caller of PUT {goal}. The middle state survives the
   * removal because a cleared goal and a never-set goal are still different
   * facts on the wire, and a JSON round-trip must not merge them.
   */
  goal?: string;
  /**
   * THE PLAN, PERSISTED ON THE SESSION — see `goalPlan.ts` for the format and
   * for why this is not `todoList.ts`'s per-turn list.
   *
   * It is in the INDEX rather than in a file beside the transcript on purpose:
   * the rail already reads the index, the goalbar renders from it with no
   * second fetch, and a plan that lived in its own file would be a second
   * answer to "what is this session doing" that the index is free to disagree
   * with.
   */
  plan?: PlanStep[];
}
...
export interface CanvasDocShape {
  ...
  charts?: unknown[];
  /*
   * WHERE EACH ARTIFACT SITS ON THE CANVAS PLANE, keyed by the pad's artifact
   * id (carrying-harness plan, wave B2). Typed here for the same reason
   * `charts` is: the field is carried verbatim either way, and a shape nobody
   * can see in the type is a shape nobody maintains. The CLIENT validates it
   * (`canvasMemory.parseFrames`) — this file stores the session's documents
   * and has never been their validator.
   *
   * It is deliberately NOT part of `canvasFingerprint`. Dragging a frame is a
   * click, not a turn, so by the owner's own rule (2026-09-02, "activity means
   * any type of AI call or tool call, or actual generation") it must not stamp
   * `updatedAt` and re-sort the session rail.
   */
...
/**
 * A FRESH ID CARRIES A RANDOM TAIL, ALWAYS.
 *
 * `defaultSessionId()` is seconds-since-epoch modulo 10,000 — the space wraps
 * every 2 h 46 m, and `existing` only knows ONE root's index. The client keys
 * three origin-wide localStorage pads (`sequence.seqdraw.<id>`,
 * `sequence.whiteboard.<id>`, `sequence.arch-scratch.<id>`) by this id and
 * never namespaces them by root, so a New Chat in `~` could mint the id of a
 * deleted thread — or of a thread in a different repo — and inherit that
 * thread's AI Canvas drawing verbatim (owner walk 2026-09-17: "when you make
 * a new chat it brings the same exact drawing from previously").
 *
 * The random tail was already the collision fallback; making it unconditional
 * closes the cross-root case the fallback could not see. `isCheckpointSessionId`
 * accepts both spellings, so nothing downstream changes shape.
 */
export function uniqueSessionId(existing: Set<string>): string {
  let id = `${defaultSessionId()}-${Math.random().toString(36).slice(2, 6)}`;
...
/**
 * THE GOAL AND PLAN FOR ONE SESSION — the single read every other seam uses.
 *
 * The pipeline, the two plan tools and the goal runner all need this, and each
 * one reaching into `readSessionIndex().sessions.find(...)` itself is three
 * places to get the missing-session case wrong. An unknown id answers `{}`,
 * never a throw: a goal that cannot be found is a session with no goal, which
 * is a state the prompt already handles.
 */
export function readSessionGoal(
  repoRoot: string,
  id: string,
): { goal?: string; plan?: PlanStep[] } {
  const index = readSessionIndex(repoRoot);
  const entry = index?.sessions.find((s) => s.id === id);
  if (!entry) return {};
  return {
    ...(typeof entry.goal === 'string' ? { goal: entry.goal } : {}),
    ...(Array.isArray(entry.plan) ? { plan: entry.plan } : {}),
  };
}
...
export function createSession(
  ...
  /* Seed an empty architecture board so New Chat never inherits another
     thread's board via a client race, and softLoad sees a known-empty file. */
  writeSessionBoard(
    repoRoot,
    id,
    JSON.stringify({
...
/**
 * Fork: new session keeps board + canvas from the source, empty transcript.
 * Whiteboard localStorage is copied on the client (not on disk).
 */
export function forkSession(
  ...
  const index = ensureSessionsMigrated(repoRoot);
  if (!index.sessions.some((s) => s.id === sourceId)) return null;
  const created = createSession(repoRoot, opts);
  const board = readSessionBoard(repoRoot, sourceId);
  if (board && board.trim()) {
    /* Re-key scratch graphId to the new session so persist binding matches. */
    let nextBoard = board;
    try {
      const parsed = JSON.parse(board) as { grounded?: { graphId?: string } };
      if (
        parsed?.grounded &&
        typeof parsed.grounded.graphId === 'string' &&
        parsed.grounded.graphId.startsWith('scratch:')
      ) {
        parsed.grounded.graphId = `scratch:${created.id}`;
        nextBoard = JSON.stringify(parsed);
      }
    } catch {
      /* keep raw board */
    }
    writeSessionBoard(repoRoot, created.id, nextBoard);
  }
  const canvas = readSessionCanvas(repoRoot, sourceId);
  if (canvas) {
    writeJson(repoRoot, sessionRel(created.id, 'canvas.json'), {
      ...canvas,
      sessionId: created.id,
    });
  }
  const withClone = updateSession(repoRoot, created.id, { clonedFromId: sourceId });
  return {
    index: withClone ?? created.index,
    id: created.id,
    chat: created.chat,
    meta: created.meta,
  };
...
/**
 * A STABLE COMPARISON OF TWO PLANS, so `updatedAt` moves on a real change and
 * not on a re-PUT of the same plan.
 *
 * Field-by-field rather than `JSON.stringify`: key ORDER survives a stringify,
 * so a client that builds `{status, text, id}` and one that builds
 * `{id, text, status}` would read as different plans and stamp the row on every
 * activate. That is the same shape of defect as F10 and it is cheaper to not
 * write than to find.
 */
function planFingerprint(plan: readonly PlanStep[] | undefined): string {
  if (plan === undefined) return '\u0000none';
  return plan.map((s) => `${s.id}\u0001${s.status}\u0001${s.text}\u0001${s.why ?? ''}`).join('\u0002');
}
...
export function updateSession(
  ...
  body: {
    ...
    mode?: 'work' | 'code';
    clonedFromId?: string;
    /** The standing goal. `''` CLEARS IT and records the clear — see the field. */
    goal?: string;
    /** The whole plan, replaced. `[]` clears it. Validated by the ROUTE, not here. */
    plan?: PlanStep[];
  },
): SessionIndex | null {
  const index = ensureSessionsMigrated(repoRoot);
  ...
  /*
   * A PLAN THAT CHANGED IS ACTIVITY; A GOAL THAT CHANGED IS NOT.
   *
   * By the owner's own definition above — "any type of AI call or tool call, or
   * actual generation" — the two land on opposite sides, and they look similar
   * enough that it is worth saying which is which. A plan moves because
   * `write_plan` or `mark_step_done` was CALLED, or because a goal run ticked a
   * step: that is a tool call inside the thread, so it stamps the row. A goal
   * is typed into a box and saved; like `title` and `pinned` it is a click, and
   * a click is not a turn (FINDING F10).
   *
   * Compared rather than unconditional, like every other trigger here: the
   * goalbar re-PUTs the identical plan on activate, and an unconditional bump
   * would re-stamp every session merely by opening it — the exact defect F10
   * was raised for.
   */
  if (body.plan !== undefined && planFingerprint(body.plan) !== planFingerprint(entry.plan)) {
    bumpUpdatedAt = true;
  }
  ...
  /*
   * `''` IS A VALUE HERE, WHICH IS WHY THIS IS NOT `.trim() &&` LIKE `title`
   * ABOVE. An empty title is meaningless (the row would have no label and the
   * store would re-derive one), so the title path treats it as "not supplied".
   * An empty GOAL is the person clearing it, and it has to be stored as `''`
   * rather than left `undefined`, because `undefined` is what re-arms
   * auto-adoption. Collapsing the two states is how a cleared goal comes back
   * on the next message.
   */
  if (typeof body.goal === 'string') {
    goal = body.goal.trim().slice(0, 600);
  }
  ...
  if (body.chat) {
    ...
    /*
     * THE CHAT WRITES THE TITLE AND NOTHING ELSE.
     *
     * A goal is NOT derived here, and the asymmetry with `title` one line above
     * is the whole point: a LABEL may follow the conversation, because being
     * wrong about it costs a row in a rail. A STANDING INSTRUCTION may not,
     * because being wrong about it costs every later turn — the goal is
     * rendered into each prompt this session assembles and the goal run aims at
     * it. See the `goal` field for the owner report this removal answers.
     */
  }
  if (body.meta) {
  ...
  const sessions = index.sessions.map((s) =>
    s.id === id
      ? {
          ...
          /* `goal` is spread only when it is a string, so a session that has
             never had one keeps the key ABSENT rather than gaining an explicit
             `undefined` — `''` and absent mean different things here and a
             JSON round-trip must not merge them. */
