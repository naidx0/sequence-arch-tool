/**
 * P3 — APPLY proposed file edits when Build is on.
 *
...
export function permissionAutoWrites(permission: string | undefined): boolean {
  /* Build applies directly; Plan stages for an accept. Default-deny, so an
     unknown value stages rather than writes — see askTools.ts on why this list
     is a membership test and never an exclusion list. */
