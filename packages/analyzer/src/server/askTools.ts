...
import { renderWalkToolHintSection, validateWalk, walkEvidence } from './teachWalk.js';
import {
  BOARD_TOOL_NAMES,
  BOARD_TOOL_SCHEMAS,
  executeBoardTool,
...
export const ASK_TOOL_ALLOWLIST: readonly AskToolName[] = [
  ...
  'update_todos',
  /*
   * THE SESSION'S PLAN, not the turn's list. `update_todos` above owns the
   * per-turn work list that dies with the turn; these two own the plan that
   * OUTLIVES it and that a goal run works down. Both are gated in
   * `askToolsForJobMode` on the session actually HAVING a goal — see there.
   */
  'write_plan',
  'mark_step_done',
  'walk_example',
...
export const ASK_DISPATCHABLE_TOOLS: readonly string[] = [
  ...
  CANVAS_STORY_TOOL,
  /*
   * THE BOARD WRITERS, and this is the line that makes them reachable.
   *
   * `ASK_TOOL_ALLOWLIST` is not this set — the comment above it says so — and
   * a `board.*` case added to the dispatch switch WITHOUT this entry is
   * unreachable code: `executeAskTool` refuses on the name before the switch
   * is ever consulted. That is exactly what happened on the first attempt here,
   * with a clean build and a green suite, and `board-tools-wired.test.ts`
 ...
 */
/**
 * THE TOOLS A LOCAL WORKSPACE CAN HONESTLY ANSWER WITH NO REPOSITORY.
 *
 * Deliberately short, and each absence is a reason rather than an oversight:
 *
 *   read_topology / who_calls / locate_symbol — need a SCANNED GRAPH. There is
 *     none, and answering from a folder walk would be the guessing the whole
 *     product exists to refuse.
 *   git_status / git_diff — need a repository.
 *   run_command — "here is a directory" must never quietly become "here is a
 *     shell". Widening that is a decision for the owner, not a fall-through in
 *     a gate.
 *
 * `search_files` is here because grepping a folder the user owns is exactly
 * what it says it is, and `propose_files` / `edit_file` because a workspace
 * nobody can write to is a folder, not a workspace.
 */
export const WORKSPACE_TOOLS: readonly AskToolName[] = [
  'read_file',
  'edit_file',
  'search_files',
  'propose_files',
...
/**
 * The same context, pointed at the workspace instead of the repository.
 *
 * BOTH the root and the resolver move, and that pair is the whole point. The
 * resolver a repo-attached server supplies closes over the repo root; leaving
 * it in place while swapping the root would jail every path against one
 * directory and open it in another — a refusal that reads like a security
 * check and is actually two halves of the context disagreeing.
 *
 * The graph and digest are dropped for the same reason: a tool given the
 * workspace must not be able to answer out of a repository's index.
 */
function workspaceContext(ctx: AskToolContext, root: string): AskToolContext {
  return {
    ...ctx,
    repoRoot: root,
    designMode: false,
 ...
 */
/**
 * HOW MUCH OF A GOAL THIS SESSION HAS — the gate on the two plan tools.
 *
 *   `none` no goal and no plan. Neither tool is offered, and the belt is
 *          byte-identical to the one built before this feature existed.
 *   `goal` a goal, no steps yet. `write_plan` only: there is a job to break
 *          down and nothing to tick.
 *   `plan` steps exist. Both, subject to the permission rule below.
 *
 * A TOOL OFFERED WHERE IT HAS NO JOB IS A ROUND THE MODEL MAY SPEND FINDING
 * THAT OUT — this module's own argument for taking `walk_example` out of the
 * base belt. A session with no goal handed `mark_step_done` will call it, be
 * refused, and pay a provider round for the refusal.
 */
export type AskGoalDepth = 'none' | 'goal' | 'plan';

export function askToolsForJobMode(
  jobMode?: AskJobMode,
  permission?: string,
  opts?: { teach?: boolean; plot?: boolean; goalDepth?: AskGoalDepth },
): readonly AskToolName[] {
  /*
   * `walk_example` IS TEACH'S OWN TOOL, so it is taken out of the base and the
   * teach branch below puts it back.
   *
   * Filtered HERE rather than at each return, because this function has five
   * exits and a rule enforced at four of them is a rule with a hole in it —
   * which is the shape of defect its own scar below is about. It closes the
   * fourth beat of the teaching journey; an engineer asking what breaks if a
   * service changes is not owed a guided tour of the answer, and a tool offered
   * where it has no job is a round the model may spend finding that out.
   */
  /*
   * THE PLAN TOOLS COME OFF THE BASE HERE, for the same reason `walk_example`
   * does and at the same place: this function has five exits, and a rule
   * enforced at four of them is a rule with a hole in it. The branches below
   * put back exactly what the session has earned.
   */
  const goalDepth: AskGoalDepth = opts?.goalDepth ?? 'none';
  const belt = ASK_TOOL_ALLOWLIST.filter(
    (t) => t !== 'walk_example' && t !== 'write_plan' && t !== 'mark_step_done',
  );
  /*
   * WHICH PLAN TOOLS THIS TURN GETS BACK.
   *
   * `write_plan` is offered whenever there is a goal, in EITHER mode. Breaking
   * a job into steps is what Plan mode is for, and a Plan turn that may not
   * write the plan makes Build the only way to plan — which is the opposite of
   * the mode's contract.
   *
   * `mark_step_done` is BUILD ONLY, and this is the line that makes "a goal run
   * must run in Build" true in the belt rather than only at the door. Ticking a
   * step is a claim that the work was DONE; Plan mode's whole contract is that
   * the turn ends in words and nothing was done. A Plan turn that could tick
   * steps would walk a plan to `[x][x][x]` having written nothing — a progress
   * display that lies, which is the failure `todoList.ts` rule 4 is about,
   * arriving through a different door.
   *
   * ML Harness refuses a run outright unless the mode is build (`longrun.start`
   * → `not_building`: "A run works the plan down, and plan mode does not run
   * steps"). `goalRunner.ts` carries that refusal; this carries its other half,
   * so a Plan turn cannot tick a step even when nobody started a run.
   */
  const planTools: AskToolName[] =
    goalDepth === 'none'
      ? []
      : goalDepth === 'goal'
        ? ['write_plan']
        : permission === 'build'
          ? ['write_plan', 'mark_step_done']
          : ['write_plan'];
  const base = jobMode === 'work' ? belt.filter((t) => t !== 'run_command') : belt;
  /*
   ...
   */
  /*
   * TEACH ALONE IS THE NARROW ONE NOW, and `plan` came off this line on
   * 2026-09-13 when the ladder collapsed to two rungs.
   *
   * The old `plan` was "reading tools only"; the new Plan is the old `propose`
   * under the name the owner uses for it — "make sure proposed mode is plan
   * mode … plan is just planning: mostly read only, writing in the AI Canvas
   * and simple files". A mode that stages file changes MUST be handed
   * `propose_files` and `propose_topology`, or the autonomy matrix's
   * `propose: true` cell is a claim the belt refuses to honour — which is the
   * two-surfaces-disagreeing defect this function's own scar is about, pointed
   * the other way.
   *
   * Caught by `plan-mode-tools.test.ts` the moment `plan` joined the loop of
   * modes that must not carry the shell: it carries the shell's absence AND
   * the staging tools, and only the first of those was true here.
   */
  if (opts?.teach === true) {
    /*
     * `update_todos` is not mutating and is still wrong on a lesson. The teach
     * contract is ONE concept, ONE visual, ONE check-in per turn
     * (docs/teach-mode.md): a model handed a work list would plan six concepts
     * and start working through them, which is the pacing failure the contract
     * exists to prevent, arriving through a tool instead of through prose.
     */
    /* NO PLAN TOOLS ON A LESSON, by the `update_todos` argument in this same
       block: a teach turn is ONE concept, ONE visual, ONE check-in, and a model
       handed a plan to work down starts working it instead of teaching. */
    return [
      ...base.filter((t) => !ASK_MUTATING_TOOLS.includes(t) && t !== 'update_todos'),
      /* Put back: the walk is the beat that turns a diagram into an
         explanation, and a lesson is the only turn that owes one. */
      'walk_example',
    ];
  }
  /*
   * ONLY BUILD KEEPS THE SHELL, and this gate carries a scar.
   *
   ...
   *
   * THE LADDER IS TWO RUNGS NOW (owner, 2026-09-13: "teach mode, plan mode,
   * build mode … it should be pretty simple"), and the collapse made this gate
   * SIMPLER rather than looser. It used to name three of four values and let
   * the fourth fall through, which is a default-deny written as a
   * default-allow; one typo in that list was the original defect. It is now
   * default-DENY: the shell is returned for `build` and for nothing else, so a
   * value this function has never heard of — a stale client, a hand-written
   * request body, a mode added later and not wired here — loses the shell
   * instead of inheriting it.
   *
   * Plan inherits propose's REFUSAL, not its old hole.
   */
  const withShell = permission !== 'build' ? base.filter((t) => t !== 'run_command') : base;
  /* Appended AFTER the shell gate, never inside `base`, so the two rules stay
     separately readable: the shell is a permission question, the plan tools are
     a "does this session have a goal" question, and folding them into one
     filter is how the next reader loses track of which decided what. */
  const withoutShell = [...withShell, ...planTools];
  /*
   * Plot/math asks must not see `propose_topology` in the native belt or hint —
   * owner walk 2026-09-15: "plot a parabola" invented Architecture modules.
   */
  if (opts?.plot === true) {
    return withoutShell.filter((t) => t !== 'propose_topology');
  }
  return withoutShell;
}
...
import { isDrawishAskQuestion, isPlotOrMathDrawAsk } from './askIntent.js';
import {
  argsFromToolObject,
  extractIdentifierFromPhrase,
  repairAskToolArgs,
  repairToolJsonText,
} from './askToolRepair.js';
import {
...
import {
  ...
  occurrenceLines,
  replaceSpans,
} from './askEditMatch.js';

/** Draw / diagram intent — re-exported from `askIntent.ts`. */
export { isDrawishAskQuestion, isPlotOrMathDrawAsk };
...
/**
 * THE BOUNDED WALK, for a caller outside the tool belt. The goal run's plan
 * skeleton and its harness read (carrying-harness plan, waves A6 and A7)
 * resolve the files a goal or a step names with the SAME walk and the same cap
 * `read_file`'s loose resolver uses — a second walk would be a second answer
 * to "what files does this repository have".
 */
export function eachRepoFile(repoRoot: string, visit: (rel: string) => boolean): void {
  let examined = 0;
  walkFiles(repoRoot, (rel) => {
...
export interface AskToolRequest {
  ...
  evidence?: string;
  /**
   * WHAT THE HARNESS REPAIRED to make this call executable (wave A1): alias
   * folds, a string parsed into an array, a `./` stripped. Named on the tool
   * result so the model learns the canonical shape and the reader sees why a
   * call ran that it did not send exactly. Empty or absent ⇒ sent as-is.
   */
  repairs?: string[];
}
...
export interface AskToolContext {
  ...
  /**
   * The block ids the CLIENT reported as already on the AI Canvas this turn,
   * from `surface.canvas`. The SAME list the prompt was rendered from, so a
   * `blockId` the model read out of "ALREADY ON THE AI CANVAS" and a `blockId`
   * the executor will accept can never disagree.
   *
   * Absent or empty ⇒ every `blockId` is refused, which is correct: nothing is
   * on the canvas to replace.
   */
  canvasBlockIds?: readonly string[];
  /**
   * THE WHOLE BOARD, not only the forty items the prompt rendered.
   *
   * `askSurface.ts` caps its listing at forty and reports the rest as a count.
   * A caller that forwarded only what it SHOWED would let a duplicate id
   * through against any item it hid, and would undercount the board for the
   * capacity check — `boardTools.BoardKnown` says so in its own comment.
   */
  boardKnown?: import('./boardTools.js').BoardKnown;
  /**
   * THE LOCAL WORKSPACE ROOT, when there is no repository.
   *
   * Its own field and never a stand-in for `repoRoot`: non-null `repoRoot`
   * means "a repository was scanned and the digest describes it", and every
   * grounding rule in `explain.ts` is written against that. See
   * `workspace.ts` for the whole argument.
   */
  workspaceRoot?: string | null;
  /**
   * The work list as it stood at the START of this round, so `update_todos`
   * can report what changed rather than only what was sent. Owned by the
   * pipeline, which replaces it after every successful call.
   */
  todos?: readonly import('./todoList.js').TodoItem[];
  /**
   * THE SESSION'S PLAN as it stood at the start of this round — what
   * `write_plan` rewrites and `mark_step_done` ticks. Owned by the pipeline,
   * which replaces it after every successful call, exactly like `todos`.
   */
  goalPlan?: readonly PlanStep[];
  /**
   * PUT THE PLAN ON DISK, NOW — before the tool result goes back to the model.
   *
   * A plan that only lives in the round's variables is a plan that a crash, a
   * Stop, or a closed window loses; and the goal run reads the plan off DISK
   * before every turn, so a tick that never landed means the next turn aims at
   * the step that was just finished and does it again. The whole mechanism
   * assumes the file is the truth.
   *
   * Absent ⇒ the call still succeeds and the plan lives for the turn only. That
   * is the honest behaviour for a caller with no session (the CLI, the bench, a
   * pipeline test), which must not be made to invent one.
   */
  persistPlan?: (plan: readonly PlanStep[]) => void;
  /**
   * The item ids of the chart drawn THIS TURN — the only parts a walk may
   * name. Empty means nothing has been drawn yet, and `walk_example` says so
   * rather than accepting a story about a picture nobody can see.
   */
  drawnPartIds?: readonly string[];
  /**
...
export interface AskToolResult {
  ...
  plan?: Concept[];
  /**
   * Set by `update_todos`. The server emits it as `todo:list` and carries it
   * into the next round's prompt. The WHOLE list every time — see todoList.ts.
   */
  todos?: import('./todoList.js').TodoItem[];
  /**
   * Set by `write_plan` / `mark_step_done` — the WHOLE session plan after the
   * call. The pipeline carries it into the next round and the server emits it
   * as `goal:plan`.
   */
  goalPlan?: PlanStep[];
  /**
   * Set by `walk_example`: one concrete input traced through the picture that
   * was drawn THIS TURN. The server emits it as `teach:walk`.
   */
  walk?: import('./teachWalk.js').TeachWalk;
  /**
   * Set by a `board.*` writer: one or two whiteboard items, ready for `wb/add`.
   * The server emits them as `board:item`. Never partial — a refusal yields
   * none, because half a connection is a line pointing at nothing.
   */
  boardItems?: import('./boardTools.js').BoardItem[];
  /**
   * Ids to remove from the board before appending `boardItems` (diagram.delete /
   * SystemBoard replace). Server emits them on the same `board:item` event.
   */
  boardRemoveIds?: string[];
...
async function executeAskToolUnderModeGates(
  ...
  /*
   * PLOT / MATH DRAWS MUST NOT LAND AS SERVICE MODULES.
   *
   * Owner walk 2026-09-15: "draw a parabola" called propose_topology /
   * diagram.upsert and invented InputModule-style cards on Architecture. The
   * belt no longer advertises those tools for plot asks; this refusal catches
   * a model that still reaches for them.
   */
  if (
    ctx.question &&
    isPlotOrMathDrawAsk(ctx.question) &&
    (name === 'propose_topology' || isDiagramToolName(name))
  ) {
    return { ok: false, evidence: plotMathArchitectureRefusal(name) };
  }
  /* Plot/math asks must land as `propose_chart`, not ASCII markdown on the
     artifact column — owner walk 2026-09-16 (parabola → Markdown card). */
  if (
    ctx.question &&
    isPlotOrMathDrawAsk(ctx.question) &&
    (isCanvasToolName(name) || isBoardToolName(name))
  ) {
    return {
      ok: false,
      evidence:
        `Refused — this ask is a math/plot/chart draw. \`${name}\` is not a chart. ` +
        `Call \`propose_chart\` with kind \`"line"\` (or the fitting chart kind) so Sequence ` +
        `renders a real picture on AI Canvas.`,
    };
  }
  if (isCanvasToolName(name)) {
    ...
    const result = executeCanvasWrite(name, args, ctx.canvasBlockIds);
  ...
  if (ctx.designMode || ctx.repoRoot === null) {
    ...
    /* `write_plan` / `mark_step_done` join this list because a PLAN IS NOT A
       CLAIM ABOUT A REPOSITORY. They read and write the session's own steps and
       touch no file, so there is nothing here for them to fabricate — and a
       General chat with a goal is exactly the case the goal run was asked for. */
    if (name === 'propose_topology' || name === 'propose_chart' || name === 'update_todos' || name === 'write_plan' || name === 'mark_step_done' || name === 'walk_example' || isBoardToolName(name) || isDiagramToolName(name) || name === CANVAS_STORY_TOOL || (isCanvasToolName(name) && ctx.canvasToolsEnabled)) {
      return dispatchAskTool(name, args, ctx, onFile);
    }
    /*
     * THE LOCAL WORKSPACE — the owner's "we know where it's writing files".
     *
     * Everything else here is refused because it would FABRICATE FILE EVIDENCE
     * about a repository nobody attached, and that rule is right and stays. It
     * has nothing to say about a folder the user owns and this chat keeps its
     * own work in: reading back a note the assistant itself wrote there is not
     * a claim about anyone's codebase.
     *
     * A NAMED SUBSET, not the whole belt. `read_topology` and `who_calls` need
     * a scanned graph and there is none. `git_*` needs a repository. And
     * `run_command` stays refused with no repo whatever the permission is —
     * "here is a directory" must not quietly become "here is a shell", which is
     * a widening nobody asked for and the sort of thing that should be a
     * decision rather than a fall-through.
     */
    if (ctx.workspaceRoot != null && WORKSPACE_TOOLS.includes(name as AskToolName)) {
      return dispatchAskTool(name, args, workspaceContext(ctx, ctx.workspaceRoot), onFile);
    }
    return {
      ok: false,
      evidence:
        `refused: ${name} needs an attached repo` +
        (ctx.workspaceRoot == null
          ? ''
          : ` (the local workspace supports ${WORKSPACE_TOOLS.join(', ')})`),
    };
  }
  return dispatchAskTool(name, args, ctx, onFile);
}
...
async function dispatchAskTool(
  ...
  switch (name) {
    ...
    case 'update_todos':
      return executeUpdateTodos(args ?? {}, ctx);
    case 'write_plan':
      return executeWritePlan(args ?? {}, ctx);
    case 'mark_step_done':
      return executeMarkStepDone(args ?? {}, ctx);
    case 'walk_example':
      return executeWalkExample(args ?? {}, ctx);
    case 'board.add_note':
    case 'board.add_card':
    case 'board.connect':
    case 'board.frame':
      return executeBoardWrite(name, args ?? {}, ctx);
    case 'diagram.upsert':
    case 'diagram.layout':
    case 'diagram.delete':
      return executeDiagramWrite(name, args ?? {}, ctx);
    ...
    default:
      if (isCanvasToolName(name)) {
        const result = executeCanvasWrite(name as CanvasToolName, args, ctx.canvasBlockIds);
        if (result.ok && result.block) {
          return {
            ok: true,
            evidence: result.evidence,
...
function executeReadFile(
  ...
  const strict = executeReadFileStrict(args, ctx, onFile);
  if (strict.ok || !ctx.repoRoot) return strict;
  const rel = asString(args.path);
  if (!rel || !strict.evidence.endsWith('not found')) return strict;
  /* RESOLVE, DON'T REFUSE (wave A2): a basename or a path tail that names
     exactly one file in the repository is read, and the evidence says which.
     Bounded by the same walk cap the search tools use. */
  let examined = 0;
  const loose = resolveRepoFileLoose(rel, (visit) => {
    walkFiles(ctx.repoRoot!, (candidate) => {
      if (examined >= ASK_TOOL_SEARCH_MAX_FILES) return false;
      examined += 1;
  ...
  if (loose.kind === 'one') {
    const again = executeReadFileStrict({ ...args, path: loose.hit }, ctx, onFile);
    if (!again.ok) return again;
    return { ...again, evidence: `${resolvedNote(rel, loose.hit, loose.how)} — ${again.evidence}` };
  }
  if (loose.kind === 'several') {
    return { ok: true, evidence: severalNote('read_file', rel, loose.hits, loose.how) };
  }
  return strict;
}

function executeReadFileStrict(
  args: Record<string, unknown>,
  ctx: AskToolContext,
  onFile?: (event: AskToolFileEvent) => void,
): AskToolResult {
  ...
  let byteCut = false;
  const byteCap =
    ctx.permission === 'build' ? ASK_TOOL_READ_CAP_BYTES_FULL : ASK_TOOL_READ_CAP_BYTES;
...
function executeEditFile(
  ...
  let applied = 0;
  /* What wave A8 forgave, if anything — named in the evidence, never silent. */
  ...
  for (let i = 0; i < parsed.length; i++) {
    ...
    if (hits === 0) {
      /*
       * ONE BOUNDED FUZZY PASS, FAIL-CLOSED (wave A8). The text is usually
       * there and the bytes differ — a smart quote, an em dash, a CRLF file,
       * trailing whitespace the model dropped when it copied. `fuzzyFindEdit`
       * matches past exactly those, maps the hit back to the ORIGINAL bytes,
       * and re-normalises that span to prove it says the same thing; anything
       * it cannot prove falls through to the refusal below unchanged.
       */
      const fuzzy = fuzzyFindEdit(content, needle);
      if (fuzzy.kind === 'one' || (fuzzy.kind === 'several' && edit.replaceAll)) {
        const spans = fuzzy.kind === 'one' ? [fuzzy.span] : fuzzy.spans;
        const matched = content.slice(spans[0]!.from, spans[0]!.to);
        content = replaceSpans(content, spans, edit.newString);
        applied += spans.length;
        repairs.push(
          `edit ${i + 1} matched past ${fuzzyNote(needle, matched)}` +
            (spans.length > 1 ? ` (${spans.length} places)` : ''),
        );
        continue;
      }
      if (fuzzy.kind === 'several') {
        return {
          ok: false,
          evidence:
            `refused: edit ${i + 1} of ${parsed.length} — "oldString" is not in ${normalized} as ` +
            `written, and matches ${fuzzy.spans.length} places once quotes, dashes and trailing ` +
            `whitespace are ignored (lines ${fuzzy.lines.join(', ')}). Include more surrounding ` +
            'lines so it is unique, or set replaceAll.',
        };
      }
    ...
    if (hits > 1 && !edit.replaceAll) {
      /* THE LINE NUMBERS ARE THE FIX (wave A8): "3 times" sends the model back
         to read the file, "lines 40, 96, 212" lets it widen the span it meant. */
      const lines = occurrenceLines(content, needle);
      const shown = lines.join(', ');
      const more = hits > lines.length ? ` and ${hits - lines.length} more` : '';
      return {
        ok: false,
        evidence:
          `refused: edit ${i + 1} of ${parsed.length} — "oldString" appears ${hits} times in ` +
          `${normalized}, starting at line${lines.length === 1 ? '' : 's'} ${shown}${more}. ` +
          'Include more surrounding lines so it is unique, or set replaceAll.',
      };
    }
  ...
  return {
    ok: true,
    evidence:
      `edited ${normalized} — ${applied} replacement${applied === 1 ? '' : 's'}` +
      (repairs.length > 0 ? ` (harness ${repairs.join('; ')})` : ''),
...
function executeReadTopology(
  ...
  const strict = executeReadTopologyStrict(args, ctx);
  if (strict.ok || !ctx.digest) return strict;
  const wanted = asString(args.service);
  if (!wanted || !strict.evidence.includes('unknown service')) return strict;
  /* RESOLVE, DON'T REFUSE (wave A2): `orders` for `svc:orders`, `gateway/` for
     the service whose dir that is. One reading runs; several are listed. */
  const loose = resolveServiceLoose(ctx.digest.services, wanted);
  if (loose.kind === 'one') {
    const again = executeReadTopologyStrict({ ...args, service: loose.hit }, ctx);
    if (!again.ok) return again;
    return { ...again, evidence: `${resolvedNote(wanted, loose.hit, loose.how)} — ${again.evidence}` };
  }
  if (loose.kind === 'several') {
    return { ok: true, evidence: severalNote('read_topology', wanted, loose.hits, loose.how) };
  }
  return strict;
}

function executeReadTopologyStrict(
  args: Record<string, unknown>,
  ctx: AskToolContext,
): AskToolResult {
...
/**
 * RESOLVE, DON'T REFUSE (wave A2). The strict resolver above is right to refuse
 * an invented name; it was wrong to refuse `askPipeline` when one file of that
 * basename exists. One honest reading runs, and the evidence says so; several
 * come back as evidence the model can pick from; none is still a refusal.
 */
function executeWhoCalls(args: Record<string, unknown>, ctx: AskToolContext): AskToolResult {
  const strict = executeWhoCallsStrict(args, ctx);
  if (strict.ok || !ctx.graph) return strict;
  const target = asString(args.target);
  if (!target || !strict.evidence.includes('found no node matching')) return strict;
  const loose = resolveGraphTargetsLoose(ctx.graph, target);
  if (loose.kind === 'one') {
    const again = executeWhoCallsStrict({ ...args, target: loose.hit }, ctx);
    if (!again.ok) return again;
    return { ...again, evidence: `${resolvedNote(target, loose.hit, loose.how)} — ${again.evidence}` };
  }
  if (loose.kind === 'several') {
    const byId = new Map(ctx.graph.nodes.map((n) => [n.id, n]));
    const names = loose.hits.map((id) => {
      const n = byId.get(id);
      return n?.path ? `${n.path.replace(/\\/g, '/')} (${id})` : id;
    });
    return { ok: true, evidence: severalNote('who_calls', target, names, loose.how) };
  }
  return strict;
}
...
/**
 * A phrase is not an identifier, but it usually contains one: "the runAskPipeline
 * function" (wave A2). The strict path refuses prose; this pulls the symbol out
 * and says so, and still refuses when nothing in the phrase could be code.
 */
function executeLocateSymbol(args: Record<string, unknown>, ctx: AskToolContext): AskToolResult {
  const strict = executeLocateSymbolStrict(args, ctx);
  if (strict.ok) return strict;
  const name = asString(args.name);
  if (!name || isIdentifier(name)) return strict;
  const extracted = extractIdentifierFromPhrase(name);
  if (!extracted) return strict;
  const again = executeLocateSymbolStrict({ ...args, name: extracted }, ctx);
  if (!again.ok) return again;
  return { ...again, evidence: `${resolvedNote(name, extracted, 'identifier in the phrase')} — ${again.evidence}` };
}
 ...
 */
function executeLocateSymbolStrict(args: Record<string, unknown>, ctx: AskToolContext): AskToolResult {
  const name = asString(args.name);
...
function executeWhoCallsStrict(args: Record<string, unknown>, ctx: AskToolContext): AskToolResult {
  const graph = ctx.graph;
  if (!graph) {
 ...
 */
/* ---------------------------------------------- the plotted-series contract -- */

/**
 * The chart kinds whose items ARE a plotted series: `label` is x, `value` is y.
 *
 * Every other kind draws boxes, bars, cards or lanes, where a repeated value is
 * ordinary and a missing one is fine. These two draw a CURVE, and a curve that
 * is a straight line at zero is not a weaker picture of the function — it is a
 * picture of a different function.
 */
const PLOTTED_SERIES_KINDS: ReadonlySet<string> = new Set(['line', 'scatter']);

/**
 * How many distinct y values a plotted series owes before it is a curve at all.
 *
 * Three, not two: two points are a straight segment, which any pair of numbers
 * produces, and the refusal exists to catch a series that carries no shape.
 */
export const CHART_MIN_DISTINCT_Y = 3;

/**
 * WHAT WENT WRONG, IN THE OWNER'S WORDS (installed app, MiniCPM5-2B, 2026-09-17):
 * asked for "a parabola, y = x²" the chart came back as a FLAT LINE AT ZERO,
 * titled "y = x² curve".
 *
 * Every guard held. `validateChart` checks the SHAPE of a chart — kind, ids,
 * labels, and that no `nodeId` claims a node the scanned graph does not have —
 * and it was written for node-link pictures where an item's `value` is optional
 * decoration. Nothing anywhere read the NUMBERS. So a model that wrote eleven
 * items with `value: 0` (or with no `value` at all) produced a valid chart, a
 * confident title, and a straight line, and the harness told the model it had
 * drawn a parabola.
 *
 * THE DATA CONTRACT, FOR line AND scatter ONLY:
 *   1. every point carries a finite numeric `value` — that is the y;
 *   2. the series has at least {@link CHART_MIN_DISTINCT_Y} distinct y values;
 *   3. when the x labels are all numbers, they are strictly monotonic — a
 *      function plot walks x from one end to the other, once.
 *
 * Checked PER SERIES (`group`), because a two-series chart where one series is
 * flat is still a chart whose flat series says nothing, and because a
 * whole-chart distinct count would be satisfied by two flat series at different
 * heights.
 *
 * THE REFUSAL NAMES THE FIX, and that is the half a validator usually skips.
 * The model reads this string and gets one more round; "invalid chart" buys a
 * retry of the same mistake, while the worked numbers for the case the owner
 * actually typed buy a parabola.
 */
export function degenerateSeriesProblems(chart: {
  kind: string;
  items: ReadonlyArray<{ label?: string; value?: number; group?: string }>;
}): string[] {
  if (!PLOTTED_SERIES_KINDS.has(chart.kind)) return [];
  if (chart.items.length === 0) return [];
  const bySeries = new Map<string, Array<{ label?: string; value?: number }>>();
  for (const item of chart.items) {
    const key = typeof item.group === 'string' && item.group !== '' ? item.group : '';
  ...
  for (const [key, points] of bySeries) {
    const named = key === '' ? 'the plotted series' : `series "${key}"`;
    const missing = points.filter((p) => typeof p.value !== 'number' || !Number.isFinite(p.value));
    if (missing.length > 0) {
      problems.push(
        `${named} has ${missing.length} of ${points.length} point(s) with no finite numeric ` +
          '`value`. On a line or scatter chart every item is a POINT: `label` is x and `value` ' +
          'is y, so each one needs a number you computed — not a blank.',
      );
      continue;
    }
    const ys = points.map((p) => p.value as number);
    const distinct = new Set(ys).size;
    if (distinct < CHART_MIN_DISTINCT_Y) {
      problems.push(
        `${named} is not a curve: ${points.length} point(s) with ${distinct} distinct y ` +
          `value(s)${distinct === 1 ? ` (all y = ${ys[0]})` : ''}. Sequence draws exactly the ` +
          'numbers you send, so this renders as a flat line and the title would be a claim the ' +
          'picture does not make. Compute y at each x and send the real values — y = x² over x ' +
          'in [-5,5] needs 11 points with distinct y (25, 16, 9, 4, 1, 0, 1, 4, 9, 16, 25).',
      );
      continue;
    }
    /*
     * MONOTONIC ONLY WHEN THE X LABELS ARE NUMBERS. "Jan, Feb, Mar" is a
     * perfectly good line chart and has no ordering this can check; a series
     * labelled -5 … 5 is a FUNCTION PLOT, and one whose x jumps about draws a
     * scribble over the same curve the model meant.
     */
    const xs = points.map((p) => Number(p.label));
    if (xs.every((x) => Number.isFinite(x)) && xs.length > 1) {
      const up = xs.every((x, i) => i === 0 || x > (xs[i - 1] as number));
      const down = xs.every((x, i) => i === 0 || x < (xs[i - 1] as number));
      if (!up && !down) {
        problems.push(
          `${named} plots numeric x labels that are not strictly monotonic ` +
            `(${xs.slice(0, 6).join(', ')}${xs.length > 6 ? ', …' : ''}). A function plot walks x ` +
            'from one end to the other, once: sort the points by x and drop any repeated x.',
        );
      }
    }
  }
...
function executeProposeChart(
  ...
  /*
   * THE NUMBERS, AFTER THE SHAPE. `validateChart` proved this is a chart and
   * that it claims no node the graph lacks; this proves a line or scatter chart
   * actually plots something. Refused rather than repaired: the harness cannot
   * know which function the model meant, and a picture we corrected would carry
   * the model's title over our data.
   */
  const degenerate = degenerateSeriesProblems(chart);
  if (degenerate.length > 0) {
    return {
      ok: false,
      evidence: `refused: propose_chart — ${degenerate.join(' ')}`,
    };
  }
  return {
    ok: true,
...
/**
 * `update_todos` — the model declares the WHOLE work list, every time.
 *
 * `todoList.ts` owns every rule and the reasons for them. This is the seam: it
 * refuses rather than repairs, reports what CHANGED rather than only what was
 * sent, and tells the model in its own result whether it is finished — because
 * the commonest failure of a self-managed list is a model that keeps
 * maintaining it after the work is done.
 */
/**
 * `write_plan` — the model breaks the goal into steps, or rewrites the ones it
 * could not work.
 *
 * REPLACES THE OPEN STEPS ONLY. `goalPlan.writePlan` owns that rule and the
 * argument for it (history is append-only; a model that could rewrite its own
 * ticked steps can un-finish work to buy turns). This is the seam: it refuses
 * rather than repairs, it PERSISTS before answering, and it tells the model
 * what to do next in its own result — because the commonest failure after a
 * successful plan write is a model that describes the plan instead of starting
 * it.
 */
function executeWritePlan(args: Record<string, unknown>, ctx: AskToolContext): AskToolResult {
  /* `steps` is the documented name; `plan` and `items` are the ones models
     reach for. Refusing one of those is a provider round spent on spelling. */
  const written = writePlan(ctx.goalPlan ?? [], args.steps ?? args.plan ?? args.items);
  if (!written.ok) {
    return { ok: false, evidence: `refused: write_plan — ${written.reason}` };
  }
  ctx.persistPlan?.(written.steps);
  const next = firstOpenStep(written.steps);
  const counts = planCounts(written.steps);
  return {
    ok: true,
    goalPlan: written.steps,
    evidence: `plan: ${counts.total} step(s) — ${written.added} written, ${counts.done} already done, ${counts.parked} parked`,
    content:
      `### write_plan: the plan is now ${counts.total} step(s), ${counts.done} done.\n` +
      `${renderPlanChecklist(written.steps)}\n` +
      (next
        ? `Work the FIRST open step now, with tools: ${next.text}. Do not write the plan again.`
        : 'Every step is already done or parked. Say so in one line and stop.'),
  };
}

/**
 * `mark_step_done` — tick one step, by a few of its words.
 *
 * THE ONLY THING THAT CLOSES A STEP. Not a sentence in the answer, not the
 * absence of an error, not the model saying "done" — this call, with words that
 * match exactly one open step. `goalPlan.ts`'s header carries the argument; the
 * short version is that a loop which reads prose for a verdict ends the moment
 * the model says something that sounds like finishing, and models say that
 * constantly.
 *
 * The result NAMES THE NEXT STEP, because "what now" is the question a model
 * asks the person when nothing answers it — and a run has no person to ask.
 */
function executeMarkStepDone(args: Record<string, unknown>, ctx: AskToolContext): AskToolResult {
  const marked = markStepDone(ctx.goalPlan ?? [], String(args.step ?? args.words ?? args.text ?? ''));
  if (!marked.ok) {
    const list =
      marked.matches && marked.matches.length > 0
        ? ` Open steps: ${marked.matches.map((m) => `"${m}"`).join('; ')}`
        : '';
    return { ok: false, evidence: `refused: mark_step_done — ${marked.reason}${list}` };
  }
  ctx.persistPlan?.(marked.steps);
  const { counts } = marked;
  return {
    ok: true,
    goalPlan: marked.steps,
    evidence: `ticked "${marked.ticked.text}" — ${counts.done} of ${counts.total} done`,
    content:
      `### mark_step_done: ticked "${marked.ticked.text}" (${counts.done} of ${counts.total} done).\n` +
      (marked.next
        ? `Go STRAIGHT ON to the next open step now, with tools: ${marked.next.text}. Do not stop to ` +
          'summarise and do not ask what to do next.'
        : 'Every step is ticked. Say so in one line and stop.'),
  };
}

function executeUpdateTodos(args: Record<string, unknown>, ctx: AskToolContext): AskToolResult {
  /* `items` is the documented name; `todos` is the one models reach for when
     the tool is called update_todos, and refusing that is a round spent on
     spelling. Both parse identically or neither does. */
  const parsed = parseTodos(args.items ?? args.todos);
  if (!parsed.ok) {
    return { ok: false, evidence: `refused: update_todos — ${parsed.reason}` };
  }
  const diff = diffTodos(ctx.todos ?? [], parsed.items);
  const left = remainingTodos(parsed.items);
  const done = todosSettled(parsed.items);
  const lines = [
    `### update_todos: ${todoEvidence(diff, parsed.items)}`,
    ...renderTodoSection(parsed.items),
  ];
  if (done) {
    lines.push(
      'The list is settled. Write the answer now — say what you did, and name any blocked ' +
        'step and why it is blocked.',
    );
  } else {
    lines.push(`Next: ${left[0]!.title}`);
  }
  return {
    ok: true,
    todos: parsed.items,
    evidence: todoEvidence(diff, parsed.items),
    content: lines.join('\n'),
  };
}

/**
 * `walk_example` — one concrete input, moved through the picture that was drawn.
 *
 * `teachWalk.ts` owns every rule and the reason for each. This is the seam, and
 * the only thing it adds is where `drawnPartIds` comes from: the chart THIS
 * TURN actually emitted, carried on the context by the pipeline. Never the
 * model's second telling of its own chart — that would let a walk invent the
 * picture it claims to be walking through.
 */
function executeWalkExample(args: Record<string, unknown>, ctx: AskToolContext): AskToolResult {
  const result = validateWalk(args, ctx.drawnPartIds ?? []);
  if (!result.ok) {
    return { ok: false, evidence: `refused: walk_example — ${result.reason}` };
  }
  const lines = [
    `### walk_example: ${walkEvidence(result.walk)}`,
    `Input: ${result.walk.input}`,
    ...result.walk.steps.map((s, i) => `${i + 1}. [${s.partId}] ${s.says}`),
    'The learner can now step through this beside the picture. Do not repeat the walk in prose ' +
      '— say what it SHOWS that the diagram alone did not.',
  ];
  return {
    ok: true,
    walk: result.walk,
    evidence: walkEvidence(result.walk),
    content: lines.join('\n'),
  };
}

/**
 * One `board.*` writer, against what the board actually holds.
 *
 * `boardTools.ts` owns every rule and its reasoning; this is the seam, and the
 * only thing it adds is where `BoardKnown` comes from — the client's own report
 * of the board this turn (`surface.board`), never a second guess at it.
 *
 * ABSENT GROUNDING IS NOT PERMISSIVE. With no `boardKnown` the module is handed
 * an empty node list and an empty board, which refuses every `add_card` (there
 * is nothing to ground against) and every `connect` (there is nothing to
 * connect) while still allowing a note and a frame. That is the honest reading
 * of "the client told us nothing": a sketch needs no repository, a card does.
 */
function executeBoardWrite(
  name: string,
  args: Record<string, unknown>,
  ctx: AskToolContext,
): AskToolResult {
  const known = ctx.boardKnown ?? { nodeIds: [], items: [] };
  const result = executeBoardTool(name, args, known);
  if (!result.ok) return { ok: false, evidence: result.evidence };
  return {
    ok: true,
    boardItems: result.items,
    evidence: result.evidence,
    content: result.content,
  };
}

function executeDiagramWrite(
  name: string,
  args: Record<string, unknown>,
  ctx: AskToolContext,
): AskToolResult {
  if (!isDiagramToolName(name)) {
    return { ok: false, evidence: `refused: unknown diagram tool ${name}` };
  }
  const known = ctx.boardKnown ?? { nodeIds: [], items: [] };
  const result = executeDiagramTool(name, args, known);
  if (!result.ok) return { ok: false, evidence: result.evidence };
  return {
    ok: true,
    boardItems: result.items,
    ...(result.removeIds && result.removeIds.length > 0
      ? { boardRemoveIds: result.removeIds }
      : {}),
    evidence: result.evidence,
    content: result.content,
  };
...
function executeRunCommand(
   ...
   */
  const widen = ctx.permission === 'build';
  /*
   ...
   */
  if (ctx.sandboxExec === true && ctx.permission === 'build' && ctx.repoRoot) {
...
export function repairToolJson(body: string): string {
  /* The reversals now live beside the argument repairs (`askToolRepair.ts`):
     curly quotes, trailing commas, and — since wave A1 — Python's single-quoted
     repr and its literals, each a reversal of one known transform. */
  return repairToolJsonText(body);
}

/**
 * Apply the argument repairs to a set of requests that arrived STRUCTURED (the
 * provider's own tool_calls), which never went through the fence parser. Every
 * repair is recorded on the request so the result line can name it.
 */
export function repairAskToolRequests(
  requests: ReadonlyArray<AskToolRequest>,
): AskToolRequest[] {
  return requests.map((r) => {
    const { args, repairs } = repairAskToolArgs(r.name, r.args);
...
/** The result line carries the repairs the call needed, so the model sees the canonical shape. */
export function withRepairNote(tr: AskToolRequest, result: AskToolResult): AskToolResult {
  if (!tr.repairs || tr.repairs.length === 0) return result;
  return { ...result, evidence: `${result.evidence} (harness repaired the call: ${tr.repairs.join('; ')})` };
}

/** Names the fence parser and bare-JSON salvage accept beyond the static allowlist. */
export function isFencedAskToolName(name: string): boolean {
  return (
    (ASK_TOOL_ALLOWLIST as readonly string[]).includes(name) ||
    isCanvasToolName(name) ||
    name === CANVAS_STORY_TOOL ||
    isBoardToolName(name) ||
    isDiagramToolName(name)
  );
}
    ...
    try {
      ...
      if (obj && typeof obj === 'object') {
        ...
        if (typeof r.name === 'string') {
          const allowed = isFencedAskToolName(r.name);
          if (!allowed) {
            malformed.push(
          ...
          if (allowed) {
            ...
            const folded = argsFromToolObject(r);
            const repaired = repairAskToolArgs(r.name, folded.args);
            const repairs = [...folded.repairs, ...repaired.repairs];
            requests.push({
              id,
              name: r.name,
              ...(repaired.args ? { args: repaired.args } : {}),
...
function tryParseAskToolRequest(raw: string): AskToolRequest | null {
  try {
    ...
    if (!isFencedAskToolName(r.name)) return null;
    const id =
      typeof r.id === 'string' && r.id.trim() !== '' ? r.id : r.name;
    const folded = argsFromToolObject(r);
    const repaired = repairAskToolArgs(r.name, folded.args);
...
export function salvageBareToolRequests(text: string): {
  ...
  const requests: AskToolRequest[] = [];
  const nameHit =
    /"name"\s*:\s*"(?:propose_topology|propose_chart|propose_files|read_file|search_files|run_command|git_status|git_diff|call_mcp|call_plugin|fetch_url|board\.(?:add_note|add_card|connect|frame)|canvas\.write_\w+|canvas\.set_story_route)"/g;
  const removals: { start: number; end: number }[] = [];
...
/**
 * THE BELT FOR A CHAT WITH NO REPOSITORY AND A WORKSPACE OF ITS OWN.
 *
 * Its own section rather than a line bolted onto the repo belt: the repo belt
 * opens "--- TOOLS (attached repo) ---" and everything under it is written
 * about a scanned repository. A chat that has no repository must not be handed
 * a section whose header asserts one — that is the two-surfaces-disagreeing
 * defect `askToolsForJobMode` already carries a scar about, with the subject
 * changed.
 *
 * Four tools, and the sentence that keeps them honest. See `WORKSPACE_TOOLS`
 * for why the other eleven are absent.
 */
export function renderWorkspaceToolHintSection(): string {
  return [
    '--- TOOLS (your local workspace — NOT a repository) ---',
    'No repository is attached. You DO have a local workspace folder this chat keeps its own ' +
      'work in, and you can read, search and write files there. It is a plain folder: it is not ' +
      'the user\'s codebase, it has not been scanned, and nothing in it describes a project ' +
      'unless you have actually read the file and it says so.',
    'Emit fenced tool calls, then stop:',
    '```sequence-tool',
    '{"id":"t1","name":"propose_files","args":{"title":"Save the notes","rationale":"why this file","files":[{"path":"notes.md","content":"…"}]}}',
    '```',
    'Available here: `read_file`, `search_files`, `edit_file`, `propose_files`. Paths are ' +
      'workspace-relative — never absolute, never `..`.',
    'NOT available with no repository: `read_topology`, `who_calls`, `locate_symbol` (they need ' +
      'a scanned graph), `git_status`, `git_diff`, and `run_command`. Do not call them and do ' +
      'not apologise for them; say plainly that the user would need to attach a repository.',
    'Never describe these files as "the repo", never infer a project structure from their names, ' +
      'and never claim what is in a file you have not read.',
  ].join('\n');
}
...
export function renderCanvasToolHintSection(): string[] {
  return [
    ...
    'INTERACTIVE IS ALLOWED: `canvas.write_html` RUNS SCRIPTS. A slider, a play button, a '
      + 'stepper, a canvas animation — an explanation the reader can drive beats one they '
      + 'can only look at. There is NO network inside it (default-src none), so everything '
      + 'must be inline; use Graphite variables (var(--ink-1), var(--accent)) to match the theme.',
    'Operational: `canvas.set_story_route` with `{ "title": string, "steps": [{ "blockId", "caption" }] }` for guided story navigation.',
    'Each takes `{ "title"?: string, "content": string, "blockId"?: string }`. Mermaid goes in `content` as source text.',
    'REVISE, DO NOT PILE UP: when the user asks to change, fix, extend or redraw something ' +
      'already on the canvas, pass `blockId` with that block\'s id (listed under "ALREADY ON THE ' +
      'AI CANVAS") and it is replaced in place. Omit `blockId` only when you mean to ADD a block. ' +
      'An id that is not on that list is refused — never invent one.',
 ...
 */
/** MCP / plugin / fetch hints — deferred on drawing turns so chart/board land first. */
export const ASK_MCP_TOOL_HINT_LINES: readonly string[] = [
...
export const ASK_DRAWING_PREFER_NATIVE_TOOLS_LINE =
  'For drawing on Sequence surfaces: math/plots/curves → `propose_chart` (kind line / data); repository architecture → `propose_topology` or `diagram.upsert`; freehand layout → `board.*` / canvas writers. Prefer those before `call_mcp` — MCP tools do not land on Architecture, the whiteboard, or AI Canvas.';

/** Soft-refuse architecture IR writers when the ask is a math/plot draw. */
export function plotMathArchitectureRefusal(name: string): string {
  return (
    `Refused — this ask is a math/plot/chart draw, not repository architecture. ` +
    `\`${name}\` places service/module IR on Architecture or SeqDraw. Call \`propose_chart\` ` +
    `(kind \`"line"\` for a function/curve, or another chart kind that fits) instead. ` +
    `Do not retry ${name}.`
  );
}
...
export function renderAskToolHintSection(
  jobMode?: AskJobMode,
  permission?: string,
  opts?: { teach?: boolean; deferMcpHints?: boolean; plot?: boolean },
): string[] {
  ...
  /*
   * THE WORK LIST, AND THE ONE SENTENCE THAT DECIDES WHETHER IT HELPS.
   *
   * "Several steps" is the gate. A model told it HAS a list will write one for
   * "what does this file do", and a three-row list on a one-round question is
   * pure ceremony in front of the answer — the same defect as a padded plan,
   * which PLAN_MODE_INSTRUCTIONS rule 6 already names.
   */
  /*
   * THE FENCE HINT FOR THE PLAN TOOLS — and it is not optional decoration.
   *
   * A local model that ignores native `tools` sees only these lines, and the
   * goal run's whole purpose is "enforce long-running tasks even with weaker
   * models". A weak model with no documented way to call `mark_step_done` never
   * ticks anything: every step gets two strikes, every step parks, and the run
   * stops with "the plan could not be worked" while the model was doing the
   * work. The belt the model is TOLD about is the belt it is given.
   */
  if (belt.includes('write_plan')) {
    lines.push(
      '`write_plan` args `{ "steps": ["…", "…"] }` — the plan for the STANDING GOAL, which ' +
        'outlives this turn. It replaces the remaining OPEN steps and leaves ticked and parked ' +
        'ones alone. Every step names an action and what to do it to; a condition ("baseline ' +
        'measured") is a gate, not a step, and is refused. Write it once, then work the first step.',
    );
  }
  if (belt.includes('mark_step_done')) {
    lines.push(
      '`mark_step_done` args `{ "step": "a few words from the line" }` — tick one open step the ' +
        'MOMENT it is done, then go straight on to the next one. This is the ONLY thing that ' +
        'closes a step: saying so in the answer does not, and the next turn will aim at it again.',
    );
  }
  if (belt.includes('update_todos')) {
    lines.push(
      '`update_todos` args `{ "items": [{ "id", "title", "status", "note"? }] }` — a VISIBLE work ' +
        'list for a job of SEVERAL steps. Call it once with every step you intend to do, then ' +
        'again after each one you finish. Send the WHOLE list every time: it replaces the ' +
        'previous one, so a list missing a finished step un-finishes it on screen. status is ' +
        'pending | active | done | blocked; exactly one may be active; a blocked step MUST say ' +
        'why in note. Do NOT use it for a question you can answer in one round, and stop calling ' +
        'it once every step is done or blocked.',
    );
  }
  ...
  if (belt.includes('propose_files')) {
    const autoWrite = permission === 'build';
    lines.push(
      autoWrite
  ...
  lines.push('To inspect the working tree: `git_status` (no args) and `git_diff` with args `{ "path": "rel.ts" }`.');
  if (!opts?.deferMcpHints) {
    lines.push(...ASK_MCP_TOOL_HINT_LINES);
  } else {
    lines.push(ASK_DRAWING_PREFER_NATIVE_TOOLS_LINE);
  }
  lines.push(
...
export interface OpenAiAskToolSupplemental {
  canvas?: boolean;
  board?: boolean;
}

/** Mirror `canvasToolsEnabled` / board belt gates for native OpenAI tool lists. */
export function askOpenAiToolSupplemental(input: {
  teachTurn: boolean;
  question: string;
  surfaceId?: string;
  /** Design mode = the home workspace: no repository, and the canvas is the surface. */
  designMode?: boolean;
}): OpenAiAskToolSupplemental | undefined {
  const plotAsk = isPlotOrMathDrawAsk(input.question);
  /* `designMode` mirrors `canvasToolsEnabled`'s home-workspace arm. A native
     tool list offering fewer tools than the belt describes is the belt lying
     to the model about what it can call. */
  const canvas =
    input.teachTurn ||
    input.designMode === true ||
    input.surfaceId === 'ai-canvas' ||
    isDrawishAskQuestion(input.question);
  /* Plot/math draws stay on propose_chart — do not arm diagram/board writers. */
  const board =
    !plotAsk && (input.surfaceId === 'task-board' || isDrawishAskQuestion(input.question));
...
/** Native canvas / board writers appended when the turn can reach those surfaces. */
export function openaiSupplementalAskToolDefinitions(
  opts: OpenAiAskToolSupplemental,
  ...
  if (opts.canvas) {
    for (const name of CANVAS_TOOL_NAMES) {
      out.push({
        ...
        function: {
          name,
          description: `Write a block on the AI Canvas (${name}).`,
          parameters: CANVAS_TOOL_SCHEMAS[name] as Record<string, unknown>,
        },
      });
    }
    out.push({
      ...
      function: {
        name: CANVAS_STORY_TOOL,
        description: 'Set guided story navigation on the AI Canvas.',
        parameters: CANVAS_STORY_TOOL_SCHEMA,
      },
    });
  }
  if (opts.board) {
    for (const name of BOARD_TOOL_NAMES) {
      out.push({
        ...
        function: {
          name,
          description: `Draw on the whiteboard (${name}).`,
          parameters: BOARD_TOOL_SCHEMAS[name] as Record<string, unknown>,
        },
      });
    }
    for (const name of DIAGRAM_TOOL_NAMES) {
      out.push({
        ...
        function: {
          ...
          description:
            name === 'diagram.upsert'
              ? 'Draw a SystemBoard on SeqDraw from IR (roles, tracks, issues). No coordinates.'
              : name === 'diagram.layout'
                ? 'Re-layout a SystemBoard IR on SeqDraw (same shape as upsert).'
                : 'Remove SeqDraw marks by id.',
          parameters: DIAGRAM_TOOL_SCHEMAS[name] as Record<string, unknown>,
        },
      });
    }
  }
...
export const ASK_TOOL_REGISTRY: Record<AskToolName, AskToolRegistryEntry> = {
  ...
  propose_files: {
    ...
    description: 'Propose file edits for human review (does not write to disk unless Build).',
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string' },
  ...
  update_todos: {
    uiVerb: 'update_todos',
    description:
      'Keep a visible work list for a job that takes several steps. Call it once at the start ' +
      'with every step you intend to do, then again after each step you finish. Send the WHOLE ' +
      'list every time — this REPLACES the previous one, so a list that has lost a finished step ' +
      'un-finishes it on the reader\'s screen. Each item is {id, title, status, note?} where ' +
      'status is pending | active | done | blocked. Exactly one step may be "active"; a "blocked" ' +
      'step MUST say why in note. Do not use this for a one-step answer, and stop calling it once ' +
      ...
      properties: {
        items: {
          ...
          items: {
            type: 'object',
            properties: {
              id: { type: 'string', description: 'Stable id for this step.' },
              title: { type: 'string', description: 'What the step is, in plain English.' },
              status: { type: 'string', enum: ['pending', 'active', 'done', 'blocked'] },
              note: { type: 'string', description: 'Required when blocked: why.' },
            },
  ...
  write_plan: {
    uiVerb: 'write_plan',
    description:
      "Write or rewrite THIS CONVERSATION'S PLAN — the steps that work the standing goal down. " +
      'It REPLACES the remaining OPEN steps and keeps everything already ticked or parked exactly ' +
      'as it is, so send the open steps you intend to do and nothing else. Every step must name an ' +
      ...
      'measured", "tests passing") is a gate, not a step, and is refused. This plan survives the ' +
      'turn: a run works it down one step per turn, so write it for a reader who has forgotten ' +
      'everything you know right now. Then work the first step — do not write the plan twice.',
    parameters: {
      type: 'object',
      properties: {
        steps: {
          type: 'array',
          description: 'The open steps, in order. One short imperative line each.',
          items: { type: 'string' },
        },
      },
      required: ['steps'],
  ...
  mark_step_done: {
    uiVerb: 'mark_step_done',
    description:
      "Tick one open step of this conversation's plan as done, by a few of its words. Call it the " +
      'MOMENT the step is actually done, then go straight on to the next open step without ' +
      'stopping to summarise. Send enough of the line to match exactly one open step: words that ' +
      'match two are refused rather than guessed, and words that match none are refused rather ' +
      'than invented. This is the ONLY thing that closes a step — saying a step is finished in ' +
      'the answer does not tick it, and the next turn will aim at it again.',
    parameters: {
      type: 'object',
      properties: {
        step: {
  ...
  walk_example: {
    uiVerb: 'walk_example',
    description:
      'AFTER you have drawn a picture, trace ONE concrete example through it so the learner sees ' +
      'the thing happen rather than only its parts. `input` must be a SPECIFIC thing ("the ' +
      'sentence \'the cat sat\'", "a 404 from /orders") and never "some data". Every step names ' +
      'a partId from the chart you just drew — any other id is refused and the whole walk is ' +
      'lost — with one clause saying what happens to THAT input at THAT part.',
    parameters: {
      type: 'object',
      properties: {
        input: { type: 'string', description: 'The concrete thing being traced.' },
        steps: {
          type: 'array',
          description: 'In the order the input actually moves. 2 to 12.',
          items: {
            type: 'object',
            properties: {
              partId: { type: 'string', description: 'An item id from the chart drawn this turn.' },
              says: { type: 'string', description: 'What happens to the input here, one clause.' },
            },
            required: ['partId', 'says'],
            additionalProperties: false,
          },
        },
      },
...
export function openaiAskToolDefinitions(
  ...
  supplemental?: OpenAiAskToolSupplemental,
): OpenAiAskToolDefinition[] {
  const out: OpenAiAskToolDefinition[] = [];
  ...
  if (supplemental) {
    out.push(...openaiSupplementalAskToolDefinitions(supplemental));
  }
  return out;
}
