...
export const INTERNAL_RESULT_FIELDS = [
  ...
  'carryOut',
  /*
   * THE SESSION'S PLAN AS THE TURN LEFT IT, and how many tools it called.
   *
   * Internal for the reason `carryOut` is: the CALLER owns the session, the
   * plan is already on disk (the plan tools persist it the moment they run),
   * and a client shown a copy would hold a second answer to "what is the plan"
   * that its own next GET is free to disagree with.
   *
   * `toolCallsMade` exists for the GOAL RUNNER's narration rule — "announced a
   * move and called no tool" is a claim about CALLS, and reading the answer's
   * prose for it is exactly the phrase-matching that whole feature is built to
   * avoid. A client renders the tool rows as they stream; it needs no count.
   */
  'goalPlan',
  'toolCallsMade',
] as const;
