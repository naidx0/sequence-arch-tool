...
export interface GetStatusDetachedResponse {
  attached: false;
  /**
   * WHY, when the server was ASKED for a repo and could not open it.
   *
   * Absent in the ordinary unattached case — the home screen — and present
   * only when a startup `--repo` was refused. "Nothing is open" and "what you
   * asked for could not be opened" are different sentences to a reader, and
   * only one of them owes an explanation.
   *
   * Added 2026-09-20: an unscannable folder used to take the whole server
   * process down, so this state could not exist. It can now, and the reason
   * the scan gave is the thing worth carrying — the alternative a reader was
   * being shown was "connect ECONNREFUSED 127.0.0.1:60416".
   */
  attachFailure?: { path: string; reason: string };
}
...
/** One entry of the local workspace. Paths are workspace-relative, never absolute. */
...
/**
 * WHERE A GENERAL CHAT KEEPS ITS FILES.
 *
 * ALWAYS 200, never 404. `exists: false` with a populated `root` is the honest
 * state of a machine that has never written anything, and is what lets a
 * surface name the folder before it holds anything — which is the whole point
 * of the endpoint. Reading it never creates the directory.
 */
export interface GetWorkspaceResponse {
  /** Absolute, so the user can find it in their own file manager. */
  root: string;
  exists: boolean;
