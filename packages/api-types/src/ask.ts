...
import type { PlanStep } from './sessions.js';

/* ------------------------------- SeqDraw wire ------------------------------ */

/** Point in board space — same as the whiteboard / SeqDraw model. */
export interface BoardWirePoint {
  x: number;
  y: number;
}

/** Free text sticky the agent may place (claims nothing about the code). */
export interface BoardWireText {
  kind: 'text';
  id: string;
  at: BoardWirePoint;
  text: string;
...
/** Shape mark (rect / ellipse / line / arrow). */
...
/** Grounded pointer at a scanned node — never invent nodeId. */
export interface BoardWireNodeRef {
  kind: 'noderef';
  id: string;
  at: BoardWirePoint;
  nodeId: string;
...
/**
 * One agent mark on Sequence's spatial canvas (SeqDraw / whiteboard).
 * Closed set — no freehand strokes from the model (those stay human-only).
 */
export type BoardWireItem = BoardWireText | BoardWireShape | BoardWireNodeRef;
...
export interface AskSurfaceCanvasBlock {
  /** The real block id — the only id a revision may name. */
  id: string;
  type: 'markdown' | 'mermaid' | 'html' | 'react' | 'svg';
  title?: string;
  /** Size of the block's payload, so "big" and "small" are not guesses. */
  chars: number;
  /**
   * FOCUSED RESOLUTION: the first N characters of the payload, sent for the
   * block the user selected (or the newest one) and for no other. Everything
   * else stays "blurry" — id, type, title, size. Ported from tldraw's prompt
   * parts; see `docs/research/agent-drawing-and-teaching-visuals.md` §3.
   */
  excerpt?: string;
}

/**
 * WHAT IS ALREADY DRAWN, so a turn can revise instead of only replacing.
 *
 * The single most valuable idea in the tldraw agent template, and the half
 * Sequence never had: the writers were a closed set from the first day, and the
 * agent could not see a single thing it had written.
 */
export interface AskSurfaceCanvas {
  blocks: AskSurfaceCanvasBlock[];
  /** Blocks beyond the cap — reported as a count, never silently dropped. */
  omitted?: number;
}

/** One item already drawn on the whiteboard, as the agent may see it. */
export interface AskSurfaceBoardItem {
  /** The item's real id — the only id an edit or a connection may name. */
  id: string;
  kind: 'stroke' | 'shape' | 'text' | 'noderef';
  label?: string;
  /** Rounded board coordinates. Sub-pixel precision is noise in a prompt. */
  at?: { x: number; y: number };
  /** Present only on a `noderef`: the scanned node this stands for. */
  nodeId?: string;
}

/**
 * WHAT IS ALREADY ON THE WHITEBOARD.
 *
 * POSITIONS ARE CARRIED, which is the one way this differs from
 * {@link AskSurfaceCanvas}. The AI Canvas is a document — blocks are an ordered
 * list and "where" means nothing. A whiteboard is SPATIAL: "put the cache next
 * to the database" cannot be followed by an agent that knows only what exists,
 * and an agent with no positions places everything at the origin, which draws a
 * pile rather than a diagram.
 */
 ...
 */
/**
 * THE SESSION'S STANDING GOAL AND PLAN, AS THE PROMPT SEES THEM.
 *
 * NOT A REQUEST FIELD, AND THAT IS THE DESIGN. The goal is stored on the
 * session (`SessionIndexEntry.goal` / `.plan`) and the server reads it from
 * there using `threadId`, so there is exactly one answer to "what is this
 * thread for". A client-supplied goal would be a second answer, free to
 * disagree with the stored one, and the goal run — which has no client at all,
 * only a persisted row and a server loop — would have to be the one to lose
 * that argument. It is also why an old client needs no change to benefit.
 *
 * `runLive` is the SWITCH BETWEEN TWO VOICES, ported from ML Harness
 * `conductor._goal_note` / `_plan_note`. Idle, the goal is CONTEXT: answer what
 * was asked, and do not grind a checklist nobody started. Live, it is an ORDER:
 * work the FIRST open step now, with tools, tick it, and go straight on. The
 * same words in both places produce a model that grinds a checklist when the
 * person asked a question, or one that writes a summary when the person pressed
 * Work on goal — both were measured there.
 */
export interface AskGoalContext {
  /** The person's own words. Absent or empty ⇒ no goal section at all. */
  goal?: string;
  /** `- [ ] / - [x] / - [!] … — parked: why`, as the pipeline renders it. */
  plan?: PlanStep[];
  /** True only while a goal run is working this session down. */
  runLive: boolean;
  /**
   * The step THIS turn is judged on — the first open one, chosen by the runner
   * before the turn and passed in so the prompt and the verdict cannot
   * disagree about which step was aimed at.
   */
  aimingAt?: string;
  /**
...
export interface PostAskRequest {
   ...
   */
  /*
   * Two rungs since 2026-09-13 — see web2/src/state/types.ts for why the
   * four-rung ladder collapsed. THE SERVER DEFAULT-DENIES: askTools.ts hands
   * the shell to `build` and to nothing else, so an old client still sending
   * `propose` or `autoEdit` loses the shell rather than inheriting it, which
   * is the safe direction for a value this build no longer knows.
   */
...
export type AskStreamEvent =
  ...
  | {
       ...
       *
       * The fourth beat of the teaching journey — ask what they know, draw the
       * thing, "have you seen this before?", then WALK an example through it.
       * The first three shipped; a box-and-arrow chart shows you the parts and
       * only a worked example shows what actually happens.
       *
       * Every `partId` is an item id of the chart this turn emitted — the
       * server refuses the whole walk otherwise — so the client can light the
       * part each beat is about instead of redrawing. One diagram, a moving
       * highlight; see `teachWalk.ts` for why that beat the alternative.
       */
      type: 'teach:walk';
      input: string;
  ...
  | {
      /**
       * ONE OR MORE MARKS THE AGENT DREW ON THE SPATIAL CANVAS (SeqDraw).
       *
       * Sequence's own agent-drawing protocol (port of the tldraw agent action
       * idea, not the tldraw package). Appended by id on the client so a
       * mid-stroke human edit is never replaced wholesale.
       */
      type: 'board:item';
      items: BoardWireItem[];
      /** Ids to remove before appending (diagram.delete / SystemBoard replace). */
      removeIds?: string[];
    }
  | {
      /**
