...
export const EXCLUDE_FILES = new Set([
  ...
  /*
   * And the site's link audit, for the same reason one step removed: `site/` is
   * an excluded tree above, so this test's subject is absent from the mirror
   * too. It is the third time this shape has appeared -- audits travelling
   * without the thing they audit -- which is why the rule now has a name in the
   * comment above rather than three separate one-off exclusions.
   */
