 ...
 *
 * STALENESS HERE IS NOT A PID CHECK, and that is deliberate. This driver never
 * asks whether a process is alive: `tasklist` says whether a sandbox is running,
 * and that is the question. The only process this file looks up is its OWN
 * (`bornOfSelf`, to write the lock), never a pid from a file and never a child
 * it spawned — so the Windows handle-retention trap that makes `OpenProcess`
 * report an exited child as alive cannot reach it. `gpu-lock.mjs` carries the
 * measurement, next to the probe it actually protects.
 *
