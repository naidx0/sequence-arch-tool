...
/*
 * ── THE LIVENESS PROBE MUST NOT BECOME A HANDLE PROBE ─────────────────────
 *
 * `holderAlive` pays for a PowerShell round-trip to ask WMI whether a pid is
 * running. That looks like an obvious thing to "simplify" into
 * `process.kill(pid, 0)` or a bare `OpenProcess`, and doing so would be a
 * correctness regression rather than a speed-up.
 *
 ...
 * ML BUILD measured it on ml-harness (`32673f0`), where it flaked a push-lock
 * test one run in six. Reproduced on this machine, six runs each:
 *
 ...
 *
 * A reaper that reads a dead holder as alive never clears a stranded lock, and
 * the card stays locked by nobody — the exact failure `pid`/`born` were added
 * to end.
 *
 * This is a source scan because the defect is a future EDIT, not current
 * behaviour: there is nothing to exercise yet, and the thing worth holding is
 * the choice of probe.
 *
 * IT SCANS CODE, NOT PROSE. The first version matched the comments that WARN
 * about `OpenProcess` and failed on both files, which would have left exactly
 * two options: delete the warning, or delete the test. A gate that punishes
 * documenting the hazard it guards is worse than no gate -- the same shape as
 * an example of a thing that must not BE the thing.
 */

/** Source with comments removed, so a warning about a pattern is not the pattern. */
function code(text) {
  return text
    .replace(/\/\*[\s\S]*?\*\//g, ' ')
    .replace(/(^|[^:])\/\/.*/g, '$1 ');
}
test('the card lock asks WMI whether a pid runs, not a handle whether it exists', () => {
  const src = code(fs.readFileSync(path.join(HERE, 'gpu-lock.mjs'), 'utf8'));

  assert.match(
    src,
    /Get-CimInstance Win32_Process -Filter "ProcessId=\$\{pid\}"/,
    'holderAlive no longer asks WMI — a handle probe reads an exited child as alive',
  );
  /* `process.kill(pid, 0)` is the tempting swap, and on Windows it is exactly
     the handle probe. Signalling is fine; probing with it is not. */
  assert.doesNotMatch(
    src,
    /process\.kill\([^)]*,\s*0\s*\)/,
    'process.kill(pid, 0) is a handle probe on Windows — it cannot decide a dead holder',
  );
  assert.doesNotMatch(src, /OpenProcess/, 'OpenProcess reads an exited child as alive');
});

test('and the sandbox driver does not grow a pid liveness check at all', () => {
  /*
   * Its staleness question is "is a sandbox running", which `tasklist` answers
   * about the machine. The only process it looks up is its own, to write the
   * lock. If a pid check ever appears here it inherits every trap above, so it
   * should arrive deliberately and with this test updated.
   */
  const src = code(fs.readFileSync(path.join(HERE, 'sandbox-run.mjs'), 'utf8'));
  assert.doesNotMatch(src, /OpenProcess/);
  assert.doesNotMatch(src, /process\.kill\([^)]*,\s*0\s*\)/);
  const cimLookups = src.match(/Get-CimInstance Win32_Process/g) ?? [];
  assert.equal(
    cimLookups.length,
    1,
    'exactly one process lookup is expected here — bornOfSelf, over this process',
  );
  assert.match(src, /ProcessId=\$\{process\.pid\}/, 'and it must be OUR pid, not one from a file');
});
