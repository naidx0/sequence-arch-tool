...
import { shouldAdhocSign, appBundleIn } from '../../packages/desktop/scripts/adhoc-sign.mjs';
import { isArtifact, manifest } from '../../packages/desktop/scripts/checksums.mjs';
...
describe('the app icon', () => {
  ...
  it('the generator that produced it is checked in beside it', () => {
    ...
    assert.match(
      ...
      /docs\/brand\/marks\/sequence-mark\.svg/,
...
/* ══════════════════════════════════════════════════════════════════════════
   "THE PACKAGE IS BROKEN" — 2026-09-19

   A person who downloaded the published 0.1.0 installer reported it as
   corrupted. The hosted asset was fetched and tested here the same day:
   200, 187,852,868 bytes exactly as published, an MZ header, and `7za t`
   answering "Everything is Ok" over 17,001 files. The archive is INTACT.

   So the fault is in everything around the file, and there are three of them.
   Each one below is a rule about one.
   ══════════════════════════════════════════════════════════════════════════ */

const BUILDER_YML = fs.readFileSync(
  path.join(ROOT, 'packages', 'desktop', 'electron-builder.yml'),
  'utf8',
);

describe('1 — an artifact with no installer to corrupt', () => {
  it('Windows ships a zip beside the NSIS installer', () => {
    /*
     * An unsigned 180 MB NSIS installer is the shape SmartScreen blocks and
     * that antivirus quarantines mid-write, and NSIS itself refuses a touched
     * file with "the installer you are trying to use is corrupted or
     * incomplete". A zip has none of that surface: no installer, no CRC, no
     * write outside the folder it lands in, and Windows extracts it natively.
     *
     * It is also the only artifact that answers the owner's ask literally —
     * "literally run the product from wherever" — on a machine where the
     * person may not install anything.
     */
    const win = BUILDER_YML.slice(BUILDER_YML.indexOf('\nwin:'), BUILDER_YML.indexOf('\nextraMetadata:'));
    assert.match(win, /target: nsis/, 'the installer is still the default path');
    assert.match(win, /target: zip/, 'no portable Windows artifact — nothing to fall back to');
  });

  it('macOS still ships both a dmg and a zip', () => {
    const mac = BUILDER_YML.slice(BUILDER_YML.indexOf('\nmac:'), BUILDER_YML.indexOf('\nwin:'));
    assert.match(mac, /target: dmg/);
    assert.match(mac, /target: zip/);
  });
});

describe('2 — an app macOS will actually open', () => {
  /*
   * `mac.identity: null` is an owner ruling (2026-09-10, no Apple developer
   * account) and it is kept. What it DOES is the problem: app-builder-lib
   * returns from `sign()` before any ad-hoc path is reached
   * (out/macPackager.js:182-189), so the bundle carries no signature at all.
   * Apple silicon refuses an unsigned executable outright — "Sequence is
   * damaged and can't be opened" — which is a corrupted-download report about
   * a perfectly good file.
   *
   * These lock the BRANCH rather than the platform running the suite. There
   * is no Mac in this project, and a check that only runs on the target
   * platform is how the Windows spawn bug above survived for a month.
   */
  it('the packer is told to run the hook', () => {
    assert.match(BUILDER_YML, /afterPack: scripts\/adhoc-sign\.mjs/);
  });

  it('signs on darwin and does nothing anywhere else', () => {
    assert.equal(shouldAdhocSign('darwin', {}), true);
    assert.equal(shouldAdhocSign('mas', {}), true);
    assert.equal(shouldAdhocSign('win32', {}), false);
    assert.equal(shouldAdhocSign('linux', {}), false);
  });

  it('STANDS DOWN WHEN A REAL CERTIFICATE IS CONFIGURED', () => {
    /*
     * Re-signing a properly signed bundle with "-" would replace a
     * distribution signature with a worthless one — a worse failure than the
     * one the hook fixes, because the artifact still looks signed.
     */
    assert.equal(shouldAdhocSign('darwin', { CSC_LINK: 'file:///cert.p12' }), false);
    assert.equal(shouldAdhocSign('darwin', { CSC_NAME: 'Developer ID Application: Someone' }), false);
    assert.equal(shouldAdhocSign('darwin', { CSC_IDENTITY_AUTO_DISCOVERY: 'true' }), false);
  });

  it('finds the bundle by extension, not by guessing the product name', () => {
    const dir = path.join('x', 'mac-arm64');
    assert.equal(appBundleIn(dir, () => ['Sequence.app', 'LICENSE']), path.join(dir, 'Sequence.app'));
    assert.equal(appBundleIn(dir, () => ['LICENSE']), null);
  });
});

describe('3 — a claim about a download somebody can check', () => {
  it('hashes what a person downloads and nothing else', () => {
    /*
     * A `.blockmap` is derived from the artifact beside it and nobody fetches
     * it by hand; `builder-debug.yml` is a log. Rows nobody can act on are
     * how a manifest becomes something people stop reading.
     */
    assert.equal(isArtifact('Sequence Setup 0.1.0.exe'), true);
    assert.equal(isArtifact('Sequence-0.1.0-arm64.dmg'), true);
    assert.equal(isArtifact('Sequence-0.1.0-win.zip'), true);
    assert.equal(isArtifact('Sequence Setup 0.1.0.exe.blockmap'), false);
    assert.equal(isArtifact('builder-debug.yml'), false);
  });

  it('writes the format sha256sum -c and shasum -c both read', () => {
    const text = manifest([
      { name: 'b.dmg', sha256: 'bb' },
      { name: 'a.exe', sha256: 'aa' },
    ]);
    /* SORTED, so two builds of the same artifacts produce the same file and a
       diff between them is a real change rather than a readdir shuffle. */
    assert.equal(text, 'aa  a.exe\nbb  b.dmg\n');
  });

  it('the portable artifact has a smoke that runs it OUTSIDE the build tree', () => {
    /*
     * `desktop-smoke --packaged` runs release/win-unpacked/Sequence.exe, which
     * is inside the tree it was built in - so a path baked in at build time
     * passes it. The Tauri shell in this project's history did exactly that.
     * The zip only earns its place if something extracts it elsewhere and
     * runs it, and the README records the measurement.
     */
    const smoke = path.join(ROOT, 'tools', 'desktop-zip-smoke.mjs');
    assert.ok(fs.existsSync(smoke), 'tools/desktop-zip-smoke.mjs is missing');
    const text = fs.readFileSync(smoke, 'utf8');
    assert.match(text, /Sequence Zip Smoke/, 'the extract path must carry a space');
    assert.match(text, /SEQUENCE_DESKTOP_SMOKE/, 'it must use the real main-process hook');
    assert.match(
      fs.readFileSync(path.join(ROOT, 'packages', 'desktop', 'README.md'), 'utf8'),
      /desktop-zip-smoke/,
      'a gate nobody is told about is a gate nobody runs',
    );
  });

  it('the dist script actually runs it, or the manifest is a script nobody calls', () => {
    const pkg = JSON.parse(
      fs.readFileSync(path.join(ROOT, 'packages', 'desktop', 'package.json'), 'utf8'),
    );
    assert.match(pkg.scripts.dist, /checksums\.mjs/);
  });
});
